...
\begin{description}
  ...
  \begin{description}
    ...
    \item[$\Y_R$] The set of Bandersnatch ring roots. A subset of $\Y_{384}$. See section \ref{sec:cryptography} and appendix \ref{sec:bandersnatch}.
  \end{description}
...
%  \item[$\mathcal{W}$] Unused.
%  \item[$\mathcal{X}$] Unused.
...
\begin{description}
  ...
  \item[$\psi$] Votes regarding any past judgements. %	ψ
...
\begin{description}
  \item[$\varepsilon$] The instruction sequence. %	ε
  ...
  \item[$\zeta$] The exit-reason resulting from all machine state transitions. %	ζ
