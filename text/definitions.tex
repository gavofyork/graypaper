...
\begin{description}
  ...
  \item[$\mathbb{U}$] The set of partial state, used during accumulation. See equation \ref{eq:partialstate}.  \item[$\mathbb{V}_{\mu}$] The set of validly readable indices for \textsc{pvm} \textsc{ram} $\mu$. See appendix \ref{sec:virtualmachine}.
...
\begin{description}
  ...
  \item[$\mathbf{D}$] The set of accumulated work-reports in the present block. See equation \ref{eq:accumulatedwork}.\\
...
\begin{description}
  ...
  \item[$\theta$] The gas counter. % θ
  \item[$\varrho$] The registers. %	ϱ
...
\begin{description}
  ...
  \item[$\mathsf{W}_E = 684$] The basic size of our erasure-coded pieces. See equation \ref{eq:erasurecoding}.
