...
\begin{description}
  ...
  \item[$\recent$] Log of recent activity. See equation \ref{eq:recentspec}. %	β
  \begin{description}
    \item[$\recenthistory$] Information on the most recent blocks. See equation \ref{eq:recenthistoryspec}.
    \item[$\beefybelt$] The Merkle mountain belt for accumulating Accumulation outputs. See equation \ref{eq:beefybeltspec}.
  \end{description}
  ...
  \end{description}
  \item[$\accountspre$] The (prior) state of the service accounts. See equation \ref{eq:serviceaccounts}. %	δ
  \begin{description}
    \item[$\accountspostacc$] The post-accumulation, pre-transfer intermediate state. See equation \ref{eq:accountspostaccdef}. %	δ
    \item[$\accountspostxfer$] The post-transfer, pre-preimage integration intermediate state. See equation \ref{eq:accountspostxferdef}. %	δ
  \end{description}
  \item[$\entropy$] The entropy accumulator and epochal randomness. See equation \ref{eq:entropycomposition}. %	η
  \item[$\stagingset$] The validator keys and metadata to be drawn from next. See equation \ref{eq:validatorkeys}. %	ι
  \item[$\activeset$] The validator keys and metadata currently active. See equation \ref{eq:validatorkeys}. %	κ
  \item[$\previousset$] The validator keys and metadata which were active in the prior epoch. See equation \ref{eq:validatorkeys}. %	λ
  \item[$\reports$] The pending reports, per core, which are being made available prior to accumulation. See equation \ref{eq:reportingstate}. %	ρ
  \begin{description}
    \item[$\reportspostjudgement$] The post-judgment, pre-guarantees-extrinsic intermediate state. See equation \ref{eq:removenonpositive}. %	δ
    \item[$\reportspostguarantees$] The post-guarantees-extrinsic, pre-assurances-extrinsic, intermediate state. See equation \ref{eq:reportspostguaranteesdef}. %	δ
  \end{description}
  \item[$\thestate$] The overall state of the system. See equations \ref{eq:statetransition}, \ref{eq:statecomposition}. %	σ
  \item[$\thetime$] The most recent block's timeslot. See equation \ref{eq:timeslotindex}. %	τ
  \item[$\authqueue$] The authorization queue. See equation \ref{eq:authstatecomposition}. %	ϕ
  \item[$\disputes$] Past judgments on work-reports and validators. See equation \ref{eq:disputesspec}. %	ψ
  \begin{description}
  ...
  \end{description}
  \item[$\privileges$] The privileged service indices. See equation \ref{eq:privilegesspec}. %	χ
  \begin{description}
  ...
  \end{description}
  \item[$\activity$] The activity statistics for the validators. See equation \ref{eq:activityspec}. % π
  \item[$\ready$] The accumulation queue. See equation \ref{eq:readyspec}. % ω
  \item[$\accumulated$] The accumulation history. See equation \ref{eq:accumulatedspec}. % ξ
  \item[$\lastaccout$] The most recent Accumulation outputs. See equations \ref{eq:lastaccoutspec} and \ref{eq:finalstateaccumulation}. % θ
\end{description}
...
\begin{description}
  ...
  \item[$\registers$] The registers. % ϑ
...
\begin{description}
  ...
  \item[$\Cauthpoolsize = 8$] The maximum number of items in the authorizations pool.
  \item[$\Cslotseconds = 6$] The slot period, in seconds.
  \item[$\Cauthqueuesize = 80$] The number of items in the authorizations queue.
  \item[$\Crotationperiod = 10$] The rotation period of validator-core assignments, in timeslots.
  \item[$\Cmaxpackagexts = 128$] The maximum number of extrinsics in a work-package.
  \item[$\Cassurancetimeoutperiod = 5$] The period in timeslots after which reported but unavailable work may be replaced.
  \item[$\Cvalcount = 1023$] The total number of validators.
  \item[$\Cmaxauthcodesize = 64,000$] The maximum size of is-authorized code in octets.
  \item[$\Cmaxbundlesize = 13,794,305$] The maximum size of an encoded work-package together with its extrinsic data and import implications, in octets.
  \item[$\Cmaxservicecodesize = 4,000,000$] The maximum size of service code in octets.
  \item[$\Cecpiecesize = 684$] The basic size of erasure-coded pieces in octets. See equation \ref{eq:erasurecoding}.
  \item[$\Csegmentsize = \Csegmentecpieces\Cecpiecesize = 4104$] The size of a segment in octets.
  \item[$\Cmaxpackageimports = 3,072$] The maximum number of imports in a work-package.
  \item[$\Csegmentecpieces = 6$] The number of erasure-coded pieces in a segment.
  \item[$\Cmaxreportvarsize = 48\cdot2^{10}$] The maximum total size of all unbounded blobs in a work-report, in octets.
  \item[$\Cmemosize = 128$] The size of a transfer memo in octets.
  \item[$\Cmaxpackageexports = 3,072$] The maximum number of exports in a work-package.
  \item[$\mathsf{X}$] Context strings, see below.
  \item[$\Cepochtailstart = 500$] The number of slots into an epoch at which ticket-submission ends.
...
\begin{description}
  \item[$\Xavailable = \token{\$jam\_available}$] \emph{Ed25519} Availability assurances.
  \item[$\Xbeefy = \token{\$jam\_beefy}$] \emph{\textsc{bls}} Accumulate-result-root-\textsc{mmr} commitment.
  \item[$\Xentropy = \token{\$jam\_entropy}$] On-chain entropy generation.
  \item[$\Xfallback = \token{\$jam\_fallback\_seal}$] \emph{Bandersnatch} Fallback block seal.
  \item[$\Xguarantee = \token{\$jam\_guarantee}$] \emph{Ed25519} Guarantee statements.
  \item[$\Xannounce = \token{\$jam\_announce}$] \emph{Ed25519} Audit announcement statements.
  \item[$\Xticket = \token{\$jam\_ticket\_seal}$] \emph{Bandersnatch Ring\textsc{vrf}} Ticket generation and regular block seal.
  \item[$\Xaudit = \token{\$jam\_audit}$] \emph{Bandersnatch} Audit selection entropy.
  \item[$\Xvalid = \token{\$jam\_valid}$] \emph{Ed25519} Judgments for valid work-reports.
  \item[$\Xinvalid = \token{\$jam\_invalid}$] \emph{Ed25519} Judgments for invalid work-reports.
\end{description}
