...
Note that the union of the two characterizes inputs to the Accumulation invocation function $\Psi_A$ (defined in equation \ref{eq:accinvocation}).
...
\begin{equation}
  ...
  \accpar\colon\abracegroup[\;]{\begin{aligned}
    ...
    &\ \begin{aligned}
      ...
      M(s) &\equiv R_A(s, \ps¬accounts, \local¬acc(\ps¬accounts\subb{s}_\sa¬authority), (\local¬acc(s)_\ao¬poststate)_\ps¬accounts) \\
      \mathbf{m} &= \set{\build{\kv{s}{M(s)}}{s \in \keys{\ps¬accounts}, M(s) \ne \none}} \\
...
A service's state can be modified by both itself and its authority service. The $R_A$ function resolves conflicting modifications of a service $s$, in favour of those made by the authority service ($\mathbf{a}$). Note that a service may only be removed by its authority service, and only when the service's storage is empty. As a service's code must be present in its preimage storage in order to be executed, and preimage removal must be staged over at least $\Cexpungeperiod$ slots, it is not possible for a service to execute in the same block in which it is removed. There is therefore never a conflict between service removal and service state modification for $R_A$ to resolve.
\begin{gather}
  R_D(\mathbf{a}, \mathbf{w}, \mathbf{b}) \equiv \set{\build{\kv{\mathbf{k}}{\mathbf{v}}}{\kv{\mathbf{k}}{\mathbf{v}} \in \mathbf{a}, \mathbf{k} \in \mathbf{w}}} \cup (\mathbf{b} \setminus \mathbf{w}) \\
  R_P(\mathbf{p}, \mathbf{l}) \equiv \set{\build{\kv{h}{\mathbf{p}\subb{h}}}{\kv{\tup{h, z}}{\mathbf{r}} \in \mathbf{l}, \mathbf{r} \ne \sq{}}} \\
  R_A^\dagger(\mathbf{o}, \mathbf{a}, \mathbf{w}, \mathbf{r}, \mathbf{b}) \equiv \tup{\begin{aligned}
    &\is{\sa¬codehash}{R(\mathbf{o}_\sa¬codehash, \mathbf{a}_\sa¬codehash, \mathbf{b}_\sa¬codehash)},
      \is{\sa¬storage}{R_D(\mathbf{a}_\sa¬storage, \mathbf{w}, \mathbf{b}_\sa¬storage)}, \\
    &\is{\sa¬requests}{R_D(\mathbf{a}_\sa¬requests, \mathbf{r}, \mathbf{b}_\sa¬requests)},
      \is{\sa¬balance}{\mathbf{b}_\sa¬balance},
      \is{\sa¬authority}{\mathbf{a}_\sa¬authority}, \\
    &\is{\sa¬authbalance}{\mathbf{a}_\sa¬authbalance + \mathbf{b}_\sa¬authbalance - \mathbf{o}_\sa¬authbalance},
      \is{\sa¬minaccgas}{R(\mathbf{o}_\sa¬minaccgas, \mathbf{a}_\sa¬minaccgas, \mathbf{b}_\sa¬minaccgas)}, \\
    &\is{\sa¬minmemogas}{R(\mathbf{o}_\sa¬minmemogas, \mathbf{a}_\sa¬minmemogas, \mathbf{b}_\sa¬minmemogas)}, \\
    &\is{\sa¬preimages}{R_P(\mathbf{o}_\sa¬preimages, R_D(\mathbf{a}_\sa¬requests, \mathbf{r}, \mathbf{b}_\sa¬requests))}, \\
    &\is{\sa¬created}{\mathbf{o}_\sa¬created},
      \is{\sa¬gratis}{\mathbf{o}_\sa¬gratis},
      \is{\sa¬lastacc}{\mathbf{o}_\sa¬lastacc},
      \is{\sa¬parent}{\mathbf{o}_\sa¬parent}
  \end{aligned}} \\
  \begin{aligned}
    R_A(s, \mathbf{o}, \mathbf{a}, \mathbf{b}) &\equiv \begin{cases}
      \mathbf{a}^\dagger &\when \mathbf{a}^\dagger = \none \lor \mathbf{o}\subb{s}_\sa¬authority = s \\
      R_A^\dagger(\mathbf{o}\subb{s}, \mathbf{a}^\dagger, \mathbf{w}, \mathbf{r}, \mathbf{b}\subb{s}) &\otherwise
    \end{cases} \\
    &\phantom{{}\equiv{}} \where \mathbf{a}^\dagger = (\mathbf{a}_\ao¬poststate)_\ps¬accounts\subb{s} \\
    &\phantom{{}\equiv{}} \also \mathbf{w} = \set{\build{\mathbf{k}}{\tup{d, \mathbf{k}} \in \mathbf{a}_\ao¬writtenkeys, d = s}} \\
    &\phantom{{}\equiv{}} \also \mathbf{r} = \set{\build{\tup{h, z}}{\tup{d, h, z} \in \mathbf{a}_\ao¬modifiedreqs, d = s}}
  \end{aligned}
\end{gather}

$\local¬fnprovide$ is the preimage integration function, which transforms a dictionary of service states and a set of service/blob pairs into a new dictionary of service states. Preimage provisions into services which no longer exist or whose relevant request is dropped are disregarded:
\begin{align}
...
The single-service accumulation function, $\accone$, transforms an initial state-context, a sequence of deferred-transfers, a sequence of work-reports, a dictionary of services enjoying free accumulation (with the values indicating the amount of free gas) and a service index into an alterations state-context, a set of modified service storage keys, a set of modified preimage requests, a sequence of \emph{transfers}, a possible accumulation-output, the actual \textsc{pvm} gas used and a set of preimage provisions. This function wrangles the work-digests of a particular service from a set of work-reports and invokes \textsc{pvm} execution with said data:
\begin{equation}
  \label{eq:acconeout}
  \acconeout \equiv \tuple{
    \begin{alignedat}{1}
      \isa{&\ao¬poststate}{\partialstate},\;
      \isa{\ao¬writtenkeys}{\protoset{\tuple{\serviceid, \blob}}},\;
      \isa{\ao¬modifiedreqs}{\protoset{\tuple{\serviceid, \hash, \bloblength}}},\;\\
