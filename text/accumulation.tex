...
\begin{align}
  \mathbf{W}^! &\equiv [ w \mid w \orderedin \mathbf{W}, |(w_\wr¬context)_\cx¬prerequisites| = 0 \wedge w_\wr¬srlookup = \emset ] \\
...
\begin{equation}
  E\colon\abracegroup{
    ...
    &(\mathbf{r}, \mathbf{x}) \mapsto \sq{\build{
...
\begin{align}
  ...
  \quad\where \mathbf{q} &= E(\concatall{\ready_{m\dots}} \concat \concatall{\ready_{\dots m}} \concat \mathbf{W}^Q, \srmap(\mathbf{W}^!))
\end{align}
...
\begin{equation}
  \accseq\colon\abracegroup{
    ...
    &\quad\also (j, \mathbf{o}', \mathbf{t}, \mathbf{b}, \mathbf{u}) = \accseq(g - \!\!\!\!\sum_{(s, u) \in \mathbf{u}^*}\!\!\!\!u, \mathbf{w}_{i\dots}, \mathbf{o}^*, \emset)
  }
\end{equation}

We come to define the parallelized accumulation function $\accpar$ which, with the help of the single-service accumulation function $\accone$, transforms an initial state-context, together with a sequence of work-reports and a dictionary of privileged always-accumulate services, into a tuple of the total gas utilized in \textsc{pvm} execution $u$, a posterior state-context $(\mathbf{d}', \mathbf{i}', \mathbf{q}', m', \mathbf{a}', v', \mathbf{x}')$ and the resultant accumulation-output pairings $\mathbf{b}$ and deferred-transfers $\concatall{\mathbf{t}}$:
\begin{equation}
  \accpar\colon\abracegroup[\;]{\begin{aligned}
    &\tup{\partialstate, \seq{\workreport}, \dict{\serviceid}{\gas}} \to \tup{\partialstate, \defxfers, \servouts, \gasused} \\
    &\tup{\mathbf{o}, \mathbf{w}, \mathbf{f}} \mapsto \tup{\tup{\mathbf{d}', \mathbf{i}', \mathbf{q}', m', \mathbf{a}', v', \mathbf{z}'}, \concatall{\mathbf{t}}, \mathbf{b}, \mathbf{u}}\!\!\!\!\!\!\\
    &\text{where:}\\
    &\ \begin{aligned}
      \mathbf{s} &= \set{\build{\mathbf{r}_s}{w \in \mathbf{w}, \mathbf{r} \in w_\mathbf{r}}} \cup \Fkeys{\mathbf{f}} \\
      \mathbf{u} &= \sq{\tup{s, \accone(\mathbf{o}, \mathbf{w}, \mathbf{f}, s)_u} \mid s \orderedin \mathbf{s}} \\
      \mathbf{b} &= \set{ \build{\tup{s, b}}{s \in \mathbf{s},\, b = \accone(\mathbf{o}, \mathbf{w}, \mathbf{f}, s)_b,\, b \ne \none }} \\
      \mathbf{t} &= \sq{\accone(\mathbf{o}, \mathbf{w}, \mathbf{f}, s)_\mathbf{t} \mid s \orderedin \mathbf{s}} \\
      ...
      &\tup{\mathbf{d}, \mathbf{i}, \mathbf{q}, m, \mathbf{z}} = \mathbf{o} \\
      \tup{m', \mathbf{a}^*, v^*, \mathbf{z}'} &= (\accone(\mathbf{o}, \mathbf{w}, \mathbf{f}, m)_\mathbf{o})_{\tup{m, \mathbf{a}, v, \mathbf{z}}} \\
      ...
      \mathbf{n} &= \bigcup_{s \in \mathbf{s}}(\set{ (\accone(\mathbf{o}, \mathbf{w}, \mathbf{f}, s)_\mathbf{o})_\mathbf{d} \setminus \Fkeys{\mathbf{d} \setminus \set{s}} }) \\
...
\begin{align}
  ...
  &\textstyle \mathbf{I} \equiv \set{ s \mapsto \tup{\sum_{(s, u) \in \mathbf{u}}(u), \len{N(s)}} \;\middle\mid\; N(s) \ne [] } \\
...
\begin{equation}
  \selectxfers\colon \abracegroup[\;]{
    ...
    d &\mapsto \sq{\,t \mid s \orderedin \serviceid,\ t \orderedin \mathbf{t},\ t_s = s,\ t_d = d\,}
  }\!\!\!\!
\end{equation}
...
\begin{align}
  \mathbf{x} &= \set{ s \mapsto \Psi_T(\accountspostacc, \thetime', s, \selectxfers(s)) \mid (s \mapsto a) \in \accountspostacc } \\
  \accountspostxfer &\equiv \set{ s \mapsto a' \;\middle\mid\; (s \mapsto \tup{a, u}) \in \mathbf{x} } \\
  &\where a' = \begin{cases}
...
\begin{align}
  ...
  &\textstyle \mathbf{X} \equiv \set{ d \mapsto \tup{\len{\selectxfers(d)}, u} \;\middle\vert\; \begin{aligned}
