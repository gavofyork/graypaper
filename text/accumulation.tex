...
Accumulation may be defined as some function whose arguments are $\mathbf{W}$ and $\accounts$ together with selected portions of (at times partially transitioned) state and which yields the posterior service state $\accountspostpreimage$ together with additional state elements $\iota'$, $\varphi'$ and $\privileges'$.
...
\begin{align}
  \accumulated &\in \seq{\{\hash\}}_\mathsf{E} \\
...
\begin{align}
  \ready &\in \seq{\seq{(\workreport, \{\hash\})}}_\mathsf{E}
\end{align}
...
\begin{align}
  \using m &= \H_\¬timeslot \bmod \mathsf{E}\\
...
Our formalisms begin by defining $\partialstate$ as a characterization of (\ie values capable of representing) state components which are both needed and mutable by the accumulation process. This comprises the service accounts state (as in $\accountspre$), the upcoming validator keys $\iota$, the queue of authorizers $\varphi$ and the privileges state $\privileges$. Formally:
\begin{equation}
  \label{eq:partialstate}
  \partialstate \equiv \left(\begin{aligned}
    &\isa{\mathbf{d}}{\dict{\N_S}{\serviceaccount}} \,,\;
    \isa{\mathbf{i}}{\seq{\mathbb{K}}_\mathsf{V}} \,,\;
    \isa{\mathbf{q}}{\seq{\seq{\hash}_\mathsf{Q}}_\mathsf{C}} \,,\\
    &\isa{m}{\N_S} \,,\;
    \isa{\mathbf{a}}{\seq{\N_S}_\mathsf{C}} \,,\;
    \isa{v}{\N_S} \,,\;
    \isa{\mathbf{z}}{\dict{\N_S}{\gas}}
  \end{aligned}\right)
\end{equation}
...
\begin{equation}
  \servouts \equiv \{(\N_S, \hash)\} \qquad \gasused \equiv \seq{\tup{\N_S, \gas}}
\end{equation}
...
\begin{equation}
  \Delta_+\colon\left\{\begin{aligned}
    &(\gas, \seq{\workreport}, \partialstate, \dict{\N_S}{\gas}) \to (\N, \partialstate, \defxfers, \servouts, \gasused) \\
...
\begin{equation}
  \Delta_*\colon\left\{\;\begin{aligned}
    &(\partialstate, \seq{\workreport}, \dict{\N_S}{\gas}) \to (\partialstate, \defxfers, \servouts, \gasused) \\
...
\begin{equation}
  \fnprovide\colon\left\{\begin{aligned}
    &(\dict{\N_S}{\serviceaccount}, \{\tup{\N_S, \blob}\}) \to \dict{\N_S}{\serviceaccount} \\
    &(\mathbf{d}, \mathbf{p}) \mapsto \mathbf{d}'\;\where \mathbf{d}' = \mathbf{d}\;\text{except:} \\
    &\quad\forall \tup{s, \mathbf{i}} \in \mathbf{p},\;
      s \in \keys{\mathbf{d}},\;
      \mathbf{d}[s]_\mathbf{l}[\blake(\mathbf{i}), |\mathbf{i}|] = \sq{}:\\
    &\qquad \mathbf{d}'[s]_\mathbf{l}[\blake(\mathbf{i}), |\mathbf{i}|] = [\tau']\\
    ...
    %\begin{aligned}
    %  &\mathbf{d}'[s]_\mathbf{l}[\blake(\mathbf{i}), |\mathbf{i}|] = \tau' \wedge \\
...
Given the result of the top-level $\Delta_+$, we may define the posterior state $\privileges'$, $\varphi'$ and $\iota'$ as well as the first intermediate state of the service-accounts $\accountspostacc$ and the Accumulation Output Log $\lastaccout'$:
\begin{align}
  \using &g = \max\left(\mathsf{G}_T, \mathsf{G}_A\cdot \mathsf{C} + \textstyle \sum_{x \in \mathcal{V}(\alwaysaccers)}(x)\right)\\
  \also &\mathbf{o} = (\accountspre, \iota, \varphi, \manager, \assigners, \delegator, \alwaysaccers) \\
  &(n, \mathbf{o}', \mathbf{t}, \lastaccout', \mathbf{u}) \equiv
    \Delta_+(g, \mathbf{W}^*, \mathbf{o}, \alwaysaccers) \\
  &(\accountspostacc, \iota', \varphi', \manager', \assigners', \delegator', \alwaysaccers') \equiv \mathbf{o}'
\end{align}
...
\begin{align}
  &\mathbf{I} \in \dict{\N_S}{\tuple{\gas, \N}} \\
...
\begin{align}
  \mathbf{x} &= \{ s \mapsto \Psi_T(\accountspostacc, \tau', s, \selectxfers(s)) \mid (s \mapsto a) \in \accountspostacc \} \\
...
\begin{align}
  ...
  &\mathbf{X} \in \dict{\N_S}{\tuple{\N, \gas}} \\
...
\begin{align}
  \xtpreimages &\in \lseq \ltuple\N_S\ts\blob\rtuple \rseq \\
  ...
  \forall \tup{s, \mathbf{p}} &\in \xtpreimages : Y(\accountspre, s, \blake(\mathbf{p}), |\mathbf{p}|)
\end{align}
...
\begin{align}
  \using \mathbf{P} = \{
    (s, \mathbf{p})
    \mid
      \tup{s, \mathbf{p}} \in \xtpreimages, Y(\accountspostxfer, s, \blake(\mathbf{p}), |\mathbf{p}|)
    \}\\
  \accountspostpreimage = \accountspostxfer \text{ ex. } \forall \tup{s\ts\mathbf{p}} \in \mathbf{P} : \left\{\,\begin{aligned}
      ...
      \accountspostpreimage[s]_\mathbf{l}[\blake(\mathbf{p}), |\mathbf{p}|] &= [\tau']
    \end{aligned}\right.\!\!\!\!
\end{align}
