...
We define the outer accumulation function $\accseq$ which transforms a gas-limit, a sequence of work-reports, an initial partial-state and a dictionary of services enjoying free accumulation, into a tuple of the number of work-results accumulated, a posterior state-context and the resultant accumulation-output pairings:
\begin{equation}
  \label{eq:accseq}
  \accseq\colon\abracegroup{
    &\tuple{\gas, \defxfers, \sequence{\workreport}, \partialstate, \dictionary{\serviceid}{\gas}} \to \tuple{\N, \partialstate, \local¬fnservouts, \local¬fngasused} \\
    &\tup{g, \mathbf{t}, \mathbf{w}, \psX, \mathbf{f}} \!\mapsto\! \begin{cases}
    ...
    &\quad\also \tup{\psX^*, \mathbf{t}^*, \mathbf{b}^*, \mathbf{u}^*} = \accpar(\psX, \mathbf{t},\mathbf{w}\sub{\dots i}, \mathbf{f}) \\
    &\quad\also \tup{j, \psX', \mathbf{t}, \mathbf{b}, \mathbf{u}} = \accseq(g - \!\!\!\!\sum_{\tup{s, u}\, \in \mathbf{u}^*}\!\!\!\!u, \mathbf{t}^*, \mathbf{w}\sub{i\dots}, \psX^*, \emset)
  }
\end{equation}
...
\begin{equation}
  ...
  \accpar\colon\abracegroup[\;]{\begin{aligned}
    &\tuple{\partialstate, \defxfers, \sequence{\workreport}, \dictionary{\serviceid}{\gas}} \to \tuple{\partialstate, \defxfers, \local¬fnservouts, \local¬fngasused} \\
    &\tup{\psX, \mathbf{t}, \mathbf{w}, \mathbf{f}} \mapsto \tup{
      \tup{
    ...
    &\ \begin{aligned}
      \using \mathbf{s} &= \set{\build{
        ...
        }} \cup \keys{\mathbf{f}} \cup \set{\build{\mathbf{x}_\dx¬dest}{\mathbf{x} \in \mathbf{t}}} \\
      \mathbf{u} &= \sq{\build{
