...
\newcommand*{\local¬fnpartitionxfers}{T}
...
The union of the two $\operandtuple \cup \defxfer$ characterizes inputs to the Accumulation invocation function $\Psi_A$ (defined in equation \ref{eq:accinvocation}).
...
It is possible for a service's state to be modified by both the service's Accumulate logic and its supervisor's Accumulate logic. We wish to avoid inconsistent state resulting from concurrent modification while not unnecessarily restricting parallel accumulation. To this end, we optimistically permit execution of a service's Accumulate logic and its supervisor's Accumulate logic in the same round, but discard all results\footnote{With one exception: provided preimages are not discarded} of the service's execution if both modify the service's state. Note that ejection of a service is considered a modification of its state.

As the result of accumulating a transfer cannot reasonably be discarded (this would cause funds to be lost), deferred transfers to a service are \emph{never} accumulated in parallel with execution of the Accumulate logic of the supervisor. We define the function $\local¬fnpartitionxfers$ which partitions a sequence of deferred transfers into those which may be accumulated in parallel with the Accumulate logic of the services $\mathbf{s}$ and those which must be accumulated in a later round.
\begin{equation}
  \begin{gathered}
    \local¬fnpartitionxfers(\mathbf{t}, \mathbf{s}, \mathbf{d}) \equiv \local¬fnpartitionxfers^*(\mathbf{t}, \mathbf{s}, \emptyset, \mathbf{d}) \\
    \local¬fnpartitionxfers^*\colon\abracegroup{
      ...
      \end{cases} \\
      &\quad\where q = (t_\dx¬dest \in \keys{\mathbf{d}} \land \effectivesupervisor{\mathbf{d}}{t_\dx¬dest} \in (\mathbf{s} \cup \mathbf{i}) \setminus \set{t_\dx¬dest}) \lor {} \\
      &\quad\phantom{\where q = {}} \exists s \in \mathbf{i} \setminus \set{t_\dx¬dest} : \effectivesupervisor{\mathbf{d}}{s} = t_\dx¬dest \\
      &\quad\also \mathbf{i}' = \begin{cases}
        \mathbf{i} &\when q = \top \lor t_\dx¬dest \not\in \keys{\mathbf{d}} \\
        \mathbf{i} \cup \set{t_\dx¬dest} &\otherwise
      \end{cases} \\
      &\quad\also \tup{\mathbf{t}^!, \mathbf{t}^Q} = \local¬fnpartitionxfers^*(\mathbf{t}, \mathbf{s}, \mathbf{i}', \mathbf{d})
    }
  \end{gathered}
\end{equation}

Note that a deferred transfer may still be discarded, and the funds effectively burnt, if by the point of its accumulation the destination service has been ejected. See the definition of $\Psi_A$ in equation \ref{eq:accinvocation}.
...
\begin{equation}
  ...
  \accseq\colon\abracegroup{
    ...
    &\quad\also \mathbf{s} = \set{\build{d_\wd¬serviceindex}{r \in \mathbf{r}\sub{\dots i}, d \in r_\wr¬digests}} \cup \keys{\mathbf{f}} \\
    &\quad\also \tup{\mathbf{t}^!, \mathbf{t}^Q} = \local¬fnpartitionxfers(\mathbf{t}, \mathbf{s}, \psX_\ps¬accounts) \\
  ...
  \accpar\colon\abracegroup[\;]{\begin{aligned}
    ...
    &\ \begin{aligned}
      ...
      N(\mathbf{o}) \in \acconeout &\equiv \tup{
        ...
        \is{\ao¬defxfers}{\sq{}},
        \is{\ao¬yield}{\none},
        \is{\ao¬gasused}{\mathbf{o}_\ao¬gasused},
        \is{\ao¬provisions}{\mathbf{o}_\ao¬provisions}
      } \\
      \local¬acc(s) &\equiv \begin{cases}
        N(\mathbf{o}) &\when s \in \mathbf{o}_\ao¬modifiedaccounts \land s \in \mathbf{y}_\ao¬modifiedaccounts \setminus \set{\effectivesupervisor{\ps¬accounts}{s}} \\
        \mathbf{o} &\otherwise
      \end{cases} \\
      &\phantom{{}\equiv{}} \where \mathbf{o} = \accone(\psX, \mathbf{t}, \mathbf{r}, \mathbf{f}, s) \\
      &\phantom{{}\equiv{}} \also \mathbf{y} = \accone(\psX, \mathbf{t}, \mathbf{r}, \mathbf{f}, \effectivesupervisor{\ps¬accounts}{s}) \\
      \mathbf{u} &= \sq{\build{
      ...
      M(s) &\equiv \begin{cases}
        (\local¬acc(s)_\ao¬poststate)_\ps¬accounts\subb{s} &\when s \in \local¬acc(s)_\ao¬modifiedaccounts \\
        (\local¬acc(\effectivesupervisor{\ps¬accounts}{s})_\ao¬poststate)_\ps¬accounts\subb{s} &\otherwise
      \end{cases} \\
...
The single-service accumulation function, $\accone$, transforms an initial state-context, a sequence of deferred-transfers, a sequence of work-reports, a dictionary of services enjoying free accumulation (with the values indicating the amount of free gas) and a service index into an alterations state-context, a set identifying the modified services, a sequence of \emph{transfers}, a possible accumulation-output, the actual \textsc{pvm} gas used and a set of preimage provisions. This function wrangles the work-digests of a particular service from a set of work-reports and invokes \textsc{pvm} execution with said data:
\begin{equation}
  \label{eq:acconeout}
  \acconeout \equiv \tuple{
    \begin{alignedat}{3}
      \isa{\ao¬poststate&}{\partialstate},\;
      \isa{&\ao¬modifiedaccounts&}{\protoset{\serviceid}},\;
