...
\begin{align}
  ...
  &\accone \colon \abracegroup[\;]{
    &\begin{aligned}
      ...
      \end{aligned}}
      &\to \acconeout \\
      \tup{\psX, \mathbf{t}, \mathbf{r}, \mathbf{f}, s} &\mapsto \Psi_A(\psX, \thetime', s, g, \mathbf{t}^* \concat \mathbf{i})
    \end{aligned} \\
    &\text{where:} \\
    &\ \begin{aligned}
      \mathbf{t}^* &= \sq{\build{
      ...
      g &= \subifnone{\mathbf{f}\sub{s}, 0} + \sum_{t \in \mathbf{t}^*}(t_\dx¬gas) + \!\!\!\!\!\!\!\!\sum_{w \in \mathbf{w}, \mathbf{r} \in w_\wr¬digests, \mathbf{r}_\wd¬serviceindex = s}\!\!\!\!\!\!\!\!(\mathbf{r}_\wr¬authgasused) \\
      \mathbf{i} &= \sq{\build{
