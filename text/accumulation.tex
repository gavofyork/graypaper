...
Accumulation may be defined as some function whose arguments are $\mathbf{W}$ and $\accounts$ together with selected portions of (at times partially transitioned) state and which yields the posterior service state $\accountspostpreimage$ together with additional state elements $\stagingset'$, $\varphi'$ and $\privileges'$.
...
Our formalisms begin by defining $\partialstate$ as a characterization of (\ie values capable of representing) state components which are both needed and mutable by the accumulation process. This comprises the service accounts state (as in $\accountspre$), the upcoming validator keys $\stagingset$, the queue of authorizers $\varphi$ and the privileges state $\privileges$. Formally:
\begin{equation}
  ...
  \partialstate \equiv \left(\begin{aligned}
    ...
    &\isa{m}{\N_S} \,,\;
    \isa{\mathbf{a}}{\seq{\N_S}_\corecount} \,,\;
    \isa{v}{\N_S} \,,\;
...
Given the result of the top-level $\Delta_+$, we may define the posterior state $\privileges'$, $\varphi'$ and $\stagingset'$ as well as the first intermediate state of the service-accounts $\accountspostacc$ and the Accumulation Output Log $\lastaccout'$:
\begin{align}
  \using &g = \max\left(\mathsf{G}_T, \mathsf{G}_A\cdot \corecount + \textstyle \sum_{x \in \mathcal{V}(\alwaysaccers)}(x)\right)\\
  \also &\mathbf{o} = (\accountspre, \stagingset, \varphi, \manager, \assigners, \delegator, \alwaysaccers) \\
  &(n, \mathbf{o}', \mathbf{t}, \lastaccout', \mathbf{u}) \equiv
    \Delta_+(g, \mathbf{W}^*, \mathbf{o}, \alwaysaccers) \\
  &(\accountspostacc, \stagingset', \varphi', \manager', \assigners', \delegator', \alwaysaccers') \equiv \mathbf{o}'
\end{align}
...
\begin{align}
  ...
  &\where a' = \begin{cases}
    a \exc a'_a = \thetime' &\when s \in \keys{\mathbf{I}} \\
    a &\otherwise
  \end{cases}
\end{align}
...
\begin{align}
  ...
  \forall i \in \N_\mathsf{E} : \ready'^\circlearrowleft_{m - i} &\equiv \begin{cases}
    ...
    E(\ready^\circlearrowleft_{m - i}, \accumulated'_{\mathsf{E} - 1}) &\when i \ge \thetime' - \thetime
  \end{cases}
\end{align}
...
\begin{align}
  \xtpreimages &\in \seq{\tuple{ \N_S,\, \blob }} \\
