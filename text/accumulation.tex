...
\begin{equation}
  \begin{gathered}
    ...
    \local¬fnpartitionxfers^*\colon\abracegroup{
      ...
      &\tup{\sq{t} \concat \mathbf{t}, \mathbf{s}, \mathbf{i}, \mathbf{d}} \equiv \begin{cases}
        \tup{\mathbf{t}^!, \sq{t} \concat \mathbf{t}^Q} &\when q = \top \\
        \tup{\sq{t} \concat \mathbf{t}^!, \mathbf{t}^Q} &\otherwise
      \end{cases} \\
      &\quad\where q = (t_\dx¬dest \in \mathbf{d} \land \effectivesupervisor{\mathbf{d}}{t_\dx¬dest} \in (\mathbf{s} \cup \mathbf{i}) \setminus \set{t_\dx¬dest}) \lor {} \\
