...
Accumulation of a work-package/work-report may be deferred in the case that it has a not-yet-fulfilled dependency or cancelled entirely in the case of an invalid dependency. Dependencies are specified as work-package hashes, and an epochal history of all such accumulated work-package hashes is maintained along with the segment roots. This is the state item $\accumulated$. Formally:
\begin{equation}
  \accumulated \in \seq{\dict{\H}{\H}}_\mathsf{S}\\
\end{equation}

We also maintain knowledge of not-yet-accumulated work-reports in the state item $\ready$. Each of these became available at most $\mathsf{S}$ blocks ago but have or had unfulfilled dependencies. Formally:
\begin{align}
  \ready &\in \seq{\seq{(\mathbb{W}, \{\H\})}}_\mathsf{S}\\
  \mathsf{S} &\equiv 1000
\end{align}
...
\begin{align}
  ...
  \mathbf{W}^Q &\equiv [ w \mid w \orderedin \mathbf{W}, (w_x)_p \ne \none ] \\
  \ready'_{\mathsf{S} - 1} &= R(\mathbf{W}^Q, \mathbf{D}) \\
  \accumulated'_{\mathsf{S} - 1} &= \{ TODO \} \\
...
TODO: Check segments-root of dependencies when dependencies are fulfilled.
...
\begin{align}
  &Q\colon\left\{\begin{aligned}
    ...
    &(\mathbf{r}, \mathbf{a}) \mapsto \begin{cases}
      [] &\when w = \none \\
      \mathbf{g} \frown Q(R(\mathbf{r}, \mathbf{g}), \mathbf{a} \cup m)\!\!\!\! &\otherwise
    \end{cases}\!\!\!\!
  \end{aligned}\right.\\
  \nonumber &\where m = \{((w_s)_h \mapsto (w_s)_e) \mid w \in \mathbf{g}\}\\
...
\begin{equation}
  \mathbf{W}^* \equiv \mathbf{W}^! \frown Q(\ready)
\end{equation}

We work with a limited amount of gas per block and therefore may not be able to process all items in $\mathbf{W}^*$ in a single block. We process as many as possible while staying within our gas limit and mark the work-reports that are accumulated as $\mathbf{D}$.
