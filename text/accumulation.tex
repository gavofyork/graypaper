...
It is possible for a service's state to be modified by both the service's Accumulate logic and its authority service's Accumulate logic. We wish to avoid inconsistent state resulting from concurrent modification while not unnecessarily restricting parallel accumulation. To this end, we optimistically permit execution of a service's Accumulate logic and its authority service's Accumulate logic in the same round, but discard all results\footnote{With one exception: provided preimages are not discarded} of the service's execution if both modify the service's state. Note that ejection of a service is considered a modification of its state.

As the result of accumulating a transfer cannot reasonably be discarded (this would cause funds to be lost), deferred transfers to a service are \emph{never} accumulated in parallel with execution of the Accumulate logic of the authority service. We define the function $T$ which partitions a sequence of deferred transfers into those which may be accumulated in parallel with the Accumulate logic of the services $\mathbf{s}$ and those which must be accumulated in a later round.
\begin{equation}
  \local¬fnpartitionxfers\colon\abracegroup{
    &\tuple{\sequence{\defxfer}, \protoset{\serviceid}, \dictionary{\serviceid}{\serviceaccount}} \to \tuple{\sequence{\defxfer}, \sequence{\defxfer}} \\
    &\tup{\sq{}, \mathbf{s}, \mathbf{d}} \mapsto \tup{\sq{}, \sq{}} \\
    &\tup{\sq{t} \concat \mathbf{t}, \mathbf{s}, \mathbf{d}} \mapsto \begin{cases}
      \tup{\mathbf{t}^!, \sq{t} \concat \mathbf{t}^Q} &\when d = \top \\
      \tup{\sq{t} \concat \mathbf{t}^!, \mathbf{t}^Q} &\otherwise
    \end{cases} \\
    &\quad\where d = t_\dx¬dest \in \mathbf{d} \land \fneffectiveauthority(\mathbf{d}, t_\dx¬dest) \in \mathbf{s} \setminus \set{t_\dx¬dest} \\
    &\quad\also \mathbf{s}' = \begin{cases}
      \mathbf{s} &\when d = \top \\
      \mathbf{s} \cup \set{t_\dx¬dest} &\otherwise
    \end{cases} \\
    &\quad\also \tup{\mathbf{t}^!, \mathbf{t}^Q} = \local¬fnpartitionxfers(\mathbf{t}, \mathbf{s}', \mathbf{d})
  }
\end{equation}
  ...
  \accpar\colon\abracegroup[\;]{\begin{aligned}
    ...
    &\ \begin{aligned}
      ...
      \local¬acc(s) &\equiv \begin{cases}
        N(\mathbf{o}) &\when s \in \mathbf{o}_\ao¬modifiedaccounts \land s \in \mathbf{y}_\ao¬modifiedaccounts \setminus \set{\fneffectiveauthority(\ps¬accounts, s)} \\
        \mathbf{o} &\otherwise
      \end{cases} \\
      &\phantom{{}\equiv{}} \where \mathbf{o} = \accone(\psX, \mathbf{t}, \mathbf{r}, \mathbf{f}, s) \\
      &\phantom{{}\equiv{}} \also \mathbf{y} = \accone(\psX, \mathbf{t}, \mathbf{r}, \mathbf{f}, \fneffectiveauthority(\ps¬accounts, s)) \\
      \mathbf{u} &= \sq{\build{
      ...
      M(s) &\equiv \begin{cases}
        ...
        (\local¬acc(\fneffectiveauthority(\ps¬accounts, s))_\ao¬poststate)_\ps¬accounts\subb{s} &\otherwise
      \end{cases} \\
