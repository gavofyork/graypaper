...
We define the outer accumulation function $\Delta_+$ which transforms a gas-limit, a sequence of work-reports, an initial partial-state and a dictionary of services enjoying free accumulation, into a tuple of the total gas utilized in \textsc{pvm} execution $u$, a posterior state-context $\mathbf{o}$ and the resultant deferred-transfers $\defxfer$ and accumulation-output pairings $\servouts$:
\begin{equation}
...
\begin{align}
  ...
  \forall i \in \N_\mathsf{E} : \ready'^\circlearrowleft_{m - i} &\equiv \begin{cases}
    ...
    [] &\when 1 \le i < s \\
    E(\ready^\circlearrowleft_{m - i}, \mathbf{W}^*_{\dots n}) &\when i \ge s\\
