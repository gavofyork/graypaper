...
However, faults and culprits should never list keys that have already been included in the punish-set following some previously seen offence (as per equations \ref{eq:culprits} and \ref{eq:faults}), which means that although logically these minimum offenses must exist for each corresponding verdict, they may not be actually included in this block's faults or culprits extrinsic. In fact, if they are already in the prior-state's punish-set, they must have been reported by an earlier offense extrinsic, possibly of a different nature than the one they'd be included in now (ie a guarantor of a bad report may have been included before as a fault offender because it voted against the consensus vote of a disputed different report). 
...
Because offenses can be produced later than the corresponding verdict, it is also not possible to check, at the time the verdict is included in a block, that any or all of the corresponding offenses have already been included in a suitable extrinsic. At best, we can demand eventual consistency that all of the eligible offenders will either be in prior-state or be reported in an extrinsic in some block after the one including the originating verdict. 
