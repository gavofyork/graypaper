...
Although this should not be expected in practice, in theory it is possible that the number of offenses could be large (up to 1/3V faults and up to 3 culprits for each bad verdict). In order not to burden block creation overmuch, we specify practical limits to the number of verdicts and offenses which can be included in a single block.

Formally,
\begin{align}
  \label{eq:limitextrinsicverdicts}\len{\xtverdicts} &\le \Cmaxextrinsicverdicts \\
  \label{eq:limitextrinsicoffenses}\len{\xtculprits} + \len{\xtfaults} &\le \Cmaxextrinsicoffenses
\end{align}
