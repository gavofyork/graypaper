...
Although this should not be expected in practice, in theory it is possible that the number of offenses could be large (up to $\onethird \valcount$ faults and up to three culprits for each bad verdict). In order not to burden block creation overmuch, we specify practical limits to the number of verdicts and offenses which can be included in a single block.
