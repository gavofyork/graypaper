...
There are some constraints placed on the composition of this extrinsic: any verdict containing solely valid judgments implies the same report having at least one valid entry in the faults sequence $\xtfaults$. Good verdicts should only be reported when there is at least one associated new fault offender, as they otherwise would have no effect. Bad verdicts should always be reported.
...
Although this should not be expected in practice, in theory it is possible that the number of offenses could be large (up to $\onethird$ of validators as faults and up to three culprits for each bad verdict). In order not to over burden block creation, we specify practical limits to the number of verdicts and offenses which can be included in a single block as can be seen in equation \ref{eq:disputesextrinsics}.
