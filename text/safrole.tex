...
\begin{align}
  \ticketaccumulator \in \sequence[:\epochlen]{\safroleticket} \,,\quad
  \sealtickets \in \sequence[\epochlen]{\safroleticket} \cup \sequence[\epochlen]{\bskey}
\end{align}
...
\begin{align}
    ...
    \safroleticket \in \sequence{\safroleticket} &\implies \abracegroup[\,]{
        ...
        &\H_\¬sealsig \in \bssignature{\H_\¬authorbskey}{\mathsf{X}_T \concat \entropy'_3 \append i_\st¬entryindex}{\encodeunsignedheader{\H}}\,,\\
        &\isticketed = 1
    }\\
    \label{eq:ticketconditionfalse}
    \safroleticket \in \sequence{\bskey} &\implies \abracegroup[\,]{
        &i = \H_\¬authorbskey\,,\\
        &\H_\¬sealsig \in \bssignature{\H_\¬authorbskey}{\mathsf{X}_F \concat \entropy'_3}{\encodeunsignedheader{\H}}\,,\\
        &\isticketed = 0
    }\\
  \H_\¬vrfsig &\in \bssignature{\H_\¬authorbskey}{\mathsf{X}_E \concat \banderout{\H_\¬sealsig}}{\sq{}} \\
...
\begin{equation}
  Z\colon\abracegroup[\,]{
    \sequence[\epochlen]{\safroleticket} &\to \sequence[\epochlen]{\safroleticket}\\
    \mathbf{s} &\mapsto \sq{\mathbf{s}_0, \mathbf{s}_{\len{\mathbf{s}} - 1}, \mathbf{s}_1, \mathbf{s}_{\len{\mathbf{s}} - 2}, \dots}\\
  }
\end{equation}

Finally, $F$ is the fallback key sequence function which selects an epoch's worth of validator Bandersnatch keys ($\sequence[\epochlen]{\bskey}$) from the validator key set $\mathbf{k}$ using the entropy collected on-chain $r$:
\begin{equation}\label{eq:fallbackkeysequence}
  F\colon \abracegroup[\ ]{
    \tuple{\hash,\,\sequence{\valkey}} &\to \sequence[\epochlen]{\bskey}\\
...
\begin{align}\label{eq:ticketsextrinsic}
  \xttickets &\in \sequence{\tuple{
    \isa{\xt¬entryindex}{\Nmax{\ticketentries}},\,
    \isa{\xt¬proof}{\bsringproof{\epochroot'}{\mathsf{X}_T \concat \entropy'_2 \append \xt¬entryindex}{\sq{}}}
  }} \\
