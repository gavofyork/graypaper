...
Finally, $\ticketaccumulator$ is the ticket accumulator, a sequence of best-scoring ticket identifiers to be used for the next epoch, and $\sealtickets$ is the current epoch's slot-sealer sequence:
\begin{align}
  \label{eq:ticketaccumulatorsealticketsspec}
  \ticketaccumulator \in \sequence[:\Cepochlen]{\safroleticket} \,,\quad
  \sealtickets \in \sequence[\Cepochlen]{\safroleticket}
\end{align}
...
If too few tickets were submitted in the previous epoch, it is possible for $\sealtickets'[m']$ to be the invalid ticket $\zeroticket$, defined as:
\begin{equation}
  \zeroticket \equiv \tup{\is{\st¬id}{\zerohash}, \is{\st¬entryindex}{0}}
\end{equation}

In this case, the corresponding key is pseudorandomly selected from $\activeset'$.
...
\begin{align}
  \nonumber \using i &= \sealtickets'[m']\colon \\
  \label{eq:ticketconditiontrue}
  i \ne \zeroticket &\implies \abracegroup[\,]{
  ...
  i = \zeroticket &\implies \abracegroup[\,]{
      &\H_\¬authorbskey = F(\entropy'_2, \activeset', m')\,,\\
...
The posterior slot-sealer sequence $\sealtickets'$ is one of three expressions depending on the circumstance of the block. If the block is not the first in an epoch, then it remains unchanged from the prior $\sealtickets$. If the block signals the next epoch (by epoch index) and the previous block's slot was within the closing period of the previous epoch, then it is built from the value of the prior ticket accumulator $\ticketaccumulator$. Otherwise, it is filled with invalid tickets, implying use of the fallback key in every slot in the epoch. Formally:
\begin{align}
  \label{eq:slotkeysequence}
  \sealtickets' &\equiv \begin{cases}
    Z(P(\ticketaccumulator)) &\when e' = e + 1 \wedge m \geq \Cepochtailstart \\
    \sealtickets &\when e' = e \\
    \sq{\zeroticket, \zeroticket, \dots} &\otherwise
  \end{cases}
\end{align}

Here, we use $P$ as the accumulator padding function and $Z$ as the outside-in sequencer function, defined as follows:
\begin{align}
  P&\colon\abracegroup[\,]{
    \sequence[:\Cepochlen]{\safroleticket} &\to \sequence[\Cepochlen]{\safroleticket} \\
    \mathbf{s} &\mapsto \mathbf{s} \concat \sq{\zeroticket, \zeroticket, \dots}\interval{}{\Cepochlen - \len{\mathbf{s}}}
  } \\
  Z&\colon\abracegroup[\,]{
    \sequence{\safroleticket} &\to \sequence{\safroleticket} \\
    \mathbf{s} &\mapsto \begin{cases}
        \mathbf{s} &\when \len{s} \le 1 \\
        \sq{\mathbf{s}_1} \concat Z(\mathbf{s}\interval{2}{}) \concat \sq{\mathbf{s}_0} &\otherwise
    \end{cases}
  }
\end{align}
...
The winning-tickets marker $\H_\¬winnersmark$ is either empty or, if the block is the first after the end of the submission period for tickets, then the final sequence of ticket identifiers. Formally:
\begin{align}
  \label{eq:winningticketsmarker}
  \H_\¬winnersmark &\equiv \begin{cases}
    Z(P(\ticketaccumulator)) &\when e' = e \wedge m < \Cepochtailstart \le m' \\
    \none & \otherwise
  \end{cases}
\end{align}
...
To reduce the number of distributed and submitted tickets while maintaining an adequate level of anonymity at the end of epochs, we require the first byte of the identifier of each submitted ticket to be no greater than $\Cmaxticketfirstbyte$:
\begin{equation}
  \forall x \in \mathbf{n}: x_\st¬id\subb{0} \le \Cmaxticketfirstbyte
\end{equation}
