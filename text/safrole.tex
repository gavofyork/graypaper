...
We must introduce $\mathbb{K}$, the set of validator key tuples. This is a combination of cryptographic public keys and metadata key which is an opaque octet sequence, but utilized to specify practical identifiers for the validator, not least a hardware address.
...
The header must contain a valid seal and valid entropy source. These are two signatures both using the current slot's seal key; the message data of the former is the header's serialization omitting the seal component $\mathbf{H}_s$, whereas the latter is used as a bias-resistant entropy source and thus its message must already have been fixed: we use the entropy stemming from the \textsc{vrf} output of the seal signature. Formally:
\begin{align}
...
Note that it can be shown that in the case of an empty extrinsic $\xttickets = []$, as implied by $m' \ge \mathsf{Y}$, then $\gamma'_\mathbf{a} = \gamma_\mathbf{a}$.
