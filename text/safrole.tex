...
Note that on epoch changes the posterior queued validator key set $\gamma'_k$ is defined such that incoming keys belonging to the judgement punish set $\punishset'$ are replaced with a null key containing only zeroes. The origin of this punish set is explained in section \ref{sec:disputes}.
...
\begin{align}
    \nonumber \using i = \gamma_\mathbf{s}[\mathbf{H}_t]^\circlearrowleft\colon \\
    \label{eq:ticketconditiontrue}
    \gamma_\mathbf{s} \in \seq{\mathbb{C}} &\implies \left\{\,\begin{aligned}
    ...
    \gamma_\mathbf{s} \in \seq{\H_B} &\implies \left\{\,\begin{aligned}
        &i = \mathbf{H}_a\,,\\
