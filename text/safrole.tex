...
\begin{align}
  ...
  \gamma_\mathbf{s} \in \lseq \mathbb{C} \rseq_{\mathsf{E}} \cup \seq{\hash_B}_\mathsf{E}
\end{align}
...
\begin{align}\label{eq:ticket}
  \mathbb{C} &\equiv \ltuple\isa{\mathbf{y}}{\hash}\ts\isa{r}{\N_\mathsf{N}}\rtuple
\end{align}

As we state in section \ref{sec:sealandentropy}, Safrole requires that every block header $\H$ contain a valid seal $\H_\¬sealsig$, which is a Bandersnatch signature for a public key at the appropriate index $m$ of the current epoch's seal-key series, present in state as $\gamma_\mathbf{s}$.
...
\begin{align}
  \mathbb{K} &\equiv \blob_{336} \\
  \forall k \in \mathbb{K} : k_b \in \hash_B &\equiv k_{0\dots+32} \\
  \forall k \in \mathbb{K} : k_e \in \hash_E &\equiv k_{32\dots+32} \\
  \forall k \in \mathbb{K} : k_{BLS} \in \blob_{BLS} &\equiv k_{64\dots+144} \\
  \forall k \in \mathbb{K} : k_m \in \blob_{128} &\equiv k_{208\dots+128}
\end{align}
...
\begin{align}
    \nonumber \using i = \gamma'_\mathbf{s}[\H_\¬timeslot]^\circlearrowleft\colon \\
    \label{eq:ticketconditiontrue}
    \gamma'_\mathbf{s} \in \seq{\mathbb{C}} &\implies \left\{\,\begin{aligned}
        &i_\mathbf{y} = \banderout{\H_\¬sealsig}\,,\\
        &\H_\¬sealsig \in \bandersig{\H_\¬authorbanderkey}{\mathsf{X}_T \concat \eta'_3 \doubleplus i_r}{\seunsignedheader(\H)}\,,\\
        &\mathbf{T} = 1
    \end{aligned}\right.\\
    \label{eq:ticketconditionfalse}
    \gamma'_\mathbf{s} \in \seq{\hash_B} &\implies \left\{\,\begin{aligned}
        &i = \H_\¬authorbanderkey\,,\\
        &\H_\¬sealsig \in \bandersig{\H_\¬authorbanderkey}{\mathsf{X}_F \concat \eta'_3}{\seunsignedheader(\H)}\,,\\
        &\mathbf{T} = 0
    \end{aligned}\right.\\
  \H_\¬vrfsig &\in \bandersig{\H_\¬authorbanderkey}{\mathsf{X}_E\frown \banderout{\H_\¬sealsig}}{[]} \\
...
\begin{align}
  \eta'_0 &\equiv \blake(\eta_0 \frown \banderout{\H_\¬vrfsig})
\end{align}
...
\begin{equation}\label{eq:fallbackkeysequence}
  F\colon \left\{\ \begin{aligned}
    \tuple{\hash \ts \seq{\mathbb{K}}} &\to \seq{\mathbb{H}_B}_{\mathsf{E}}\\
    \tup{r \ts \mathbf{k}} &\mapsto \left[
    \mathbf{k}[\de(\blake_4(r \frown \mathcal{E}_4(i)))]^\circlearrowleft_b
