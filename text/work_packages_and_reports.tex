...
\begin{equation}
  ...
  \workpackage \equiv \tuple{
    ...
    \isa{\wp¬authworkingsetsize}{\workingsetsize},\
...
\begin{equation}\label{eq:workitem}
  \workitem \equiv \tuple{\begin{aligned}
    ...
    \isa{\wi¬refworkingsetsize}{\workingsetsize},
...
The first term to be introduced, $\tup{\wr¬authtrace, \wr¬authgasused}$, is the authorization trace, the result of the Is-Authorized function together with the amount of gas it used. The Is-Authorized logic must be executed first in order to ensure that the work-package warrants the needed core-time.

The next term, $E$, is $\top$ if there is no valid report for the given arguments. This is the case if the Is-Authorized function fails, or if the segment-root dictionary $\wr¬srlookup$ does not contain the expected entries. $\wr¬srlookup$ is expected to have entries for all unique work-package hashes of imported segments not identified directly via a segment-root but rather through a work-package hash.
...
As long as the guarantor is unable to satisfy the above constraints, then it should consider the work-package unable to be guaranteed. Auditors are not expected to populate $\wr¬srlookup$ but rather to reuse the value in the work-report they are auditing.

The third term, $\tup{\wr¬digests, \allexports}$ is the sequence of results for each of the work-items in the work-package together with all segments exported by each work-item. The fourth definition $\countupexports$ performs an ordered accumulation (\ie counter) in order to ensure that the Refine function has access to the total number of exports made from the work-package up to the current work-item.

The above relies on two functions, $\importsegmentdata_\wr¬srlookup$ and $\extrinsicdata$ which, respectively, define the import segment data (utilising the segment-root dictionary $\wr¬srlookup$) and the extrinsic data for some work-item argument $\wiX$. We also define the segment-root lookup function $\segrootlookup_\wr¬srlookup$, which collapses a segment-root or work-package hash into a segment-root, and $\justifysegmentdata_\wr¬srlookup$, which compiles justifications of segment data:
\begin{equation}
  \begin{aligned}
...
Finally, we may define $\wr¬avspec$ as the data availability specification of the package using $\makebundle$ together with the yet to be defined \emph{Availability Specifier} function $\newavailabilityspecifier$ (see section \ref{sec:availabiltyspecifier}):
\begin{equation}
...
The functions $\fnmerklizecd$ and $\fnmerklizewb$ are the fixed-depth and simple binary Merkle root functions, defined in equations \ref{eq:constantdepthmerkleroot} and \ref{eq:simplemerkleroot}. The functions $\fnerasurecode{}{}$ and $\fnecoriginalshards$ are the erasure-coding and original-chunk-count functions, defined in appendix \ref{sec:erasurecoding}.
