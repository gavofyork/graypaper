...
The ability to communicate large amounts of data from one work-package to some subsequent work-package is a key feature of the \Jam availability system. An export segment, defined as the set $\G$, is an octet sequence of fixed length $\mathsf{W}_G = 4104$. It is the smallest datum which may individually be imported from---or exported to---the long-term \emph{Imports DA} during the Refine function of a work-package. Being an exact multiple of the erasure-coding piece size ensures that the data segments of work-package can be efficiently placed in the DA system.
...
\begin{align}
  ...
  \forall \wpX &\in \mathbb{P}: \Big(|\se(p)| + 
  \!\!\sum_{\wiX \in \wpX_\wp¬workitems}\!\!\big(|\wiX_\wi¬importsegments|\cdot\mathsf{W}_G + \!\!\!\!\!\!\sum_{(h, l) \in \wiX_\wi¬extrinsics} \!\!\!l\big)\Big) \le \mathsf{W}_B \\
...
\begin{equation}
  \itemtoresult\colon\left\{\begin{aligned}
    ...
    ((s, c, \mathbf{y}, a), o) &\mapsto (s, c, \mathcal{H}(\mathbf{y}), \is{g}{a}, o)
  \end{aligned}\right.
\end{equation}
