...
\begin{equation}
  \itemtoresult\colon\left\{\begin{aligned}
    ...
    \end{aligned}
    }, \wl¬data, u} &\mapsto \tup{\begin{aligned}
      s, c, \is{\wl¬payloadhash}{\mathcal{H}(\mathbf{y})}, \is{g}{a}, \wl¬data, u,\\
...
\begin{equation}\label{eq:workresultfunction}
  \Xi \colon \left\{\begin{aligned}
    ...
    (\wpX, c) &\mapsto \begin{cases}
        \error &\when \mathbf{o} \not\in \Y \\
        \tup{s, \is{x}{\wpX_\wp¬context}, c, \is{a}{\wpX_\wp¬authorizer}, \mathbf{o}, \mathbf{l}, \mathbf{r}, g} &\otherwise
    \end{cases}
  \end{aligned}\right.
\end{equation}
...
\begin{align*}
  ...
  \countupexports(\wpX, j) \equiv \,&\begin{cases}
    ...
    \multicolumn{2}{l}{\where (r, \mathbf{e}, u) = \Psi_R(
      j, \wpX, \mathbf{o}, \importsegmentdata(w, \mathbf{l}), \ell
    )}\\
