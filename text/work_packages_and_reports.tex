...
\begin{equation}
  \begin{aligned}
    ...
    \where \mathbf{x} &= [\extrinsicdata(w) \mid w \orderedin \mathbf{p}_\wp¬workitems]\\
    \also \mathbf{i} &= [\importsegmentdata(w) \mid w \orderedin \mathbf{p}_\wp¬workitems]\\
    \also \mathbf{j} &= [\var{\mathcal{J}(\mathbf{s}, n)} \mid (\mathcal{M}(\mathbf{s}), n) \orderedin w_\wi¬importsegments, w \orderedin \mathbf{p}_\wp¬workitems]
  \end{aligned}
\end{equation}
