...
\begin{align}
  ...
  &\begin{aligned}
    &\forall \wpX \in \workpackage: \Big(|\wpX_\wp¬authtoken| + |\wpX_\wp¬authconfig| +
    \!\!\sum_{\wiX \in \wpX_\wp¬workitems}\!\!S(\wiX)\Big) \le \mathsf{W}_B \\
    &\where S(\wiX \in \workitem) \equiv |\wiX_\wi¬payload| + |\wiX_\wi¬importsegments|\cdot\mathsf{W}_G + \!\!\!\!\!\!\sum_{(h, l) \in \wiX_\wi¬extrinsics} \!\!\!l
  \end{aligned}\\
...
\begin{equation}
  \itemtodigest\colon\left\{\begin{aligned}
    (\workitem, \blob \cup \mathbb{J}, \N_G) &\to \mathbb{L}\\
    \tup{\tup{\begin{aligned}
...
\begin{equation}\label{eq:workdigestfunction}
  \Xi \colon \left\{\begin{aligned}
    (\workpackage, \N_\mathsf{C}) &\to \workreport \\
...
\begin{align*}
  ...
  \countupexports(\wpX, j) \equiv \,&\begin{cases}
    ...
    \multicolumn{2}{l}{\also z = |\mathbf{o}| + \sum_{k < j, (r \in \blob, \dots) = I(p, k)} |r|}
  \end{cases}
\end{align*}
...
\begin{equation}
  \forall (h \mapsto e) \in \wr¬srlookup : \exists \wpX, \¬core \in \workpackage, \N_\mathsf{C} : \blake(\wpX) = h \wedge (\Xi(\wpX, \¬core)_s)_e = e\!\!\!\!
\end{equation}
...
\begin{equation}
  \begin{aligned}
    \extrinsicdata(\wiX \in \workitem) &\equiv [\mathbf{d} \mid (\blake(\mathbf{d}), |\mathbf{d}|) \orderedin \wiX_\wi¬extrinsics] \\
