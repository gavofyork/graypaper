...
%TODO: use r instead of g for refinement gas limit.
...
We limit both the total number of exported items and the total number of imported items to $\mathsf{W}_M = 3072$, and the total number of extrinsics to $\mathsf{T} = 128$:
\begin{equation}
  \!\!\!\!\begin{aligned}
...
Given the resultant datum $\wl¬result$ of some work-item, we define the item-to-result function $\itemtoresult$ as:
\begin{equation}
  \itemtoresult\colon\left\{\begin{aligned}
    ...
    \end{aligned}
    }, \wl¬result, u} &\mapsto \tup{\begin{aligned}
      s, c, \is{\wl¬payloadhash}{\mathcal{H}(\mathbf{y})}, \is{g}{a}, \wl¬result, u,\\
      \is{i}{|\mathbf{i}|}, e, \is{x}{|\mathbf{x}|}, \is{z}{\sum_{(h, z) \in \mathbf{x}} z}
    \end{aligned}
    }
  \end{aligned}\right.
\end{equation}
...
\begin{align*}
  ...
  (\wr¬digests, \allexports) = \,&\transpose[
    (\itemtoresult(\wpX_\wp¬workitems[j], r, u), \mathbf{e})
    \mid
...
\begin{equation}
  \forall (h \mapsto e) \in \wr¬srlookup : \exists \wpX, \¬core \in \mathbb{P}, \N_\mathsf{C} : \mathcal{H}(\wpX) = h \wedge (\Xi(\wpX, \¬core)_s)_e = e\!\!\!\!
\end{equation}
