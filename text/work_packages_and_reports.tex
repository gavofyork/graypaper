...
\begin{equation}
  \itemtodigest\colon\left\{\begin{aligned}
    (\workitem, \Y \cup \mathbb{J}, \N_G) &\to \mathbb{L}\\
    \tup{\tup{\begin{aligned}
...
\begin{equation}
  \forall (h \mapsto e) \in \wr¬srlookup : \exists \wpX, \¬core \in \workpackage, \N_\mathsf{C} : \hash(\wpX) = h \wedge (\Xi(\wpX, \¬core)_s)_e = e\!\!\!\!
\end{equation}
...
\begin{equation}
  \begin{aligned}
    \extrinsicdata(\wiX \in \workitem) &\equiv [\mathbf{d} \mid (\hash(\mathbf{d}), |\mathbf{d}|) \orderedin \wiX_\wi¬extrinsics] \\
