...
Service accounts are held in state under $\accounts$, a partial mapping from a service identifier $\N_S$ into a tuple of named elements which specify the attributes of the service relevant to the \Jam protocol. Formally:
\begin{align}\label{eq:serviceaccounts}
  \N_S &\equiv \N_{2^{32}} \\
  \accounts &\in \dict{\N_S}{\serviceaccount}
\end{align}

The service account is defined as the tuple of storage dictionary $\mathbf{s}$, preimage lookup dictionaries $\sa¬preimages$ and $\sa¬requests$, code hash $\sa¬codehash$, balance $\sa¬balance$ and gratis storage offset $\sa¬gratis$, as well as the two code gas limits $\sa¬minaccgas$ \& $\sa¬minmemogas$. We also record certain usage characteristics concerning the account: the time slot at creation $\sa¬created$, the time slot at the most recent accumulation $\sa¬lastacc$ and the parent service $\sa¬parent$. Formally:
\begin{align}\label{eq:serviceaccount}
  \serviceaccount \equiv \tuple{\ \begin{aligned}
    ...
    \sa¬requests &\in \dict{\tuple{\hash,\N_L}}{\seq{\timeslot}_{:3}}\,,\\
...
Thus, the balance of the service of index $s$ would be denoted $\accounts\subb{s}_\sa¬balance$ and the storage item of key $\mathbf{k} \in \blob$ for that service is written $\accounts\subb{s}_\sa¬storage[\mathbf{k}]$.
...
\begin{align}
  \forall \mathbf{a} \in \serviceaccount : \tup{\mathbf{a}_\mathbf{m}, \mathbf{a}_\mathbf{c}} \equiv \begin{cases}
...
Restated, we must be able to define some \emph{historical} lookup function $\histlookup$ which determines whether the preimage of some hash $h$ was available for lookup by some service account $\mathbf{a}$ at some timeslot $t$, and if so, provide its preimage:
\begin{equation}
\begin{aligned}
  \histlookup\colon \left\{\ \begin{aligned}
    (\serviceaccount, \N_{\H_\¬timeslot - \mathsf{D} \dots \H_\¬timeslot}, \hash) &\to \optional{\blob} \\
    (\mathbf{a}, t, \blake(\mathbf{p})) &\mapsto v : v \in \{ \mathbf{p}, \none \}
  \end{aligned}\right.
\end{aligned}
\end{equation}
...
The historical status component $h \in \seq{\timeslot}_{:3}$ is a sequence of up to three time slots and the cardinality of this sequence implies one of four modes:
\begin{itemize}
...
\begin{equation}
  \begin{aligned}\label{eq:historicallookup}
    ...
    &\histlookup(\mathbf{a}, t, h) \equiv \begin{cases}
      \mathbf{a}_\sa¬preimages[h]\!\!\!\! &\when h \in \keys{\mathbf{a}_\sa¬preimages} \wedge I(\mathbf{a}_\sa¬requests\subb{h, \len{\mathbf{a}_\sa¬preimages[h]}}, t) \!\!\!\!\! \\
      \none &\otherwise
    \end{cases}\\
...
\begin{align}
  \forall \mathbf{a} \in \mathcal{V}(\accounts):\left\{\begin{aligned}
    \mathbf{a}_\sa¬items \in \N_{2^{32}} &\equiv 2\cdot\len{\,\mathbf{a}_\sa¬requests\,} + \len{\,\mathbf{a}_\sa¬storage\,} \\
    \mathbf{a}_\sa¬octets \in \N_{2^{64}} &\equiv \sum\limits_{\,(h, z) \in \keys{\mathbf{a}_\sa¬requests}\,} \!\!\!\!81 + z \\
    &\phantom{\equiv\ } + \sum\limits_{(x, y) \in \mathbf{a}_\sa¬storage} 34 + \len{y} + \len{x} \\
    \mathbf{a}_\sa¬minbalance \in \balance &\equiv \max(0, \mathsf{B}_S + \mathsf{B}_I \cdot \mathbf{a}_\sa¬items + \mathsf{B}_L \cdot \mathbf{a}_\sa¬octets - \mathbf{a}_\sa¬gratis)
  \end{aligned}\right.
\end{align}
...
\begin{align}
  \privileges \equiv \tuple{
    \isa{\manager}{\N_S} ,
    \isa{\assigners}{\seq{\N_S}_\corecount} ,
    \isa{\delegator}{\N_S} ,
    \isa{\alwaysaccers}{\dict{\N_S}{\gas}}
  }
\end{align}
