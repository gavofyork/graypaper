...
\begin{align}\label{eq:serviceaccounts}
  \N_S &\equiv \N_{2^{32}} \\
  \delta &\in \dict{\N_S}{\serviceaccount}
\end{align}
...
\begin{align}\label{eq:serviceaccount}
  \serviceaccount \equiv \tuple{\ \begin{aligned}
    \sa¬storage &\in \dict{\Y}{\Y}\,,\ 
    \sa¬preimages \in \dict{\H}{\Y}\,,\\
...
Thus, the balance of the service of index $s$ would be denoted $\delta[s]_\sa¬balance$ and the storage item of key $\mathbf{k} \in \Y$ for that service is written $\delta[s]_\sa¬storage[\mathbf{k}]$.
...
\begin{aligned}
  \Lambda\colon \left\{\ \begin{aligned}
    (\serviceaccount, \N_{\mathbf{H}_t - \mathsf{D}\dots\mathbf{H}_t}, \mathbb{H}) &\to \mathbb{Y}\bm{?} \\
...
The preimage lookup for some service of index $s$ is denoted $\delta[s]_\sa¬preimages$ is a dictionary mapping a hash to its corresponding preimage. Additionally, there is metadata associated with the lookup denoted $\delta[s]_\sa¬requests$ which is a dictionary mapping some hash and presupposed length into historical information.
...
\begin{equation}\label{eq:preimageconstraints}
  \forall \mathbf{a} \in \serviceaccount, (h \mapsto \mathbf{d}) \in \mathbf{a}_\sa¬preimages \Rightarrow
    h = \hash(\mathbf{d})\wedge
    \tup{h \ts |\mathbf{d}|} \in \keys{\mathbf{a}_\sa¬requests}
\end{equation}
...
\begin{equation}
  \begin{aligned}\label{eq:historicallookup}
    &\Lambda\colon (\serviceaccount, \N_T, \H) \to \Y\bm{?} \\
    &\Lambda(\mathbf{a}, t, h) \equiv \begin{cases}
      \mathbf{a}_\sa¬preimages[h]\!\!\!\! &\when h \in \keys{\mathbf{a}_\sa¬preimages} \wedge I(\mathbf{a}_\sa¬requests[h, |\mathbf{a}_\sa¬preimages[h]|], t) \!\!\!\!\! \\
      \none &\otherwise
    \end{cases}\\
...
\begin{align}
  \forall \mathbf{a} \in \mathcal{V}(\delta):\left\{\begin{aligned}
    \mathbf{a}_\sa¬items \in \N_{2^{32}} &\equiv 2\cdot|\,\mathbf{a}_\sa¬requests\,| + |\,\mathbf{a}_\sa¬storage\,| \\
    \mathbf{a}_\sa¬octets \in \N_{2^{64}} &\equiv \sum\limits_{\,(h, z) \in \keys{\mathbf{a}_\sa¬requests}\,} \!\!\!\!81 + z \\
    &\phantom{\equiv\ } + \sum\limits_{(x, y) \in \mathbf{a}_\sa¬storage} 34 + |y| + |x| \\
