...
The service account is defined as the tuple of storage dictionary $\sa¬storage$, preimage lookup dictionaries $\sa¬preimages$ and $\sa¬requests$, code hash $\sa¬codehash$, balance $\sa¬balance$ and gratis storage offset $\sa¬gratis$, authority service $\sa¬authority$ and its protected balance $\sa¬authbalance$, as well as the two code gas limits $\sa¬minaccgas$ \& $\sa¬minmemogas$. We also record certain usage characteristics concerning the account: the time slot at creation $\sa¬created$, the time slot at the most recent accumulation $\sa¬lastacc$ and the parent service $\sa¬parent$. Formally:
\begin{align}\label{eq:serviceaccount}
  \serviceaccount \equiv \tuple{\ \begin{aligned}
    ...
    \sa¬authority \in \tuple{\serviceid, \timeslot}\,,\
    \sa¬authbalance \in \balance\,,\\
    \sa¬minaccgas &\in \gas\,,\
...
\subsection{Service Authorities}
Each service has a designated \emph{authority} service $\sa¬authority$; all non-privileged mutating operations are permitted to be done on a service by its authority service. Only the authority service may designate a new authority service. A service's authority service may be itself.

In addition to the regular balance of a service $\sa¬balance$, there is an additional authority balance $\sa¬authbalance$. This balance is aggregated with the regular balance when determining data footprint limits for the service, but may only be transferred by the authority service.

A service's authority service designation $\sa¬authority$ is a tuple of the authority service's identifier and the time slot in which the authority service was created. The creation time slot is included to prevent unintentional transfer of authority in the case that the authority service is ejected and a new service is created with the same identifier. Following ejection of a service's authority service, the authority service designation effectively reverts to the service itself. This behaviour is encoded by the function $\fneffectiveauthority$, which returns a service's effective authority service.
\begin{equation}\label{eq:effectiveauthority}
  \fneffectiveauthority\colon\abracegroup{
    \tuple{\dictionary{\serviceid}{\serviceaccount}, \serviceid} &\to \serviceid \\
    \tup{\mathbf{d}, s} &\mapsto \begin{cases}
      u &\when u \in \mathbf{d} \land \mathbf{d}\subb{u}_\sa¬created = r \\
      s &\otherwise
    \end{cases} \\
    &\phantom{{}\mapsto{}} \where \tup{u, r} = \mathbf{d}\subb{s}_\sa¬authority
  }
\end{equation}
