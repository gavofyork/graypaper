...
The service account is defined as the tuple of storage dictionary $\sa¬storage$, preimage lookup dictionaries $\sa¬preimages$ and $\sa¬requests$, code hash $\sa¬codehash$, balance $\sa¬balance$ and gratis storage offset $\sa¬gratis$, supervisor $\sa¬supervisor$ and its protected balance $\sa¬supervisorbalance$, as well as the two code gas limits $\sa¬minaccgas$ \& $\sa¬minmemogas$. We also record certain usage characteristics concerning the account: the time slot at creation $\sa¬created$, the time slot at the most recent accumulation $\sa¬lastacc$ and the parent service $\sa¬parent$. Formally:
\begin{align}\label{eq:serviceaccount}
  \serviceaccount \equiv \tuple{\ \begin{aligned}
    ...
    \sa¬supervisor \in \tuple{\serviceid, \timeslot}\,,\
...
Each service has a designated \emph{supervisor} $\sa¬supervisor$; all non-privileged mutating operations are permitted to be done on a service by its supervisor. Only the supervisor may designate a new supervisor. A service's supervisor may be itself.
...
A service's supervisor designation $\sa¬supervisor$ is a tuple of the supervisor's identifier and the time slot in which the supervisor was created. The creation time slot is included to prevent unintentional transfer of authority in the case that the supervisor is ejected and a new service is created with the same identifier. We define the \emph{effective supervisor} of a service to be the designated supervisor if this service exists and has the expected creation slot, or the service itself otherwise. For convenience, given a service accounts dictionary $\mathbf{d}$ and a service identifier $s$, we express the effective supervisor of $s$ as $\effectivesupervisor{\mathbf{d}}{s}$. Formally:
\begin{equation}\label{eq:effectivesupervisor}
  \begin{aligned}
    \forall \mathbf{d} \in \dictionary{\serviceid}{\serviceaccount}, s \in \serviceid : \effectivesupervisor{\mathbf{d}}{s} &\equiv \begin{cases}
      ...
      s &\otherwise
    \end{cases} \\
    &\phantom{{}\equiv{}} \where \tup{u, r} = \mathbf{d}\subb{s}_\sa¬supervisor
  \end{aligned}
\end{equation}
