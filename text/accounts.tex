...
\begin{align}\label{eq:serviceaccount}
  \serviceaccount \equiv \tuple{\ \begin{aligned}
    ...
    \sa¬preimages \in \dict{\H}{\blob}\,,\\
    \sa¬requests &\in \dict{\tuple{\H,\N_L}}{\seq{\N_T}_{:3}}\,,\\
    \sa¬gratis &\in \N_B\,,\ 
    \sa¬codehash \in \H\,,\ 
    \sa¬balance \in \N_B\,,\ 
    \sa¬minaccgas \in \N_G\,,\\
    \sa¬minmemogas &\in \N_G\,,\ 
    \sa¬created \in \N_T\,,\ 
    \sa¬lastacc \in \N_T\,,\ 
    \sa¬parent \in \N_S\\
    %i, o, f
...
\begin{aligned}
  \histlookup\colon \left\{\ \begin{aligned}
    (\serviceaccount, \N_{\h¬timeslot - \mathsf{D} \dots \h¬timeslot}, \H) &\to \blob\bm{?} \\
...
\begin{equation}
  \begin{aligned}\label{eq:historicallookup}
    &\histlookup\colon (\serviceaccount, \N_T, \H) \to \blob\bm{?} \\
...
\begin{align}
  \forall \mathbf{a} \in \mathcal{V}(\accounts):\left\{\begin{aligned}
    ...
    \mathbf{a}_\sa¬minbalance \in \N_B &\equiv \max(0, \mathsf{B}_S + \mathsf{B}_I \cdot \mathbf{a}_\sa¬items + \mathsf{B}_L \cdot \mathbf{a}_\sa¬octets - \mathbf{a}_\sa¬gratis)
  \end{aligned}\right.
\end{align}
