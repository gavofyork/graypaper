...
\subsection{State}
The state of the reporting and availability portion of the protocol is largely contained within $\availassignments$, which tracks the work-report guarantees which have been reported but are not yet known to be available to a super-majority of validators, together with the time at which each was reported. As mentioned earlier, only one guarantee may be assigned to a core at any given time. Formally:
\begin{equation}
...
\begin{align}
  ...
  \forall \cX \in \coreindex: \availassignmentspostassurances\subb{\cX} \equiv \begin{cases}
    \none &\when(\availassignmentspostjudgment\subb{\cX}_\aa¬guarantee)_\g¬workreport \in \justbecameavailable \vee \H_\¬timeslot \ge \availassignmentspostjudgment\subb{\cX}_\aa¬timestamp + \Cassurancetimeoutperiod\\
    \availassignmentspostjudgment\subb{\cX} &\otherwise
  \end{cases}
\end{align}
...
\begin{equation}
  ...
  \guarantee \equiv \tuple{
    ...
    \isa{\g¬credential}{\sequence[2:3]{\tuple{\valindex, \edsignaturebase}}}
  }
\end{equation}
  ...
  &\begin{aligned}
    ...
    \end{aligned} :
      \abracegroup[\,]{
        ...
        &\mathbf{c}\sub{v} = \g¬workreport_\wr¬core \wedge \Crotationperiod(\floor{\nicefrac{\thetime'}{\Crotationperiod}} - 1) \le \g¬timeslot \le \thetime'\\
      }\\
...
No guarantees may be placed on cores with a guarantee pending availability on it. A guarantee is valid only if the report's authorizer hash is present in the authorizer pool of the core on which the work is reported. Formally:
