...
\begin{equation}
  \forall w \in \workreport : |w_\wr¬srlookup| + |(w_x)_\mathbf{p}| \le \mathsf{J}
\end{equation}
...
\begin{align}
  &\forall w \in \workreport:
    |w_\wr¬authtrace| + \sum_{r \in w_\wr¬digests \cap \blob} |r_\wl¬result| \le \mathsf{W}_R \\
...
\begin{align}
  \xtassurances \in \lseq\ltuple\isa{a}{\hash}\ts\isa{f}{\mathbb{B}_\mathsf{C}}\ts\isa{v}{\valindex}\ts\isa{s}{\mathbb{E}}\rtuple\rseq_\mathsf{:V}
\end{align}
...
\begin{align}
  &\forall a \in \xtassurances : a_s \in \sig{\kappa[a_v]_e}{\mathsf{X}_A\frown \blake(\se(\H_\¬parent, a_f))} \\
...
\begin{equation}
  \mathbf{G} \in (\seq{\coreindex}_{\valindex}, \seq{\mathbb{H}_K}_{\valindex})
\end{equation}
...
\begin{align}
  ...
  P(e, t) &\equiv R\left(\mathcal{F}\left(\left[\ffrac{\mathsf{C}\cdot i}{\mathsf{V}} \,\middle\mid\, i \orderedin \valindex\right], e\right), \ffrac{t \bmod \mathsf{E}}{\mathsf{R}}\right)\\
...
\begin{align}\label{eq:guarantorsig}
  &\begin{aligned}
    ...
    \end{aligned}
      : \left\{\,\begin{aligned}
        &s \in \sig{(\mathbf{k}\sub{v})_\vk¬ed}{\mathsf{X}_G\frown\blake(w)}\\
...
\begin{align}
  ...
  &\forall p \in \mathbf{p}, p \not\in \bigcup_{x \in \recenthistory}\keys{x_\mathbf{p}} \cup \bigcup_{x \in \accumulated}x \cup \mathbf{q} \cup \mathbf{a}
\end{align}
...
\begin{align}
  &\begin{aligned}
    ...
    &\quad p \in \mathbf{p} \cup \{ x \mid x \in \keys{b_\mathbf{p}} ,\, b \in \recenthistory \}
  \end{aligned}
\end{align}
