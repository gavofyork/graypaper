...
\begin{equation}\label{eq:reportingstate}
  \rho \in \seq{\tuple{\isa{w}{\workreport},\isa{t}{\N_T}}\bm{?}}_\mathsf{C}
\end{equation}
...
\begin{align}
  \avspec &\equiv \tuple{
    \isa{\as¬packagehash}{\H}\,,\;
    \isa{\as¬bundlelen}{\N_L}\,,\;
    \isa{\as¬erasureroot}{\H}\,,\;
    \isa{\as¬segroot}{\H}\,,\;
    \isa{\as¬segcount}{\N}
  }
\end{align}
...
\begin{align}
  &\forall a \in \xtassurances : a_s \in \sig{\kappa[a_v]_e}{\mathsf{X}_A\frown \blake(\se(\mathbf{H}_p, a_f))} \\
...
\begin{align}
  \forall c \in \N_\mathsf{C}: \rho^\ddagger[c] \equiv \begin{cases}
    \none &\when\rho[c]_w \in \mathbf{W} \vee \h¬timeslot \ge \rho^\dagger[c]_t + \mathsf{U}\\
...
\begin{align}\label{eq:guaranteesextrinsic}
  \xtguarantees &\in \seq{\tuple{
    ...
    \isa{\xg¬timeslot}{\N_T},\,
    \isa{\xg¬credential}{\seq{\tuple{\N_V, \mathbb{E}}}_{2:3}}
  }}_{:\mathsf{C}} \\
...
\begin{align}
  \forall x \in \mathbf{x} :\ x_t \ge \h¬timeslot - \mathsf{L}
\end{align}
