...
Every block, each core has three validators uniquely assigned to guarantee work reports for it. This is borne out with $\mathsf{V} = 1,023$ validators and $\mathsf{C} = 341$ cores, since $\nicefrac{\mathsf{V}}{\mathsf{C}} = 3$. The core index assigned to each of the validators, as well as the validators' Ed25519 keys are denoted by $\mathbf{G}_C$:
\begin{equation}
  \mathbf{G} \in \seq{(\N_\mathsf{C}, \mathbb{H}_K)}_{\N_V}
\end{equation}
...
\begin{align}
  \forall g &\in \mathbf{E}_G : g_a = \orderuniqby{v}{\tup{v, s} \in g_a}
\end{align}
  ...
  &\begin{aligned}
    ...
    \end{aligned}
      : \left\{\,\begin{aligned}
        ...
        &\mathbf{c}_v = w_c\\
      \end{aligned}\right.\\
      &k \in \mathbf{R} \Leftrightarrow \exists (w, t, a) \in \mathbf{E}_G, \exists (v, s) \in a: k = (\mathbf{k}_v)_E\\
