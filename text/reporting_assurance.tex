...
\begin{equation}
  \forall \wrX \in \workreport : \len{\wrX_\wr¬srlookup} + \len{(\wrX_\wr¬context)_\cx¬prerequisites} \le \mathsf{J}
\end{equation}
...
\begin{align}
  \xtassurances \in \seq{\tuple{
    \isa{\xa¬anchor}{\hash},\,
    \isa{\xa¬availabilities}{\bitstringoflen{\corecount}},\,
...
\begin{align}
  &\begin{aligned}
    &\forall \wrX \in \incomingreports,
    \forall p \in (\wrX_\wr¬context)_\cx¬prerequisites \cup
      \Fkeys{\wrX_\wr¬srlookup} :\\
    &\quad p \in \incomingpackagehashes \cup \set{
      \build{x}{x \in \Fkeys{b_\rh¬reportedpackagehashes},\, b \in \recenthistory}}
  \end{aligned}
\end{align}
