...
\begin{align}
  ...
  \label{eq:isauthorizedmutator}F&\colon\left\{\begin{aligned}
    (\Y_\$, \N_G, \lseq\N_R\rseq_6, \mathbb{M}) &\to (\Z_G, \lseq\N_R\rseq_2, \mathbb{M})\\
    (\mathbf{n}, \xi, \omega, \mu) &\mapsto \begin{cases}
      \Omega_G(\xi, \omega, \mu) &\when\mathbf{n} = \mathtt{\$gas} \\
...
We define the Refine service-account invocation function as $\Psi_R$. It is stateless, with the only exceptions being the ability to make a historical lookup and the ability to create inner instances of the \textsc{pvm}.

The historical-lookup host-call function, $\Omega_H$, is designed to give the same result regardless of the state of the chain for any time when auditing may occur (which we bound to be less than two epochs from being accumulated). However, the lookup anchor may be up to $\mathsf{L}$ timeslots before the recent history and therefore adds to the potential age at the time of audit. We therefore set $\mathsf{D} = 28,800$, a safe amount of 48 hours.
...
\begin{align}
  \Psi_R &\colon \left\{\begin{aligned}
    % TODO: Check this
    (\H, \N_G, \N_S, \H, \Y, \mathbb{X}, \H, \Y) &\to \Y \cup \mathbb{J} \\
    (c, g, s, p, \mathbf{y}, \mathbf{x}, a, \mathbf{o}) &\mapsto \begin{cases}
      \token{BAD} &\when s \not\in \keys{\delta} \vee H(\delta[s], \mathbf{x}_t, c) = \none \\
      \token{BIG} &\otherwhen |H(\delta[s], \mathbf{x}_t, c)| > \mathsf{S} \\
      &\otherwise\ \using (\mathbf{r}, \none) = \Psi_M(H(\delta[s], \mathbf{x}_t, c), 1, g, \mathcal{E}(s, p, \mathbf{y}, \mathbf{x}, a, \mathbf{o}), F, \emptyset)\colon\\
      \mathbf{r} &\quad\when \mathbf{r} \in \{ \oog, \panic \}  \\
      \mathbf{u} &\quad\when \mathbf{r} = \tup{g, \mathbf{u}} \\
    \end{cases} \\
  \end{aligned}\right. \\
  \label{eq:refinemutator}
  F&\colon\left\{\begin{aligned}
    (\Y_\$, \N_G, \lseq\N_R\rseq_6, \mathbb{M}, \dict{\N}{\pvm}) &\to (\N_G, \lseq\N_R\rseq_2, \mathbb{M}, \dict{\N}{\pvm})\\
    (\mathbf{n}, \xi, \omega, \mu, \mathbf{m}) &\mapsto \begin{cases}
      \Omega_H(\xi, \omega, \mu, \mathbf{m}, s, \delta, \mathbf{x}_t) &\when\mathbf{n} = \mathtt{\$lookup}\\
      \Omega_G(\xi, \omega, \mu, \mathbf{m}) &\when\mathbf{n} = \mathtt{\$gas} \\
      \Omega_M(\xi, \omega, \mu, \mathbf{m}) &\when\mathbf{n} = \mathtt{\$machine}\\
      \Omega_P(\xi, \omega, \mu, \mathbf{m}) &\when\mathbf{n} = \mathtt{\$peek}\\
      \Omega_O(\xi, \omega, \mu, \mathbf{m}) &\when\mathbf{n} = \mathtt{\$poke}\\
      \Omega_K(\xi, \omega, \mu, \mathbf{m}) &\when\mathbf{n} = \mathtt{\$invoke}\\
      \Omega_X(\xi, \omega, \mu, \mathbf{m}) &\when\mathbf{n} = \mathtt{\$expunge}\\
      (\xi - 10, [\mathtt{WHAT}, \omega_1, \dots], \mu) &\otherwise
    \end{cases}
  \end{aligned}
  \right.
...
Note for the Refine host-call dispatcher $F$ in equation \ref{eq:refinemutator}, we elide the host-call context since, being essentially stateless, it is always $\none$.
...
\begin{equation}
  \mathbf{X} \equiv \ltuple\begin{array}{lll}
    ...
    \isa{\mathbf{c}}{[\left[\H\right]_\mathsf{Q}]_\mathsf{C}}\ts&
    \isa{\mathbf{v}}{\mathbb{K}_\mathsf{V}}\ts\quad
    \isa{i}{\N_S}\ts\\
    \isa{\mathbf{t}}{\left[\mathbb{T}\right]}\ts&
...
\begin{align}
  ...
  \end{aligned} \right. \\
  F(\mathbf{n}, \xi, \omega, \mu, (\mathbf{x}, \mathbf{y})) &\equiv \begin{cases}
    G(\Omega_R(\xi, \omega, \mu, \mathbf{x}_\mathbf{s}, s, \delta^\dagger), (\mathbf{x}, \mathbf{y})) &\when\mathbf{n} = \mathtt{\$read} \\
    G(\Omega_W(\xi, \omega, \mu, \mathbf{x}_\mathbf{s}), (\mathbf{x}, \mathbf{y})) &\when\mathbf{n} = \mathtt{\$write} \\
    G(\Omega_L(\xi, \omega, \mu, s, \delta^\dagger), (\mathbf{x}, \mathbf{y})) &\when\mathbf{n} = \mathtt{\$lookup} \\
    G(\Omega_G(\xi, \omega, \mu), (\mathbf{x}, \mathbf{y})) &\when\mathbf{n} = \mathtt{\$gas} \\
    G(\Omega_I(\xi, \omega, \mu, \mathbf{x}_\mathbf{s}, s, \delta^\dagger), (\mathbf{x}, \mathbf{y})) &\when\mathbf{n} = \mathtt{\$info} \\
    \Omega_E(\xi, \omega, \mu, (\mathbf{x}, \mathbf{y})) &\when\mathbf{n} = \mathtt{\$empower}\\
    \Omega_A(\xi, \omega, \mu, (\mathbf{x}, \mathbf{y})) &\when\mathbf{n} = \mathtt{\$assign}\\
    \Omega_D(\xi, \omega, \mu, (\mathbf{x}, \mathbf{y})) &\when\mathbf{n} = \mathtt{\$designate}\\
    \Omega_C(\xi, \omega, \mu, (\mathbf{x}, \mathbf{y})) &\when\mathbf{n} = \mathtt{\$checkpoint} \\
    \Omega_N(\xi, \omega, \mu, (\mathbf{x}, \mathbf{y})) &\when\mathbf{n} = \mathtt{\$new} \\
    \Omega_U(\xi, \omega, \mu, (\mathbf{x}, \mathbf{y}), s) &\when\mathbf{n} = \mathtt{\$upgrade} \\
    \Omega_T(\xi, \omega, \mu, (\mathbf{x}, \mathbf{y}), s, \delta^\dagger) &\when\mathbf{n} = \mathtt{\$transfer} \\
    \Omega_Q(\xi, \omega, \mu, (\mathbf{x}, \mathbf{y}), s) &\when\mathbf{n} = \mathtt{\$quit} \\
    \Omega_S(\xi, \omega, \mu, (\mathbf{x}, \mathbf{y}), \mathbf{H}_t) &\when\mathbf{n} = \mathtt{\$solicit} \\
    \Omega_F(\xi, \omega, \mu, (\mathbf{x}, \mathbf{y}), \mathbf{H}_t) &\when\mathbf{n} = \mathtt{\$forget} \\
  ...
  C((\mathbf{x}\in\mathbf{X}, \mathbf{y}\in\mathbf{X}), \mathbf{o} \in \mathbb{Y} \cup \{\oog, \panic\}) &\equiv \begin{cases}
...
\begin{align}
  \Psi_T &: \begin{cases}
    (\dict{\N_S}{\mathbb{A}}, \N_S, [\mathbb{T}]) &\to \mathbb{A} \\
  ...
  F(\mathbf{n}, \xi, \omega, \mu, \mathbf{s}) &\equiv \begin{cases}
    \Omega_L(\xi, \omega, \mu, \mathbf{s}, s, \delta^\ddagger) &\when\mathbf{n} = \mathtt{\$lookup} \\
    \Omega_R(\xi, \omega, \mu, \mathbf{s}, s, \delta^\ddagger) &\when\mathbf{n} = \mathtt{\$read} \\
    \Omega_W(\xi, \omega, \mu, \mathbf{s}) &\when\mathbf{n} = \mathtt{\$write} \\
    \Omega_G(\xi, \omega, \mu) &\when\mathbf{n} = \mathtt{\$gas} \\
    \Omega_I(\xi, \omega, \mu, \mathbf{s}, s, \delta^\ddagger) &\when\mathbf{n} = \mathtt{\$info} \\
...
This defines a number of functions broadly of the form $(\xi' \in \Z_G, \omega' \in [\N_R]_2, \mu', \mathbf{s}') = \Omega_\square(\xi \in \N_G, \omega \in [\N_R]_6, \mu \in \mathbb{M}, \mathbf{s} \in \mathbf{A}, \dots)$. Functions which have a result component which is equivalent to the corresponding argument may have said components elided in the description. Functions may also depend upon particular additional parameters.
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_V(\xi, \omega, \mu, (\mathbf{x}, \mathbf{y}))$ \\
  \texttt{invoke} \\
  $g = 10$} &
  $\begin{aligned}
    \using [o, z] &= \omega_{0, 1} \\
...
\subsection{Refine Function}\label{sec:refinefunctions}

These assume some refine context $\mathbf{M} \in \dict{\N_R}{\tuple{\Y, \mathbb{M}, \seq{\N_R}_{13}, \N_R}}$, initially empty and which tends to be stated as $\mathbf{m}$ (posterior $\mathbf{m}'$).
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  \endhead
  \makecell*[l]{
  $\Omega_H(\xi, \omega, \mu, \mathbf{m}, s, \delta, t)$ \\
  \texttt{historical\_lookup} \\
  $g = 10$} &
  $\begin{aligned}
    \using \mathbf{a} &= \begin{cases}
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_M(\xi, \omega, \mu, \mathbf{m})$ \\
  \texttt{machine} \\
  $g = 10$} &
  $\begin{aligned}
    \using [p_o, p_z, i] &= \omega_{0 \dots 2} \\
