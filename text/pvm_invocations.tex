...
\begin{align}
  &\Psi_R \colon \left\{\begin{aligned}
    (\N_G, \N, \mathbb{P}, \Y, \seq{\seq{\G}}, \N) &\to (\Y \cup \mathbb{J}, \seq{\Y}) \\
    (g, i, p, \mathbf{o}, \overline{\mathbf{i}}, \segoff) &\mapsto \begin{cases}
      ...
      &\quad\also (g, \mathbf{r}, (\mathbf{m}, \mathbf{e})) = \Psi_M(\Lambda(\delta[w_s], (p_\mathbf{x})_t, w_c), 0, g, a, F, (\emptyset, []))\ \colon\\
...
\begin{longtable}{p{4cm} p{12cm}}
  ...
  $\begin{aligned}
    \using \mathbf{v} &= \begin{cases}
      \se(p) &\when \registers_9 = 0 \\
      \mathbf{o} &\when \registers_9 = 1 \\
      p_\mathbf{w}[\registers_{10}]_\mathbf{y} &\when \registers_9 = 2 \wedge \registers_{10} < |p_\mathbf{w}| \\
      p_\mathbf{w}[i]_\mathbf{y} &\when \registers_9 = 3 \\
      \mathbf{x} &\when \registers_9 = 4 \wedge \registers_{10} < |p_\mathbf{w}| \wedge \registers_{11} < |p_\mathbf{w}[\registers_{10}]_\mathbf{x}| \wedge (\mathcal{H}(\mathbf{x}), |\mathbf{x}|) = p_\mathbf{w}[\registers_{10}]_\mathbf{x}[\registers_{11}] \\
      \mathbf{x} &\when \registers_9 = 5 \wedge \registers_{10} < |p_\mathbf{w}[i]_\mathbf{x}| \wedge (\mathcal{H}(\mathbf{x}), |\mathbf{x}|) = p_\mathbf{w}[i]_\mathbf{x}[\registers_{10}] \\[2pt]
      \mathbf{x} &\when \left\{\begin{aligned}
        \registers_9 = 6 &\wedge \exists j, k : (\mathcal{H}(\mathbf{x}), |\mathbf{x}|) = p_\mathbf{w}[j]_\mathbf{x}[k] \\[2pt]
        &\wedge \mathbb{Z}_{\registers_{10} \dots+ 32} \subset \mathbb{V}_{\memory} \wedge \memory_{\registers_{10}\dots+32} = \mathcal{H}(\mathbf{x}) \\
      \end{aligned}\right.\\
      \overline{\mathbf{i}}[\registers_{10}]_{\registers_{11}} &\when \registers_9 = 7 \wedge \registers_{10} < |\overline{\mathbf{i}}| \wedge \registers_{11} < |\overline{\mathbf{i}}[\registers_{10}]| \\
      \overline{\mathbf{i}}[i]_{\registers_{10}} &\when \registers_9 = 8 \wedge \registers_{10} < |\overline{\mathbf{i}}[i]| \\
      \none &\otherwise
    \end{cases} \\
    \using o &= \registers_7 \\
    \using f &= \max(\registers_8 \div 2^{32}, |\mathbf{v}|) \\
    \using l &= \min(\registers_8 \bmod 2^{32}, |\mathbf{v}| - f) \\
    ...
    \end{cases} \\
    \registers'_7 &\equiv \begin{cases}
      \mathtt{OOB} &\when \mathbb{N}_{o \dots+ l} \not\subset \mathbb{V}^*_{\memory} \\
      \mathtt{NONE} & \otherwhen \mathbf{v} = \none \\
      \mathtt{OK} &\otherwise \\
    \end{cases}
  \end{aligned}$\\
