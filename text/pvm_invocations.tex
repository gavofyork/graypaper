...
Elements of \textsc{pvm} state are each assumed to remain unchanged by the host-call unless explicitly specified. If a given hostcall doesn't explicitly define $\gascounter'$ then the following holds:
\begin{align}
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  $\begin{aligned}
    ...
    \using g &= 100 + \max(0, \registers_7 - a - c) \cdot 10 \\
    \tup{\execst', \gascounter', \registers'_7, (\memory'_\ram¬access)\interval{a}{\registers_7}} &\equiv \begin{cases}
      % Not enough gas to even cover the constant cost of the call, regardless of whether it's a no-op or not.
      \tup{\oog, \gascounter, \registers_7, (\memory_\ram¬access)\interval{a}{\registers_7}} &\when \gascounter < 100 \\
      % Expand the accessible RW data region, and return the new page count.
      \tup{\continue, \gascounter - g, \max(a + c, \registers_7), \mathrm{W}} &\otherwhen \registers_7 \leq b \land \gascounter \ge g \\
      % Otherwise just return the current writable page count.
      %
      % This will also trigger when the user doesn't have enough gas to request as many pages
      % as they want, in which case they will be able to gracefully handle this situation.
      \tup{\continue, \gascounter - 100, a + c, (\memory_\ram¬access)\interval{a}{\registers_7}} &\otherwise \\
    \end{cases}
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
