...
\begin{align}
  ...
  &F \in \contextmutator{\tuple{\dictionary{\N}{\pvm¬instance}, \dictionary{\N}{\pvm¬module}, \sequence{\segment}}} \colon
    (n, \gascounter, \registers, \memory, \tup{\mathbf{m}, \pvm¬module¬map, \mathbf{e}}) \mapsto \begin{cases}
      ...
      \Omega_{CB}(\gascounter, \registers, \memory, \tup{\mathbf{m}, \pvm¬module¬map}) &\when n = \mathtt{compile\_from\_bytes}\\
      \Omega_{CD}(\gascounter, \registers, \memory, \tup{\mathbf{m}, \pvm¬module¬map}, w_\wi¬serviceindex, \accounts, (p_\wp¬context)_\wc¬lookupanchortime) &\when n = \mathtt{compile\_from\_db}\\
...
\begin{longtable}{p{4cm} p{12cm}}
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_{CB}(\gascounter, \registers, \memory, \tup{\mathbf{m}, \pvm¬module¬map})$ \\
  \texttt{compile\_from\_bytes} = 9 \\
  $g = 10$} &
  $\begin{aligned}
    \using \sq{p_O, p_Z} &= \registers\subrange{7}{2} \\
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_{CD}(\gascounter, \registers, \memory, \tup{\mathbf{m}, \pvm¬module¬map}, s, \mathbf{d}, t)$ \\
  \texttt{compile\_from\_db} = 10 \\
  $g = 10$} &
  $\begin{aligned}
    \using \mathbf{a} &= \begin{cases}
      \mathbf{d}\subb{s} &\when \registers_7 = 2^{64} - 1 \wedge s \in \keys{\mathbf{d}} \\
    ...
    \end{cases} \\
    \using \mathbf{v} &= \begin{cases}
      \error &\when \Nrange{\registers_8}{32} \not\subseteq \readable{\memory} \\
      \none &\otherwhen \mathbf{a} = \none \\
      \histlookup(\mathbf{a}, t, \memory\subrange{\registers_8}{32}) &\otherwise \\
    \end{cases} \\
    \using f &= \min(\registers_9, \len{\mathbf{v}}) \\
    \using l &= \min(\registers_{10}, \len{\mathbf{v}} - f) \\
    \using \mathbf{p} &= \mathbf{v}\subrange{f}{l} \\
    \using \mathbf{H} &= \keys{\pvm¬module¬map} \cup \set{\mathbf{m}\subb{n}_\pg¬handle : n \in \keys{\mathbf{m}}} \\
    \using k &= \min(n \in \N : n \not\in \mathbf{H}) \\
    \tup{\execst', \registers'_7, \pvm¬module¬map'} &\equiv \begin{cases}
      \tup{\continue, \mathtt{FULL}, \pvm¬module¬map} &\when \len{\mathbf{H}} \ge 128 \\
      \tup{\panic, \registers_7, \pvm¬module¬map} &\otherwhen \mathbf{v} = \error \\
      \tup{\continue, \mathtt{NONE}, \pvm¬module¬map} &\otherwhen \mathbf{v} = \none \\
      \tup{\continue, \mathtt{HUH}, \pvm¬module¬map} &\otherwhen \deblob(\mathbf{p}, 0) = \error \\
      \tup{\continue, k, \pvm¬module¬map \cup \set{\kv{k}{\mathbf{p}}} } &\otherwise \\
    \end{cases} \\
  \end{aligned}$\\
