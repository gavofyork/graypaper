...
\begin{align}
  ...
  &F \in \contextmutator{\tuple{\dictionary{\N}{\innerpvm}, \sequence{\segment}}} \colon
    (n, \gascounter, \registers, \memory, \tup{\mathbf{m}, \mathbf{e}}) \mapsto \begin{cases}
      ...
      \Omega_\Gemini(\gascounter, \registers, \memory, \tup{\mathbf{m}, \mathbf{e}}, \jam¬blob) &\when n = \mathtt{grow\_heap} \\
      ...
      \tup{\continue, \gascounter - \Cgasunknown, \registers', \memory, \tup{\mathbf{m}, \mathbf{e}}} &\otherwise\\
      \multicolumn{2}{l}{\where \registers' = \registers \exc \registers'_7 = \mathtt{WHAT}} \\
...
\begin{gather}
  ...
  \Psi_A \colon\abracegroup{
    ...
    \tup{\local¬basestate, t, s, g, \mathbf{i}} &\mapsto \begin{cases}
      \tup{
        ...
        &\when s \not\in \local¬basestate_\ps¬accounts \\
      \tup{
  ...
  F \in \contextmutator{\implicationspair} \colon \tup{n, \gascounter, \registers, \memory, \imXY} \mapsto \begin{cases}
    ...
    \Omega_J(\gascounter, \registers, \memory, \imXY) &\when n = \mathtt{eject} \\
...
These assume some refine context pair $\tup{\mathbf{m}, \mathbf{e}} \in \tuple{\dictionary{\N}{\innerpvm}, \sequence{\segment}}$, which are both initially empty.
\begin{equation}
  \context \equiv \tup{\mathbf{m}, \mathbf{e}}\ ,\qquad \context' \equiv \tup{\mathbf{m}', \mathbf{e}'}
\end{equation}
...
\begin{longtable}{p{3cm} p{13cm}}
  ...
  $\begin{aligned}
    ...
    \tup{\execst', \registers'_7, (\imX_\im¬state')_\ps¬accounts, \imX_\im¬modifiedaccounts'} &\equiv \begin{cases}
      ...
      \tup{\continue, \mathtt{WHO}, \mathbf{d}, \imX_\im¬modifiedaccounts} &\when d \not\in \keys{\mathbf{d}} \\
      \tup{\continue, \mathtt{HUH}, \mathbf{d}, \imX_\im¬modifiedaccounts} &\when \imX_\im¬id \not\in \set{d, \fneffectivesupervisor(\mathbf{d}, d)} \\
    ...
    \using \mathbf{v} &= \begin{cases}
      \begin{aligned}
        &\fnencode(
          ...
          \encode[4]{\mathbf{a}_\sa¬supervisor},
  ...
  $\begin{aligned}
    ...
    \using \mathbf{a} \in \serviceaccount &= \tup{\begin{aligned}
      ...
      \sa¬balance,
      \is{\sa¬supervisor}{\tup{\imX_\im¬id, (\imX_\im¬self)_\sa¬created}},
      \sa¬supervisorbalance, \\
      &\sa¬minaccgas,
      \sa¬minmemogas,
    ...
    \end{aligned}} \\
    &\phantom{{}={}} \where \tup{\sa¬balance, \sa¬supervisorbalance} = \begin{cases}
      \tup{\mathbf{a}_\sa¬minbalance, 0} &\when x \in \set{0, 1} \\
      \tup{0, \mathbf{a}_\sa¬minbalance} &\otherwise
    \end{cases} \\
    \using \mathbf{d} &= (\imX_\im¬state)_\ps¬accounts \\
    \using \mathbf{d}' &= \mathbf{d} \exc \begin{cases}
    ...
    \end{cases} \\
    \using p &= \begin{cases}
      \imX_\im¬id \in \set{s, \fneffectivesupervisor(\mathbf{d}, s)} &\when x \in \set{0, 2} \\
      \imX_\im¬id = \fneffectivesupervisor(\mathbf{d}, s) &\otherwise
    \end{cases} \\
  ...
  $\begin{aligned}
    ...
    \end{cases} \\
    \using \mathbf{d} &= (\imX_\im¬state)_\ps¬accounts \\
    \using \mathbf{d}' &= \mathbf{d} \exc \mathbf{d}'\subb{d}_\sa¬supervisor = \tup{u, \mathbf{d}\subb{u}_\sa¬created} \\
  ...
  $\begin{aligned}
    ...
    \using p_s &= \begin{cases}
      \imX_\im¬id \in \set{\dx¬source, \fneffectivesupervisor(\mathbf{d}, \dx¬source)} &\when x \in \set{0, 2} \\
      \imX_\im¬id = \fneffectivesupervisor(\mathbf{d}, \dx¬source) &\otherwise
    \end{cases} \\
    \using p_d &= o \ne 0 \lor (\imX_\im¬id \in \set{\dx¬dest, \fneffectivesupervisor(\mathbf{d}, \dx¬dest)} \land (x \in \set{1, 2} \lor s \ne d)) \\
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_J(\gascounter, \registers, \memory, \imXY)$ \\
  \texttt{eject} = 22} &
  $\begin{aligned}
    \using d &= \registers_7 \\
    \using \mathbf{d} &= (\imX_\im¬state)_\ps¬accounts \\
  ...
  $\begin{aligned}
    ...
    \using \mathbf{d}' &= \begin{cases}
      ...
      \quad \left.
        ...
        \end{aligned}
      \ \right\} &\when \mathbf{d}\subb{d}_\sa¬requests\subb{\tup{h, z}} \in \set{\sq{}, \sq{x, y}},\ y < t - \Cexpungeperiod \\
