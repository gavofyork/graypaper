...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  $\begin{aligned}
    ...
    \using \mathbf{s} &= \begin{cases}
      \none &\when n \not\in \keys{\mathbf{m}}\\
      \mathbf{m}[n]_{\mathbf{u}_{s\dots+l}} &\when \N_{s\dots+l} \in \mathbb{V}_{\mathbf{m}[n]_\mathbf{u}} \wedge \N_{o\dots+l} \in \mathbb{V}^*_\lambda \\
      \error &\otherwise
    \end{cases}\\
    (\registers'_7, \mem') &\equiv \begin{cases}
      (\mathtt{OOB}, \mem) &\when \mathbf{s} = \error \\
      (\mathtt{WHO}, \mem) &\when \mathbf{s} = \none \\
      (\mathtt{OK}, \mem') \ \where \mem' = \mem \exc \mem'_{o\dots+l} = \mathbf{s} &\otherwise
    \end{cases} \\
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  ...
  $\begin{aligned}
    ...
    \using \mathbf{s} &= \begin{cases}
      \none &\when n \not\in \keys{\mathbf{m}} \\
      \mem_{s\dots+l} &\when \N_{s\dots+l} \in \mathbb{V}_\mathbf{u} \wedge \N_{o\dots+l} \in \mathbb{V}^*_\memory \\
      \error &\otherwise
    \end{cases}\\
    (\registers'_7, \mathbf{m}') &\equiv \begin{cases}
      (\mathtt{OOB}, \mathbf{m}) &\when \mathbf{s} = \error \\
      (\mathtt{WHO}, \mathbf{m}) &\when \mathbf{s} = \none \\
      (\mathtt{OK}, \mathbf{m}')\,,\ \where \mathbf{m}' = \mathbf{m} \exc (\mathbf{m}'[n]_\mathbf{u})_{o\dots+l} = \mathbf{s} = \mathbf{u}' &\otherwise \\
    \end{cases} \\
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
