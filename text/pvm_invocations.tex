...
\begin{align}
  &\Psi_R \colon \abracegroup{
    ...
    \tup{i, p, \mathbf{r}, \overline{\mathbf{i}}, \segoff} &\mapsto \begin{cases}
      \tup{\token{BAD}, [], 0} &\when w_s \not\in \keys{\accounts} \vee \histlookup(\accounts[w_s], (p_\mathbf{x})_t, w_\wi¬codehash) = \none \\
      \tup{\token{BIG}, [], 0} &\otherwhen \len{\histlookup(\accounts[w_s], (p_\mathbf{x})_t, w_\wi¬codehash)} > \mathsf{W}_C \\
      &\otherwise: \\
      ...
      &\quad\also (u, \mathbf{o}, \tup{\mathbf{m}, \mathbf{e}}) = \Psi_M(\mathbf{c}, 0, w_g, a, F, \tup{\emptyset, []})\ \colon\\
      \tup{\mathbf{o}, [], u} &\quad\when \mathbf{o} \in \set{ \oog, \panic }  \\
  ...
  &F \in \Omega\ang{\tuple{\dict{\N}{\pvm}, \sequence{\segment}}} \colon
    (n, \gascounter, \registers, \memory, \tup{\mathbf{m}, \mathbf{e}}) \mapsto \begin{cases}
      ...
      \Omega_H(\gascounter, \registers, \memory, \tup{\mathbf{m}, \mathbf{e}}, w_s, \accounts, (p_\mathbf{x})_t) &\when n = \mathtt{historical\_lookup}\\
...
\begin{align}
  \Psi_A& \colon\abracegroup{
    ...
    \tup{\mathbf{u}, t, s, g, \mathbf{o}} &\mapsto \begin{cases}
  ...
  I&\colon\abracegroup{
    \tuple{\mathbb{U}, \serviceid} &\to \mathbf{X}\\
    \tup{\mathbf{u}, s} &\mapsto \tup{
      s,
      \mathbf{u},
      i,
      \is{\mathbf{t}}{\sq{}},
  ...
  F \in \Omega\ang{\tuple{\mathbf{X}, \mathbf{X}}} &\colon \tup{n, \gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}}} \mapsto \begin{cases}
    \Omega_G(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}}) &\when n = \mathtt{gas} \\
    \Omega_Y(\gascounter, \registers, \memory, \none, \entropyaccumulator', \none, \none, \none, \none, \mathbf{o}, \none, \tup{\mathbf{m}, \mathbf{e}}) &\when n = \mathtt{fetch}\\
    G(\Omega_R(\gascounter, \registers, \memory, \mathbf{s}, \mathbf{x}_s, \mathbf{d}), \tup{\mathbf{x}, \mathbf{y}}) &\when n = \mathtt{read} \\
    G(\Omega_W(\gascounter, \registers, \memory, \mathbf{s}, \mathbf{x}_s), \tup{\mathbf{x}, \mathbf{y}}) &\when n = \mathtt{write} \\
    G(\Omega_L(\gascounter, \registers, \memory, \mathbf{s}, \mathbf{x}_s, \mathbf{d}), \tup{\mathbf{x}, \mathbf{y}}) &\when n = \mathtt{lookup} \\
    G(\Omega_I(\gascounter, \registers, \memory, \mathbf{x}_s, \mathbf{d}), \tup{\mathbf{x}, \mathbf{y}}) &\when n = \mathtt{info} \\
    \Omega_B(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}}) &\when n = \mathtt{bless}\\
    \Omega_A(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}}) &\when n = \mathtt{assign}\\
    \Omega_D(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}}) &\when n = \mathtt{designate}\\
    \Omega_C(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}}) &\when n = \mathtt{checkpoint} \\
    \Omega_N(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}}, \H_\¬timeslot) &\when n = \mathtt{new} \\
    \Omega_U(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}}) &\when n = \mathtt{upgrade} \\
    \Omega_T(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}}) &\when n = \mathtt{transfer} \\
    \Omega_J(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}}, \H_\¬timeslot) &\when n = \mathtt{eject} \\
    \Omega_Q(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}}) &\when n = \mathtt{query} \\
    \Omega_S(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}}, \H_\¬timeslot) &\when n = \mathtt{solicit} \\
    \Omega_F(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}}, \H_\¬timeslot) &\when n = \mathtt{forget} \\
    \Omega_\Taurus(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}}) &\when n = \mathtt{yield} \\
    \Omega_\Aries(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}}, \mathbf{x}_s) &\when n = \mathtt{provide} \\
    \tup{\continue, \gascounter - 10, [\registers_0, \dots, \registers_6, \mathtt{WHAT}, \registers_8, \dots], \memory, \mathbf{x}} &\otherwise\\
    \quad \where \mathbf{d} = (\mathbf{x}_\mathbf{u})_\mathbf{d}\;,\ \mathbf{s} = (\mathbf{x}_\mathbf{u})_\mathbf{d}[\mathbf{x}_s]\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!& \\
  \end{cases} \\
  G&\colon\abracegroup{
    \tuple{\tuple{\set{\continue, \halt, \panic, \oog}, \gas, \regs, \ram, \serviceaccount}, \tuple{\mathbf{X}, \mathbf{X}}} &\to \tuple{\set{\continue, \halt, \panic, \oog}, \gas, \regs, \ram, \tuple{\mathbf{X}, \mathbf{X}}} \\
    (\tup{\execst, \gascounter, \registers, \memory, \mathbf{s}}, \tup{\mathbf{x}, \mathbf{y}}) &\mapsto (\execst, \gascounter, \registers, \memory, \tup{\mathbf{x}^*, \mathbf{y}}) \\
    &\qquad \where \mathbf{x}^* = \mathbf{x} \exc (\mathbf{x}^*_\mathbf{u})_\mathbf{d}[\mathbf{x}^*_s] = \mathbf{s}
  }\\
  C&\colon\abracegroup{
    \tuple{\gas, \blob \cup \set{\oog, \panic}, \tuple{\mathbf{X}, \mathbf{X}}} &\to \tup{\partialstate, \defxfers, \optional{\hash}, \gas, \sequence{\tuple{\serviceid, \blob}}} \\
    (u, \mathbf{o}, \tup{\mathbf{x}, \mathbf{y}}) &\mapsto \begin{cases}
      \tup{
        \mathbf{y}_\mathbf{u},
        \mathbf{y}_\mathbf{t},
        \mathbf{y}_y,
...
\begin{align}
  \Psi_T &: \begin{cases}
    \tuple{\dict{\serviceid}{\serviceaccount}, \timeslot, \serviceid, \sequence{\mathbb{T}}} &\to \tuple{\serviceaccount, \gas} \\
    ...
    \multicolumn{2}{l}{
      \begin{aligned}
        \quad\where \tup{u, \mathbf{r}, \mathbf{s}'} &= \Psi_M(\mathbf{s}_\sa¬code, 10, \sum_{r \in \mathbf{t}}{(r_g)}, \encode{t, s, \len{\mathbf{t}}), F, \mathbf{s}} \\
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  $\begin{aligned}
    \using \mathbf{v} &= \begin{cases}
      ...
      \multicolumn{2}{l}{\where \mathbf{c} = \se\left(
        \begin{aligned}
          &\fixedencode{8}{\mathsf{B}_I},
          \fixedencode{8}{\mathsf{B}_L},
          \fixedencode{8}{\mathsf{B}_S},
          \fixedencode{2}{\corecount},
          \fixedencode{4}{\mathsf{D}},
          \fixedencode{4}{\epochlen},
          \fixedencode{8}{\mathsf{G}_A},\\
          &\fixedencode{8}{\mathsf{G}_I},
          \fixedencode{8}{\mathsf{G}_R},
          \fixedencode{8}{\mathsf{G}_T},
          \fixedencode{2}{\mathsf{H}},
          \fixedencode{2}{\mathsf{I}},
          \fixedencode{2}{\mathsf{J}},
          \fixedencode{2}{\mathsf{K}},\\
          &\fixedencode{4}{\mathsf{L}},
          \fixedencode{2}{\mathsf{N}},
          \fixedencode{2}{\mathsf{O}},
          \fixedencode{2}{\mathsf{P}},
          \fixedencode{2}{\mathsf{Q}},
          \fixedencode{2}{\mathsf{R}},
          \fixedencode{2}{\mathsf{T}},
          \fixedencode{2}{\mathsf{U}},\\
          &\fixedencode{2}{\valcount},
          \fixedencode{4}{\mathsf{W}_A},
          \fixedencode{4}{\mathsf{W}_B},
          \fixedencode{4}{\mathsf{W}_C},
          \fixedencode{4}{\mathsf{W}_E},
  ...
  $\begin{aligned}
    ...
    \using \mathbf{v} &= \begin{cases}
      ...
      \mathbf{a}_\mathbf{p}[\memory\subrange{h}{32}] &\otherwise \\
    \end{cases} \\
  ...
  $\begin{aligned}
    ...
    \using \mathbf{a} &= \begin{cases}
      ...
      \none &\otherwise
    \end{cases} \\
    \using [k_o, k_z, o] &= \registers\subrange{8}{3} \\
    \using \mathbf{v} &= \begin{cases}
      \error &\when \Nrange{k_o}{k_z} \not\subseteq \readable{\memory} \\
      \mathbf{a}_\mathbf{s}[\mathbf{k}] &\otherwhen \mathbf{a} \ne \none \wedge \mathbf{k} \in \keys{\mathbf{a}_\mathbf{s}}\,,\ \where \mathbf{k} = \memory\subrange{k_o}{k_z} \\
      \none &\otherwise
    \end{cases} \\
  ...
  $\begin{aligned}
    \using [k_o, k_z, v_o, v_z] &= \registers\subrange{7}{4} \\
    \using \mathbf{k} &= \begin{cases}
      \memory\subrange{k_o}{k_z} &\when \Nrange{k_o}{k_z} \subseteq \readable{\memory} \\
      \error &\otherwise
    \end{cases} \\
    \using \mathbf{a} &= \begin{cases}
      \mathbf{s}\,,\ \exc \keys{\mathbf{a}_\mathbf{s}} = \keys{\mathbf{a}_\mathbf{s}} \setminus \set{k} & \when v_z = 0 \\
      \mathbf{s}\,,\ \exc \mathbf{a}_\mathbf{s}[\mathbf{k}] = \memory\subrange{v_o}{v_z} &\otherwhen \Nrange{v_o}{v_z} \subseteq \readable{\memory} \\
      \error &\otherwise
    \end{cases} \\
    \using l &= \begin{cases}
      \len{\mathbf{s}}\mathbf{s}[k]| &\when \mathbf{k} \in \keys{\mathbf{s}_\mathbf{s}} \\
    ...
    \tup{\execst', \registers'_7, \mathbf{s}'} &\equiv \begin{cases}
      ...
      \tup{\continue, \mathtt{FULL}, \mathbf{s}} &\otherwhen \mathbf{a}_t > \mathbf{a}_b \\
  ...
  $\begin{aligned}
    ...
    \using \mathbf{v} &= \begin{cases}
      \se(
        \mathbf{a}_c,
        \fixedencode{8}{\mathbf{a}_b, \mathbf{a}_t, \mathbf{a}_g, \mathbf{a}_m, \mathbf{a}_o},
        \fixedencode{4}{\mathbf{a}_i},
        \fixedencode{8}{\mathbf{a}_f},
        \fixedencode{4}{\mathbf{a}_r, \mathbf{a}_a, \mathbf{a}_p}
      ) &\when \mathbf{a} \ne \none \\
      \none &\otherwise
    \end{cases} \\
    \using f &= \min(\registers_{11}, \len{\mathbf{v}}) \\
...
\begin{longtable}{p{4cm} p{12cm}}
  ...
  $\begin{aligned}
    ...
    \using \mathbf{x} &= \begin{cases}
      \zeropad_{\mathsf{W}_G}(\mem\subrange{p}{z}) &\when \Nrange{p}{z} \subseteq \mathbb{V}_\memory\\
      \error &\otherwise
    \end{cases}\\
    \tup{\execst', \registers'_7, \mathbf{e}'} &\equiv \begin{cases}
      \tup{\panic, \registers_7, \mathbf{e}} &\when \mathbf{x} = \error \\
      \tup{\continue, \mathtt{FULL}, \mathbf{e}} &\otherwhen \segoff+\len{\mathbf{e}} \ge \mathsf{W}_X \\
  ...
  $g = 10$} &
  $\begin{aligned}
    \using [p_o, p_z, i] &= \registers\subrange{7}{3} \\
  ...
  $g = 10$} &
  $\begin{aligned}
    \using [n, o, s, z] &= \registers\subrange{7}{4} \\
    \tup{\execst', \registers'_7, \mem'} &\equiv \begin{cases}
      ...
      \multicolumn{2}{l}{\where \mem' = \mem \exc \mem\subrange{o}{z} = (\mathbf{m}[n]_\mathbf{u})\subrange{s}{z}}
    \end{cases} \\
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  ...
  $g = 10$} &
  $\begin{aligned}
    \using [n, s, o, z] &= \registers\subrange{7}{4} \\
    \tup{\execst', \registers'_7, \mathbf{m}'} &\equiv \begin{cases}
      ...
      \multicolumn{2}{l}{\where \mathbf{m}' = \mathbf{m} \exc (\mathbf{m}'[n]_\mathbf{u})\subrange{o}{z} = \mem\subrange{s}{z}}
    \end{cases} \\
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  ...
  $g = 10$} &
  $\begin{aligned}
    \using [n, p, c, r] &= \registers\subrange{7}{4} \\
    ...
    \tup{\registers'_7, \mathbf{m}'} &\equiv \begin{cases}
      ...
      \tup{\mathtt{OK}, \mathbf{m}'} &\otherwise\,,\ \where \mathbf{m}' = \mathbf{m} \exc \mathbf{m}'[n]_\mathbf{u} = \mathbf{u}' \\
    \end{cases} \\
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  ...
  $\begin{aligned}
    ...
    \using \tup{g, \mathbf{w}} &= \begin{cases}
      \tup{g, \mathbf{w}}: \fixedencode{8}{g} \concat \fixedencode{8}{\mathbf{w}} = \mem\subrange{o}{112} &\when \Nrange{o}{112} \subseteq \writable{\mem} \\
    ...
    \end{cases} \\
    \using \tup{c, i', g', \mathbf{w}', \mathbf{u}'} &= \Psi(\mathbf{m}[n]_\mathbf{p}, \mathbf{m}[n]_i, g, \mathbf{w}, \mathbf{m}[n]_\mathbf{u})\\
    \using \mem^* &= \mem \exc \mem^*\subrange{o}{112} = \fixedencode{8}{g'} \concat \fixedencode{8}{\mathbf{w}'}\\
  ...
  $\begin{aligned}
    ...
    \tup{\registers'_7, \mathbf{m}'} &\equiv \begin{cases}
      ...
      \tup{\mathbf{m}[n]_i, \mathbf{m} \setminus n} &\otherwise \\
    \end{cases} \\
  \end{aligned}$\\
  \bottomrule
\end{longtable}
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  \endhead
  \makecell*[l]{
  $\Omega_B(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}})$ \\
  \texttt{bless} = 14 \\
  $g = 10$}&
  $\begin{aligned}
    \using [m, a, v, o, n] &= \registers\subrange{7}{5} \\
    \using \mathbf{a} &= \begin{cases}
      \decode{4}{\memory\subrange{a}{4\corecount}} &\when \Nrange{a}{4\corecount} \subseteq \readable{\memory} \\
      \error &\otherwise
    \end{cases} \\
    \using \mathbf{z} &= \begin{cases}
      \set{\build{\kv{s}{g} \ \where \fixedencode{4}{s} \concat \fixedencode{8}{g} = \memory\subrange{o+12i}{12}}{i \in \Nmax{n}}} &\when \Nrange{o}{12n} \subseteq \readable{\memory} \\
      \error &\otherwise
    \end{cases} \\
    (\execst', \registers'_7, (\mathbf{x}'_\mathbf{u})_{\tup{m, \mathbf{a}, v, \mathbf{z}}}) &= \begin{cases}
      (\panic, \registers_7, (\mathbf{x}_\mathbf{u})_{\tup{m, \mathbf{a}, v, \mathbf{z}}}) &\when \set{\mathbf{z}, \mathbf{a}} \ni \error \\
      (\continue, \mathtt{HUH}, (\mathbf{x}_\mathbf{u})_{\tup{m, \mathbf{a}, v, \mathbf{z}}}) &\otherwhen \mathbf{x}_s \ne (\mathbf{x}_\mathbf{u})_m\\
      (\continue, \mathtt{WHO}, (\mathbf{x}_\mathbf{u})_{\tup{m, \mathbf{a}, v, \mathbf{z}}}) &\otherwhen \tup{m, v} \not\in \tuple{\serviceid, \serviceid} \\
      \tup{\continue, \mathtt{OK}, \tuple{m, \mathbf{a}, v, \mathbf{z}}} &\otherwise \\
    \end{cases}
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_A(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}})$ \\
  \texttt{assign} = 15 \\
  $g = 10$} &
  $\begin{aligned}
    \using [c, o, a] &= \registers\subrange{7}{3} \\
    \using \mathbf{q} &= \begin{cases}
      \sq{\build{\memory\subrange{o + 32i}{32}}{i \orderedin \N_\mathsf{Q}}} &\when \Nrange{o}{32\mathsf{Q}} \subseteq \readable{\memory} \\
      \error &\otherwise
    \end{cases} \\
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_D(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}})$ \\
  \texttt{designate} = 16 \\
  $g = 10$} &
  $\begin{aligned}
    \using o &= \registers_7 \\
    \using \mathbf{v} &= \begin{cases}
      \sq{\build{\memory\subrange{o + 336i}{336}}{i \orderedin \valindex}} &\when \Nrange{o}{336\valcount} \subseteq \readable{\memory} \\
      \error &\otherwise
    \end{cases} \\
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_C(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}})$ \\
  \texttt{checkpoint} = 17 \\
  $g = 10$} &
  $\begin{aligned}
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_N(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}}, t)$ \\
  \texttt{new} = 18 \\
  $g = 10$} &
  $\begin{aligned}
    \using [o, l, g&, m, f] = \registers\subrange{7}{5} \\
    \using c &= \begin{cases}
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
    $\Omega_U(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}})$ \\
    \texttt{upgrade} = 19 \\
    $g = 10$
  } &
  $\begin{aligned}
    \using [o, g, m] &= \registers\subrange{7}{3} \\
    \using c &= \begin{cases}
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_T(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}})$ \\
  \texttt{transfer} = 20 \\
  $g = 10 + \registers_9 $} &
  $\begin{aligned}
    \using [d, a, l, o] &= \registers\subrange{7}{4},  \\
    \using \mathbf{d} &= (\mathbf{x}_\mathbf{u})_\mathbf{d}\\
    \using \mathbf{t} \in \mathbb{T} \cup \set{\error} &= \begin{cases}
      \tup{\is{s}{\mathbf{x}_s}, d, a, \is{m}{\memory_{o\dots+\mathsf{W}_T}}, \is{g}{l}} &\when \Nrange{o}{\mathsf{W}_T} \subseteq \readable{\memory} \\
      \error &\otherwise
    \end{cases} \\
    ...
    (\execst', \registers'_7, \mathbf{x}'_\mathbf{t}, (\mathbf{x}'_\mathbf{s})_b) &\equiv \begin{cases}
      ...
      \tup{\continue, \mathtt{OK}, \mathbf{x}_\mathbf{t} \append \mathbf{t}, b} &\otherwise
    \end{cases} \\
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_J(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}}, t)$ \\
  \texttt{eject} = 21 \\
  $g = 10$} &
  $\begin{aligned}
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_Q(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}})$ \\
  \texttt{query} = 22 \\
  $g = 10$} &
  $\begin{aligned}
    ...
    \end{cases} \\
    \using \mathbf{a} &= \begin{cases}
      (\mathbf{x}_\mathbf{s})_\mathbf{l}[h, z] &\when \tup{h, z} \in \keys{(\mathbf{x}_\mathbf{s})_\mathbf{l}}\\
      \error &\otherwise\\
    \end{cases} \\
    \tup{\execst', \registers'_7, \registers'_8} &\equiv \begin{cases}
      ...
      \tup{\continue, 0, 0} &\otherwhen \mathbf{a} = [] \\
      \tup{\continue, 1 + 2^{32}x, 0} &\otherwhen \mathbf{a} = [x] \\
      \tup{\continue, 2 + 2^{32}x, y} &\otherwhen \mathbf{a} = [x, y] \\
      \tup{\continue, 3 + 2^{32}x, y + 2^{32}z} &\otherwhen \mathbf{a} = [x, y, z] \\
    \end{cases} \\
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_S(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}}, t)$ \\
  \texttt{solicit} = 23 \\
  $g = 10$} &
  $\begin{aligned}
    \using [o, z] &= \registers_{7, 8} \\
    ...
    \using \mathbf{a} &= \begin{cases}
      ...
      \error &\otherwise\\
    \end{cases} \\
    \tup{\execst', \registers'_7, \mathbf{x}'_\mathbf{s}} &\equiv \begin{cases}
      \tup{\panic, \registers_7, \mathbf{x}_\mathbf{s}} &\when h = \error \\
      \tup{\continue, \mathtt{HUH}, \mathbf{x}_\mathbf{s}} &\otherwhen \mathbf{a} = \error \\
      \tup{\continue, \mathtt{FULL}, \mathbf{x}_\mathbf{s}} &\otherwhen \mathbf{a}_b < \mathbf{a}_t \\
      \tup{\continue, \mathtt{OK}, \mathbf{a}} &\otherwise \\
    \end{cases} \\
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_F(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}}, t)$ \\
  \texttt{forget} = 24 \\
  $g = 10$} &
  $\begin{aligned}
    \using [o, z] &= \registers_{7, 8} \\
    ...
    \using \mathbf{a} &= \begin{cases}
      ...
      \quad \left.
        \begin{aligned}
          \keys{\mathbf{a}_\mathbf{l}} &= \keys{(\mathbf{x}_\mathbf{s})_\mathbf{l}} \setminus \set{\tup{h, z}}\ ,\\[2pt]
          \keys{\mathbf{a}_\mathbf{p}} &= \keys{(\mathbf{x}_\mathbf{s})_\mathbf{p}} \setminus \set{h}
        \end{aligned}
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_\Taurus(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}})$ \\
  \texttt{yield} = 25 \\
  $g = 10$} &
  $\begin{aligned}
    \using o &= \registers_7 \\
    \using h &= \begin{cases}
      ...
      \error &\otherwise
    \end{cases} \\
    \tup{\execst', \registers'_7, \mathbf{x}'_y} &\equiv \begin{cases}
      \tup{\panic, \registers_7, \mathbf{x}_y} &\when h = \error \\
      \tup{\continue, \mathtt{OK}, h} &\otherwise \\
    \end{cases} \\
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_\Aries(\gascounter, \registers, \memory, \tup{\mathbf{x}, \mathbf{y}}, s)$ \\
  \texttt{provide} = 26 \\
  $g = 10$} &
  $\begin{aligned}
    ...
    \using \mathbf{a} &= \begin{cases}
      ...
      \none &\otherwise
    \end{cases} \\
    \tup{\execst', \registers'_7, \mathbf{x}'_\mathbf{p}} &\equiv \begin{cases}
      \tup{\panic, \registers_7, \mathbf{x}_\mathbf{p}} &\when \mathbf{i} = \error \\
      \tup{\continue, \mathtt{WHO}, \mathbf{x}_\mathbf{p}} &\otherwhen \mathbf{a} = \none \\
      \tup{\continue, \mathtt{HUH}, \mathbf{x}_\mathbf{p}} &\otherwhen \mathbf{a}_\mathbf{l}[\tup{\blake(\mathbf{i}), z}] \ne \sq{} \\
      \tup{\continue, \mathtt{HUH}, \mathbf{x}_\mathbf{p}} &\otherwhen \tup{s^*, \mathbf{i}} \in \mathbf{x}_\mathbf{p} \\
      \tup{\continue, \mathtt{OK}, \mathbf{x}_\mathbf{p} \cup \set{\tup{s^*, \mathbf{i}}}} &\otherwise \\
    \end{cases} \\
  \end{aligned}$\\
  \bottomrule
\end{longtable}
