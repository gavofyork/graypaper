...
TODO: Bound the size of Is Authorized output or alternatively the total encoded size of a Work Report.

\begin{align}
  \Psi_I &\colon \left\{\begin{aligned}
    (\mathbb{P}, \N_\mathsf{C}) &\to \Y_{\mathsf{W}_U} \cup \mathbb{J} \\
...
\begin{align}
  &\Psi_R \colon \left\{\begin{aligned}
    \left(\begin{aligned}
      ...
      &\H, \Y_{\mathsf{W}_U}, \seq{\G}, \seq{\Y}, \N
    \end{aligned}
...
\begin{align}
  \Psi_T &: \begin{cases}
    ...
    \Psi_M(\mathbf{s}_\mathbf{c}, 10, \sum_{r \in \mathbf{t}}{(r_g)}, \se(t, s, \var{\mathbf{t}}), F, \mathbf{s}) &\otherwise
    \end{cases} \\
  \end{cases} \\
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  $\begin{aligned}
    ...
    \using \mathbf{t} \in \mathbb{T} \cup \{\error,\none\} &= \begin{cases}
      \none &\when d \in \{ \mathbf{x}_s, 2^{64} - 1 \} \\
      \tup{\is{s}{\mathbf{x}_s}, d, a, \is{m}{\memory_{o\dots+\mathsf{W}_T}}, g} &\otherwhen \N_{o\dots+\mathsf{W}_T} \subset \mathbb{V}_{\memory} \\
      \error &\otherwise
    \end{cases} \\
