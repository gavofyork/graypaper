...
\begin{align}
  ...
  I&\colon\left\{\begin{aligned}
    ...
    &\qquad\where i = \text{check}((\de_4(\blake(\se(s, \eta'_0, \H_\¬timeslot))) \bmod (2^{32}-2^9)) + 2^8) \\
  \end{aligned}\right.\\
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  $\begin{aligned}
    \using o &= \registers_7 \\
    \using \mathbf{v} &= \begin{cases}
      \left[\memory_{o + 336i \dots+ 336} \mid i \orderedin \valindex\right] &\when \mathbb{N}_{o \dots+ 336\mathsf{V}} \subseteq \mathbb{V}_{\memory} \\
      \error &\otherwise
    \end{cases} \\
