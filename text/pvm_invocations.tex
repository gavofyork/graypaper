...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  $\begin{aligned}
    \using \mathbf{a} &= \begin{cases}
      \imX_\im¬self &\when \registers_7 \in \set{ \imX_\im¬id, 2^{64} - 1 } \\
      (\imX_\im¬state)_\ps¬accounts[\registers_7] &\otherwhen \registers_7 \in \keys{(\imX_\im¬state)_\ps¬accounts} \\
      \none &\otherwise
    \end{cases} \\
    \using \sq{h, o} &= \registers\subrange{8}{2} \\
    \using z &= \registers_{11} \\
  ...
  $\begin{aligned}
    ...
    \end{cases} \\
    \using \mathbf{a} &= \begin{cases}
      \imX_\im¬self &\when s = \imX_\im¬id \\
      (\imX_\im¬state)_\ps¬accounts[s] &\otherwhen s \in \keys{(\imX_\im¬state)_\ps¬accounts} \\
      \none &\otherwise
    \end{cases} \\
