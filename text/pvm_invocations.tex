...
\begin{align}
  \Psi_R &\colon \left\{\begin{aligned}
    \left(\begin{aligned}
      &\H, \N_G, \N_S, \H, \Y, \mathbb{X}, \H,\\
      &\Y, \seq{\Y_{\mathsf{W}_S}}, \seq{\Y_{\mathsf{W}_S}}, \N
    \end{aligned}
    ...
    (c, g, s, p, \mathbf{y}, \mathbf{c}, a, \mathbf{o}, \mathbf{i}, \mathbf{x}, \ell) &\mapsto \begin{cases}
      ...
      &\otherwise: \\
      &\quad\using a = \mathcal{E}(s, p, \mathbf{y}, \mathbf{c}, a, \mathbf{o}, \var{\mathbf{i}}, \var{\mathbf{x}})\ ,\\
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  $\begin{aligned}
    ...
    \using \mathbf{x} &= \begin{cases}
      [\mem_{o+i\cdot\mathsf{W}_S\dots+\mathsf{W}_S} \mid i \orderedin \N_z] &\when \N_{o\dots+z\cdot\mathsf{W}_S} \in \mathbb{V}_\mu\\
      \error &\otherwise
    \end{cases}\\
    (\omega'_0, \mathbf{e}') &\equiv \begin{cases}
      (\mathtt{OOB}, \mathbf{e}) &\when \mathbf{x} = \error \\
      (\mathtt{FULL}, \mathbf{e}) &\otherwhen |\mathbf{e}| \ge \mathsf{W}_X \\
      (\ell + |\mathbf{e}|, \mathbf{e} \concat \mathbf{x}) &\otherwise
    \end{cases}
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
