...
\begin{align}
  \mathbf{X} &\equiv \tuple{
    ...
    \isa{y}{\optional{\hash}},
    \isa{\mathbf{p}}{\{\tuple{\serviceid, \blob}\}}
  }\\
...
\begin{align}
  ...
  G&\colon\left\{\begin{aligned}
    ((\{\continue, \halt, \panic, \oog\}, \gas, \regs, \ram, \serviceaccount), (\mathbf{X}, \mathbf{X})) &\to (\{\continue, \halt, \panic, \oog\}, \gas, \regs, \ram, (\mathbf{X}, \mathbf{X})) \\
  ...
  C&\colon\left\{\begin{aligned}
    (\gas, \blob \cup \{\oog, \panic\}, (\mathbf{X}, \mathbf{X})) &\to (\partialstate, \defxfers, \optional{\hash}, \gas, \seq{\tuple{\serviceid, \blob}}) \\
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  $\begin{aligned}
    ...
    \end{cases} \\
    \using \mathbf{a} &= \begin{cases}
      \mathbf{s}\,,\ \exc \Fkeys{\mathbf{a}_\mathbf{s}} = \Fkeys{\mathbf{a}_\mathbf{s}} \setminus \{k\} & \when v_z = 0 \\
...
These assume some refine context pair $(\mathbf{m}, \mathbf{e}) \in (\dict{\N}{\pvm}, \seq{\segment})$, which are both initially empty. Other than the gas-counter which is explicitly defined, elements of \textsc{pvm} state are each assumed to remain unchanged by the host-call unless explicitly specified.
\begin{align}
  \gascounter' &\equiv \gascounter - g\\
...
\begin{longtable}{p{4cm} p{12cm}}
  ...
  $\begin{aligned}
    ...
    \using \mathbf{p} &= \begin{cases}
      \memory_{p_o\dots+p_z} &\when \mathbb{N}_{p_o \dots+ p_z} \subseteq \readable{\memory} \\
      \error &\otherwise
    \end{cases} \\
  ...
  $\begin{aligned}
    ...
    \using (g, \mathbf{w}) &= \begin{cases}
      (g, \mathbf{w}): \se_8(g) \concat \joined{\se^\#_8(\mathbf{w})} = \mem_{o \dots+ 112} &\when \N_{o \dots+ 112} \subseteq \writable{\mem} \\
    ...
    \using \mathbf{m}^* &= \mathbf{m} \exc \begin{cases}
      ...
      \mathbf{m}^*[n]_i = \begin{cases}
        i' + 1 &\when c \in \{ \host \} \times \pvmreg\\
        i' &\otherwise
      \end{cases}
    \end{cases}\\
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  $\begin{aligned}
    ...
    \using \mathbf{a} &= \begin{cases}
      \de_4(\memory_{a\dots+4\corecount}) &\when \mathbb{N}_{a \dots+ 4\corecount} \subseteq \readable{\memory} \\
      \error &\otherwise
    \end{cases} \\
    \using \mathbf{z} &= \begin{cases}
      \left\{ (s \mapsto g) \ \where \se_4(s) \concat \se_8(g) = \memory_{o+12i\dots+12} \mid i \in \N_n \right\} &\when \mathbb{N}_{o \dots+ 12n} \subseteq \readable{\memory} \\
      \error &\otherwise
    \end{cases} \\
  ...
  $\begin{aligned}
    ...
    \using \mathbf{q} &= \begin{cases}
      \left[\memory_{o + 32i \dots+ 32} \mid i \orderedin \N_\mathsf{Q}\right] &\when \mathbb{N}_{o \dots+ 32\mathsf{Q}} \subseteq \readable{\memory} \\
      \error &\otherwise
    \end{cases} \\
  ...
  $\begin{aligned}
    \using o &= \registers_7 \\
    \using \mathbf{v} &= \begin{cases}
      \left[\memory_{o + 336i \dots+ 336} \mid i \orderedin \valindex\right] &\when \mathbb{N}_{o \dots+ 336\valcount} \subseteq \readable{\memory} \\
      \error &\otherwise
    \end{cases} \\
  ...
  $\begin{aligned}
    ...
    \using c &= \begin{cases}
      ...
      \error &\otherwise
    \end{cases}\\
    \using \mathbf{a} \in \serviceaccount \cup \{\error\} &= \begin{cases}
  ...
  $\begin{aligned}
    ...
    \using h &= \begin{cases}
      \memory_{o\dots+32} &\when \mathbb{N}_{o \dots+ 32} \subseteq \readable{\memory} \\
      \error &\otherwise
    \end{cases} \\
  ...
  $\begin{aligned}
    ...
    \using \mathbf{a} &= \begin{cases}
      ...
      \quad \left.
        \begin{aligned}
          \Fkeys{\mathbf{a}_\mathbf{l}} &= \Fkeys{(\mathbf{x}_\mathbf{s})_\mathbf{l}} \setminus \{\tup{h, z}\}\ ,\\[2pt]
          \Fkeys{\mathbf{a}_\mathbf{p}} &= \Fkeys{(\mathbf{x}_\mathbf{s})_\mathbf{p}} \setminus \{h\}
        \end{aligned}
