...
\begin{align}
  &\Psi_R \colon \left\{\begin{aligned}
    ...
    (i, p, \mathbf{o}, \overline{\mathbf{i}}, \segoff) &\mapsto \begin{cases}
      ...
      (\token{BIG}, []) &\otherwhen |\Lambda(\delta[w_s], (p_\mathbf{x})_t, w_\wi¬codehash)| > \mathsf{W}_C \\
      &\otherwise: \\
      ...
      &\quad\also (g, \mathbf{r}, (\mathbf{m}, \mathbf{e})) = \Psi_M(\mathbf{c}, 0, w_g, a, F, (\emptyset, []))\ \colon\\
...
\begin{longtable}{p{4cm} p{12cm}}
  ...
  $\begin{aligned}
    ...
    \using \mathbf{x} &= \begin{cases}
      \mathcal{P}_{\mathsf{W}_G}(\memr_{p\dots+z}) &\when (\N_{p\dots+z} \bmod 2^{32}) \subseteq \mathbb{V}_\memory\\
      \error &\otherwise
    \end{cases}\\
  ...
  $\begin{aligned}
    ...
    (\execst', \registers'_7, \mem') &\equiv \begin{cases}
      (\panic, \registers_7, \mem) &\when (\N_{o\dots+z} \bmod 2^{32}) \not\subseteq \mathbb{V}^*_\memory \\
  ...
  $\begin{aligned}
    ...
    (\execst', \registers'_7, \mathbf{m}') &\equiv \begin{cases}
      (\panic, \registers_7, \mathbf{m}) &\when (\N_{s\dots+z} \bmod 2^{32}) \not\subseteq \mathbb{V}_\memory \\
