...
\begin{longtable}{p{5cm} p{11cm}}
  ...
  $\begin{aligned}
    ...
    \end{cases} \\
    \using g_I &= \begin{cases}
      0 &\when g_R = \error \vee n \not\in \mathbf{m} \\
      \min(g_R, \gascounter - \CgasK) &\otherwise
    \end{cases} \\
    \using \tup{c, i', g_I', \pg¬gaschargedflag', \mathbf{w}', \mathbf{u}'} &= \Psi(\mathbf{m}\subb{n}_\pg¬code, \mathbf{m}\subb{n}_\pg¬pc, g_I, \mathbf{m}\subb{n}_{\pg¬gaschargedflag}, \mathbf{w}, \mathbf{m}\subb{n}_\pg¬ram)\\
    \using \mem^* &= \mem \exc \mem^*\subrange{o}{112} = \encode[8]{g_I'} \concat \encode[8]{\mathbf{w}'}\\
    ...
    \tup{\execst', \registers'_7, \registers'_8, \mem', \mathbf{m}'} &\equiv \begin{cases}
      \tup{\panic, \registers_7, \registers_8, \mem, \mathbf{m}} &\when g_R = \error \\
