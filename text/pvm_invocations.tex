...
\begin{align}
  ...
  F(n, \theta, \varrho, \mu, (\mathbf{x}, \mathbf{y})) &\equiv \begin{cases}
    G(\Omega_R(\theta, \varrho, \mu, \mathbf{s}, \mathbf{x}_s, \mathbf{d}), (\mathbf{x}, \mathbf{y})) &\when n = \mathtt{read} \\
    G(\Omega_W(\theta, \varrho, \mu, \mathbf{s}), (\mathbf{x}, \mathbf{y})) &\when n = \mathtt{write} \\
    G(\Omega_L(\theta, \varrho, \mu, \mathbf{s}, \mathbf{x}_s, \mathbf{d}), (\mathbf{x}, \mathbf{y})) &\when n = \mathtt{lookup} \\
    G(\Omega_G(\theta, \varrho, \mu), (\mathbf{x}, \mathbf{y})) &\when n = \mathtt{gas} \\
    G(\Omega_I(\theta, \varrho, \mu, \mathbf{s}, \mathbf{x}_s, \mathbf{d}), (\mathbf{x}, \mathbf{y})) &\when n = \mathtt{info} \\
    ...
    \Omega_U(\theta, \varrho, \mu, (\mathbf{x}, \mathbf{y}), \mathbf{x}_s) &\when n = \mathtt{upgrade} \\
    \Omega_T(\theta, \varrho, \mu, (\mathbf{x}, \mathbf{y}), \mathbf{x}_s, \mathbf{d}) &\when n = \mathtt{transfer} \\
    \Omega_Q(\theta, \varrho, \mu, (\mathbf{x}, \mathbf{y}), \mathbf{x}_s) &\when n = \mathtt{quit} \\
  ...
  \end{cases} \\
  G((\theta, \varrho, \mu, \mathbf{s}), (\mathbf{x}, \mathbf{y})) &\equiv (\theta, \varrho, \mu, (\mathbf{x}^*, \mathbf{y}))\ \where \mathbf{x}^* = \mathbf{x} \exc (\mathbf{x}^*_\mathbf{u})_\mathbf{d}[(\mathbf{x}^*_\mathbf{u})_s] = \mathbf{s} \\
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  \endhead
  \makecell*[l]{
  $\Omega_G(\theta, \varrho, \dots)$ \\
  \texttt{gas} = 0 \\
  $g = 10$} &
  $\begin{aligned}
    \varrho'_0 &\equiv \theta' \bmod 2^{32} \\
    \varrho'_1 &\equiv \left\lfloor \theta' \div 2^{32}\right\rfloor
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_L(\theta, \varrho, \mu, \mathbf{s}, s, \mathbf{d})$ \\
  \texttt{lookup} = 1 \\
  $g = 10$} &
  $\begin{aligned}
    \using \mathbf{a} &= \begin{cases} \mathbf{s} &\when \varrho_0 \in \{ s, 2^{32} - 1 \} \\ \mathbf{d}[\varrho_0] &\otherwise \end{cases} \\
  ...
  $\begin{aligned}
    \using \mathbf{a} &= \begin{cases}
      \mathbf{s} &\when \varrho_0 \in \{ s, 2^{32} - 1 \} \\
      \mathbf{d}[\varrho_0] &\otherwhen \varrho_0 \in \keys{\mathbf{d}} \\
      \none &\otherwise
    \end{cases} \\
  ...
  $\begin{aligned}
    ...
    \end{cases} \\
    (\varrho'_0, \mathbf{s}') &\equiv \begin{cases}
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_I(\theta, \varrho, \mu, \mathbf{s}, s, \mathbf{d})$ \\
  \texttt{info} = 4 \\
  $g = 10$} &
  $\begin{aligned}
    \using \mathbf{t} &= \begin{cases} \mathbf{s} &\when \varrho_0 \in \{s, 2^{32} - 1\} \\ (\mathbf{d} \cup \mathbf{x}_\mathbf{n})[\varrho_0] &\otherwise \end{cases} \\
    \using o &= \varrho_1 \\
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  \endhead
  \makecell*[l]{
  $\Omega_E(\theta, \varrho, \mu, (\mathbf{x}, \mathbf{y}))$ \\
  \texttt{empower} = 5 \\
  $g = 10$}&
  $\begin{aligned}
    \using [m, a, v, o, n] &= \varrho_{0 \dots 5} \\
    \using \mathbf{g} &= \begin{cases}
      ...
      \error &\otherwise
    \end{cases} \\
    (\varrho'_0, \mathbf{x}'_p) &= \begin{cases}
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_A(\theta, \varrho, \mu, (\mathbf{x}, \mathbf{y}))$ \\
  \texttt{assign} = 6 \\
  $g = 10$} &
  $\begin{aligned}
    \using o &= \varrho_1 \\
    \using \mathbf{c} &= \begin{cases}
      ...
      \error &\otherwise
    \end{cases} \\
    (\varrho'_0, \mathbf{x}') &= \begin{cases}
      (\mathtt{OK}, \mathbf{x} \exc \mathbf{x}'_\mathbf{c}[\varrho_0] = \mathbf{c}) &\when \varrho_0 < \mathsf{C} \wedge \mathbf{c} \ne \error \\
  ...
  $\begin{aligned}
    ...
    (\varrho'_0, \mathbf{x}'_i, \mathbf{x}'_\mathbf{n}, (\mathbf{x}'_\mathbf{s})_b) &\equiv \begin{cases}
  ...
  $\begin{aligned}
    ...
    \using g &= 2^{32}\cdot g_h + g_l \\
    \using m &= 2^{32}\cdot m_h + m_l \\
    (\varrho'_0, \mathbf{x}'[s]_c, \mathbf{x}'[s]_g, \mathbf{x}'[s]_m) &\equiv \begin{cases}
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_T(\theta, \varrho, \mu, (\mathbf{x}, \mathbf{y}), s, \delta)$ \\
  \texttt{transfer} = 11 \\
  $g = 10 + \varrho_1 + 2^{32}\cdot\varrho_2 $} &
  $\begin{aligned}
    \using (d, a_l, a_h, g_l, g_h, o) &= \varrho_{0..6},  \\
    ...
    (\varrho'_0, \mathbf{x}'_\mathbf{t}, (\mathbf{x}'_\mathbf{s})_b) &\equiv \begin{cases}
      ...
      (\mathtt{HIGH}, \mathbf{x}_\mathbf{t}, (\mathbf{x}_\mathbf{s})_b) &\otherwhen \theta < g \\
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_Q(\theta, \varrho, \mu, (\mathbf{x}, \mathbf{y}), s)$ \\
  \texttt{quit} = 12 \\
  $g = 10 + \varrho_1 + 2^{32}\cdot\varrho_2 $} &
  $\begin{aligned}
    \using [d, o] &= \varrho_{0,1} \\
    ...
    \using \mathbf{t} \in \mathbb{T} \cup \{\error,\none\} &= \begin{cases}
      ...
      \error &\otherwise
    \end{cases} \\
    (\varrho'_0, \mathbf{x}'_\mathbf{s}, \mathbf{x}'_\mathbf{t}) &\equiv \begin{cases}
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_S(\theta, \varrho, \mu, (\mathbf{x}, \mathbf{y}))$ \\
  \texttt{solicit} = 13 \\
  $g = 10$} &
  $\begin{aligned}
    ...
    \using \mathbf{a} &= \begin{cases}
      ...
      \error &\otherwise\\
    \end{cases} \\
    (\varrho'_0, \mathbf{x}'_\mathbf{s}) &\equiv \begin{cases}
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_Y(\theta, \varrho, \mu, (\mathbf{m}, \mathbf{e}), \mathbf{i})$ \\
  \texttt{import} = 16 \\
  $g = 10$} &
  $\begin{aligned}
    \using \mathbf{v} &= \begin{cases}
      \mathbf{i}_{\varrho_0} &\when \varrho_0 < |\mathbf{i}| \\
      \none &\otherwise
    \end{cases} \\
    \using o &= \varrho_1 \\
    \using l &= \min(\varrho_2, \mathsf{W}_E\mathsf{W}_S) \\
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_Z(\theta, \varrho, \mu, (\mathbf{m}, \mathbf{e}), \segoff)$ \\
  \texttt{export} = 17 \\
  $g = 10$} &
  $\begin{aligned}
    \using p &= \varrho_0 \\
    \using z &= \min(\varrho_1, \mathsf{W}_E\mathsf{W}_S) \\
    \using \mathbf{x} &= \begin{cases}
      \mathcal{P}_{\mathsf{W}_E\mathsf{W}_S}(\mem_{p\dots+z}) &\when \N_{p\dots+z} \in \mathbb{V}_\mu\\
      \error &\otherwise
    \end{cases}\\
    (\varrho'_0, \mathbf{e}') &\equiv \begin{cases}
  ...
  $\begin{aligned}
    ...
    \end{cases}\\
    (\varrho'_0, \mem') &\equiv \begin{cases}
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_O(\theta, \varrho, \mu, (\mathbf{m}, \mathbf{e}))$ \\
  \texttt{poke} = 20 \\
  $g = 10$} &
  $\begin{aligned}
    \using [n, a, b, l] &= \varrho_{0 \dots 4} \\
