...
\subsection{Gas cost calculation}\label{sec:gascostcalculation}
...
\begin{equation}\label{eq:fnmemgas}
  \fnmemgas(L, \ell) \equiv \floor{\frac{L \cdot \ell + 1024}{1024}}.
\end{equation}
With base cost $c$, total gas is $c + \fnmemgas(L, \ell)$. The host-call tables in the following subsections use this formula for all memory-sized terms.
...
Memory-sized gas uses $\fnmemgas$ (see \ref{sec:gascostcalculation}).
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  $\begin{aligned}
    ...
    \using g &= \CgasGeminiconst + \max(0, \registers_7 - h) \cdot \CgasGeminilinear \\
  ...
  $\begin{aligned}
    ...
    \end{cases} \\
    \using g &= \begin{cases}
      \CgasYprotoacc &\when \registers_{10} = 0 \\
      \CgasYentropy &\when \registers_{10} = 1 \\
      \CgasYauthtraceconst + \fnmemgas(\CgasYauthtracelinear, \len{\mathbf{v}}) &\when \registers_{10} = 2 \\
      \CgasYworkpackconst + \fnmemgas(\CgasYworkpacklinear, \len{\mathbf{v}}) &\when \registers_{10} \in \set{3, 4, 7} \\
      \CgasYanyimport &\when \registers_{10} = 5 \\
      \CgasYourimport &\when \registers_{10} = 6 \\
      \CgasYauthconfig &\when \registers_{10} = 8 \\
      \CgasYauthtoken &\when \registers_{10} = 9 \\
      \CgasYrefinectx &\when \registers_{10} = 10 \\
      \CgasYitemssummary &\when \registers_{10} = 11 \\
      \CgasYanyitemsummary &\when \registers_{10} = 12 \\
      \CgasYanypayloadconst + \fnmemgas(\CgasYanypayloadlinear, \len{\mathbf{v}}) &\when \registers_{10} = 13 \\
      \CgasYaccumitemsconst + \fnmemgas(\CgasYaccumitemslinear, \len{\mathbf{v}}) &\when \registers_{10} = 14 \\
      \CgasYanyaccumconst + \fnmemgas(\CgasYanyaccumlinear, \len{\mathbf{v}}) &\when \registers_{10} = 15 \\
      \CgasYprotoref &\otherwise
    \end{cases} \\
    \using o &= \registers_7 \\
  ...
  $\begin{aligned}
    ...
    \end{cases} \\
    \using g &= \begin{cases} \CgasLconst + \fnmemgas(\CgasLlinear, \len{\mathbf{v}}) &\when \mathbf{v} \ne \none \\ \CgasLconst &\otherwise \end{cases} \\
  ...
  $\begin{aligned}
    ...
    \using \mathbf{v} &= \begin{cases}
      ...
      \none &\otherwise
    \end{cases} \\
    \using g &= \begin{cases} \CgasRconst + \fnmemgas(\CgasRkeylinear, k_Z)  + \fnmemgas(\CgasRvallinear, \len{\mathbf{v}}) &\when \mathbf{v} \ne \none \\ \CgasRconst + \fnmemgas(\CgasRkeylinear, k_Z) &\otherwise \end{cases} \\
  ...
  $\begin{aligned}
    ...
    \using g &= \CgasWconst + \fnmemgas(\CgasWkeylinear, k_Z) + \fnmemgas(\CgasWvallinear, v_Z) \\
...
\begin{longtable}{p{4cm} p{12cm}}
  ...
  $\begin{aligned}
    ...
    \end{cases} \\
    \using g &= \begin{cases} \CgasHconst + \fnmemgas(\CgasHlinear, \len{\mathbf{v}}) &\when \mathbf{v} \ne \none \\ \CgasHconst &\otherwise \end{cases} \\
    \using f &= \min(\registers_{10}, \len{\mathbf{v}}) \\
    \using l &= \min(\registers_{11}, \len{\mathbf{v}} - f) \\
    \tup{\execst', \registers'_7, \memory'\subrange{o}{l}} &\equiv \begin{cases}
      \tup{\panic, \registers_7, \memory\subrange{o}{l}} &\when \mathbf{v} = \error \vee \Nrange{o}{l} \not\subseteq \writable{\memory}\\
  ...
  $\begin{aligned}
    ...
    \using \mathbf{p} &= \begin{cases}
      ...
      \error &\otherwise
    \end{cases} \\
    \using g &= \CgasMconst + \fnmemgas(\CgasMlinear, \len{\mathbf{p}}) \\
  ...
  $\begin{aligned}
    ...
    \using g &= \CgasPconst + \fnmemgas(\CgasPlinear, z) \\
  ...
  $\begin{aligned}
    ...
    \using g &= \CgasOconst + \fnmemgas(\CgasOlinear, z) \\
    \tup{\execst', \registers'_7, \mathbf{m}'} &\equiv \begin{cases}
  ...
  $\begin{aligned}
    ...
    \using g &= \begin{cases} \CgasZfreeconst + c \cdot \CgasZfreelinear &\when r = 0 \\ \CgasZallocconst + c \cdot \CgasZalloclinear &\when r \in \set{1, 2} \\ \CgasZsetmodeconst + c \cdot \CgasZsetmodelinear &\when r \in \set{3, 4} \end{cases} \\
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  $\begin{aligned}
    ...
    \using g &= \CgasBconst + n \cdot \CgasBlinear \\
    \using \mathbf{a} &= \begin{cases}
  ...
  $\begin{aligned}
    ...
    \using g &= \CgasAriesconst + \fnmemgas(\CgasArieslinear, z) \\
    \using \mathbf{d} &= (\imX_\im¬state)_\ps¬accounts\\
    \using s &= \begin{cases}
