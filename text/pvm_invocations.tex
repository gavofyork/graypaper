...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  $\begin{aligned}
    ...
    \using \mathbf{x} &= \begin{cases}
      \mathcal{P}_{\mathsf{W}_E\mathsf{W}_S}(\memr_{p\dots+z}) &\when \N_{p\dots+z} \in \mathbb{V}_\memory\\
      \error &\otherwise
    \end{cases}\\
  ...
  $\begin{aligned}
    ...
    (\registers'_7, \mem') &\equiv \begin{cases}
      ...
      (\mathtt{OK}, \mem') \ \where \mem' = \mem \exc \memwr_{a\dots+l} = \mathbf{s} &\otherwise
    \end{cases} \\
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  ...
  $g = 10$} &
  $\begin{aligned}
    \using [n, b, l] &= \registers_{7 \dots 10} \\
  ...
  $\begin{aligned}
    ...
    \end{cases} \\
    \using \mathbf{s} &= \begin{cases}
      \memr_{a\dots+l} &\when \N_{a\dots+l} \in \mathbb{V}_\mathbf{u}\\
      \error &\otherwise
    \end{cases}\\
    \using \mathbf{u}' &= \mathbf{u} \exc \begin{cases}
      (\mathbf{u}'_\mathbf{V})_{b\dots+l} = \mathbf{s} \\
      (\mathbf{u}'_\mathbf{A})_{\floor{\nicefrac{b}{\mathsf{Z}_P}}\dots\floor{\nicefrac{b+l-1}{\mathsf{Z}_P}} + 1} = [\mathrm{W}, \mathrm{W}, ...]
    \end{cases}\\
    (\registers'_7, \mathbf{m}') &\equiv \begin{cases}
