...
\begin{longtable}{p{4cm} p{12cm}}
  ...
  $\begin{aligned}
    \using \mathbf{p} &= \begin{cases}
      \memory\subrange{\registers_8}{\registers_9} &\when \registers_7 = 0 \wedge \Nrange{\registers_8}{\registers_9} \subseteq \readable{\memory} \\
      \error &\otherwhen \registers_7 = 0 \vee (\registers_7 = 1 \wedge \Nrange{\registers_8}{32} \not\subseteq \readable{\memory}) \\
      \mathbf{h}\subrange{f}{l} &\otherwhen \registers_7 = 1 \wedge \mathbf{a} \ne \none \wedge \mathbf{h} \ne \none \\
      \multicolumn{2}{l}{\where \mathbf{h} = \histlookup(\mathbf{a}, t, \memory\subrange{\registers_8}{32}),\ f = \min(\registers_{10}, \len{\mathbf{h}}),\ l = \min(\registers_9, \len{\mathbf{h}} - f)} \\
      \none &\otherwise
    \end{cases} \\
    \using \mathbf{a} &= \begin{cases}
      \mathbf{d}\subb{s} &\when \registers_{11} = 2^{64} - 1 \wedge s \in \keys{\mathbf{d}} \\
      \mathbf{d}[\registers_{11}] &\when \registers_{11} \in \keys{\mathbf{d}} \\
      \none &\otherwise
    \end{cases} \\
