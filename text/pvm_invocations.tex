...
\begin{longtable}{p{4cm} p{12cm}}
  ...
  $\begin{aligned}
    ...
    \end{cases} \\
    \using \mathbf{a} &= \begin{cases}
      \mathbf{d}\subb{s} &\when i = 2^{64} - 1 \wedge s \in \keys{\mathbf{d}} \\
      \mathbf{d}[i] &\when i \in \keys{\mathbf{d}} \\
      \none &\otherwise
    \end{cases} \\
    ...
    \tup{\execst', \registers'_7, \pvm¬module¬map'} &\equiv \begin{cases}
      ...
      \tup{\continue, \mathtt{NONE}, \pvm¬module¬map} &\otherwhen \mathbf{p} = \none \\
      \tup{\continue, \mathtt{HUH}, \pvm¬module¬map} &\otherwhen \deblob(\mathbf{p}, 0) = \error \\
      \tup{\continue, k, \pvm¬module¬map \cup \set{\kv{k}{\mathbf{p}}} } &\otherwise \\
    \end{cases} \\
    g &\equiv \begin{cases}
      \CgasGammamem + \fnmemgas(\CgasGammamemlinear, l) &\when m = 0 \\
      \CgasGammadb + \fnmemgas(\CgasGammadblinear, l) &\when m = 1 \\
      \CgasGammamem &\otherwise
    \end{cases}
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
...
\begin{longtable}{p{3cm} p{13cm}}
  ...
  $\begin{aligned}
    ...
    \end{cases} \\
    \using \mathbf{a} &= \begin{cases}
      \imX_\im¬self\,,\ \exc \mathbf{k} \not\in \keys{\mathbf{a}_\sa¬storage} & \when v_Z = 0 \\
      \imX_\im¬self\,,\ \exc \mathbf{a}_\sa¬storage\subb{\mathbf{k}} = \memory\subrange{v_O}{v_Z} &\otherwhen \Nrange{v_O}{v_Z} \subseteq \readable{\memory} \\
      \error &\otherwise
    \end{cases} \\
    \using l &= \begin{cases}
      \len{(\imX_\im¬self)_\sa¬storage\subb{\mathbf{k}}} &\when \mathbf{k} \in \keys{(\imX_\im¬self)_\sa¬storage} \\
      \mathtt{NONE} &\otherwise
    \end{cases} \\
    \tup{\execst', \registers'_7, \imX_\im¬self'} &\equiv \begin{cases}
      \tup{\panic, \registers_7, \imX_\im¬self} &\when \mathbf{k} = \error \vee \mathbf{a} = \error\\
      \tup{\continue, \mathtt{FULL}, \imX_\im¬self} &\otherwhen \mathbf{a}_\sa¬minbalance > \mathbf{a}_\sa¬balance \\
  ...
  $\begin{aligned}
    ...
    \tup{\execst', \registers'_7, (\imX'_\im¬self)_\sa¬codehash, (\imX'_\im¬self)_\sa¬minaccgas, (\imX'_\im¬self)_\sa¬minmemogas} &\equiv \begin{cases}
      ...
      \tup{\continue, \mathtt{OK}, c, a, m} &\otherwise \\
    \end{cases} \\
    g &\equiv \CgasU
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
