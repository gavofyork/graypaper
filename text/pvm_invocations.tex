...
\begin{equation}
  \pvm \equiv \tuple{\isa{\pvm¬code}{\blob}, \isa{\pvm¬ram}{\ram}, \isa{\pvm¬pc}{\pvmreg}}
\end{equation}
...
\begin{align}
  &\Psi_R \colon \abracegroup{
    ...
    \tup{i, p, \mathbf{r}, \overline{\mathbf{i}}, \segoff} &\mapsto \begin{cases}
      ...
      &\otherwise: \\
      &\quad\using a = \se(i, w_\wi¬service, \var{w_\wi¬payload}, \blake(p))\;,\ \encode{\var{\mathbf{z}}, \mathbf{c}} = \histlookup(\accounts\subb{w_\wi¬service}, (p_\wp¬context)_\wc¬lookupanchortime, w_\wi¬codehash)\\
...
\begin{align}
  \implications &\equiv \tuple{
    ...
    \isa{\im¬xfers}{\sequence{\mathbb{T}}},
...
\newcommand*{\local¬basestate}{\mathbf{u}}
\begin{align}
  \Psi_A& \colon\abracegroup{
    ...
    &\to \tuple{
      \partialstate, \defxfers, \optional{\hash}, \gas, \sequence{\tuple{\serviceid, \blob}}
    }
    \\
    \tup{\local¬basestate, t, s, g, \mathbf{o}} &\mapsto \begin{cases}
      \tup{\local¬basestate, \sq{}, \none, 0, \sq{}}
  ...
  F \in \Omega\ang{\implicationspair} &\colon \tup{n, \gascounter, \registers, \memory, \imXY} \mapsto \begin{cases}
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  $\begin{aligned}
    ...
    \using \mathbf{a} &= \begin{cases}
      ...
      \mathbf{s}\,,\ \exc \mathbf{a}_\sa¬storage[\mathbf{k}] = \memory\subrange{v_O}{v_Z} &\otherwhen \Nrange{v_O}{v_Z} \subseteq \readable{\memory} \\
      \error &\otherwise
    \end{cases} \\
    \using l &= \begin{cases}
      \len{\mathbf{s}_\sa¬storage[k]} &\when \mathbf{k} \in \keys{\mathbf{s}_\sa¬storage} \\
  ...
  $\begin{aligned}
    ...
    \using \mathbf{v} &= \begin{cases}
      \se(
        ...
        \fixedencode{8}{\mathbf{a}_\sa¬balance, \mathbf{a}_\sa¬minbalance, \mathbf{a}_\sa¬minaccgas, \mathbf{a}_\sa¬minmemogas, \mathbf{a}_\sa¬octets},
        \fixedencode{4}{\mathbf{a}_\sa¬items},
        \fixedencode{8}{\mathbf{a}_\sa¬gratis},
        \fixedencode{4}{\mathbf{a}_\sa¬created, \mathbf{a}_\sa¬lastacc, \mathbf{a}_\sa¬parent}
      ) &\when \mathbf{a} \ne \none \\
      \none &\otherwise
    \end{cases} \\
    \using f &= \min(\registers_{11}, \len{\mathbf{v}}) \\
...
\begin{longtable}{p{4cm} p{12cm}}
  ...
  $\begin{aligned}
    ...
    \tup{\execst', \registers'_7, \mem'} &\equiv \begin{cases}
      ...
      \tup{\continue, \mathtt{OOB}, \mem} &\otherwhen \Nrange{s}{z} \not\subseteq \readable{\mathbf{m}[n]_\pvm¬ram} \\
      \tup{\continue, \mathtt{OK}, \mem'} &\otherwise \\
      \multicolumn{2}{l}{\where \mem' = \mem \exc \mem\subrange{o}{z} = (\mathbf{m}[n]_\pvm¬ram)\subrange{s}{z}}
    \end{cases} \\
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  ...
  $\begin{aligned}
    ...
    \tup{\execst', \registers'_7, \mathbf{m}'} &\equiv \begin{cases}
      ...
      \tup{\continue, \mathtt{OOB}, \mathbf{m}} &\otherwhen \Nrange{o}{z} \not\subseteq \writable{\mathbf{m}[n]_\pvm¬ram} \\
      \tup{\continue, \mathtt{OK}, \mathbf{m}'}  &\otherwise \\
      \multicolumn{2}{l}{\where \mathbf{m}' = \mathbf{m} \exc (\mathbf{m}'[n]_\pvm¬ram)\subrange{o}{z} = \mem\subrange{s}{z}}
    \end{cases} \\
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  ...
  $\begin{aligned}
    ...
    \using \mathbf{u} &= \begin{cases}
      \mathbf{m}[n]_\pvm¬ram &\when n \in \keys{\mathbf{m}} \\
      \error &\otherwise\\
    \end{cases} \\
    ...
    \tup{\registers'_7, \mathbf{m}'} &\equiv \begin{cases}
      ...
      \tup{\mathtt{OK}, \mathbf{m}'} &\otherwise\,,\ \where \mathbf{m}' = \mathbf{m} \exc \mathbf{m}'[n]_\pvm¬ram = \mathbf{u}' \\
    \end{cases} \\
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  ...
  $\begin{aligned}
    ...
    \end{cases} \\
    \using \tup{c, i', g', \mathbf{w}', \mathbf{u}'} &= \Psi(\mathbf{m}[n]_\pvm¬code, \mathbf{m}[n]_\pvm¬pc, g, \mathbf{w}, \mathbf{m}[n]_\pvm¬ram)\\
    ...
    \using \mathbf{m}^* &= \mathbf{m} \exc \begin{cases}
      \mathbf{m}^*[n]_\pvm¬ram = \mathbf{u}'\\
      \mathbf{m}^*[n]_\pvm¬pc = \begin{cases}
  ...
  $\begin{aligned}
    ...
    \tup{\registers'_7, \mathbf{m}'} &\equiv \begin{cases}
      ...
      \tup{\mathbf{m}[n]_\pvm¬pc, \mathbf{m} \setminus n} &\otherwise \\
    \end{cases} \\
  \end{aligned}$\\
  \bottomrule
\end{longtable}
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  $\begin{aligned}
    ...
    \using \mathbf{q} &= \begin{cases}
      ...
      \error &\otherwise
    \end{cases} \\
    \tup{\execst', \registers'_7, (\imX'_\im¬state)_\ps¬authqueue[c], (\imX'_\im¬state)_\ps¬assigners[c]} &= \begin{cases}
      \tup{\panic, \registers_7, (\imX_\im¬state)_\ps¬authqueue[c], (\imX_\im¬state)_\ps¬assigners[c]} &\when \mathbf{q} = \error \\
      \tup{\continue, \mathtt{CORE}, (\imX_\im¬state)_\ps¬authqueue[c], (\imX_\im¬state)_\ps¬assigners[c]} &\otherwhen c \ge \corecount \\
      \tup{\continue, \mathtt{HUH}, (\imX_\im¬state)_\ps¬authqueue[c], (\imX_\im¬state)_\ps¬assigners[c]} &\otherwhen \imX_\im¬id \ne (\imX_\im¬state)_\ps¬assigners[c]\\
  ...
  $\begin{aligned}
    ...
    \using \mathbf{a} \in \serviceaccount \cup \set{\error} &= \begin{cases}
      \tup{c, \is{\mathbf{s}}{\emset}, \is{\mathbf{l}}{\set{\kv{\tup{c, l}}{\sq{}}}}, \is{b}{\mathbf{a}_\sa¬minbalance}, g, m, \is{\mathbf{p}}{\emset}, \is{r}{t}, f, \is{a}{0}, \is{p}{\imX_\im¬id}} &\when c \ne \error \\
      \error &\otherwise
    \end{cases} \\
  ...
  $g = 10 + \registers_9 $} &
  $\begin{aligned}
    \using [\dx¬dest, \dx¬amount, l, o] &= \registers\subrange{7}{4},  \\
    ...
    \using \mathbf{t} \in \mathbb{T} \cup \set{\error} &= \begin{cases}
      \tup{
        ...
        \dx¬dest,
        \dx¬amount,
        \is{\dx¬memo}{\memory\subrange{o}{\mathsf{W}_T}},
        \is{\dx¬gas}{l}
      } &\when \Nrange{o}{\mathsf{W}_T} \subseteq \readable{\memory} \\
      \error &\otherwise
    \end{cases} \\
    ...
    (\execst', \registers'_7, \imX'_\im¬xfers, (\imX'_\im¬self)_\sa¬balance) &\equiv \begin{cases}
      (\panic, \registers_7, \imX_\im¬xfers, (\imX_\im¬self)_\sa¬balance) &\when \mathbf{t} = \error \\
      (\continue, \mathtt{WHO}, \imX_\im¬xfers, (\imX_\im¬self)_\sa¬balance) &\otherwhen \dx¬dest \not \in \keys{\mathbf{d}} \\
      (\continue, \mathtt{LOW}, \imX_\im¬xfers, (\imX_\im¬self)_\sa¬balance) &\otherwhen l < \mathbf{d}[\dx¬dest]_\sa¬minmemogas \\
      (\continue, \mathtt{CASH}, \imX_\im¬xfers, (\imX_\im¬self)_\sa¬balance) &\otherwhen \dx¬amount < (\imX_\im¬self)_\sa¬minbalance \\
  ...
  $\begin{aligned}
    ...
    \using \mathbf{s}' &= ((\imX_\im¬state)_\ps¬accounts)[\imX_\im¬id] \exc \mathbf{s}'_\sa¬balance = (\imX_\im¬self)_\sa¬balance + \mathbf{d}_\sa¬balance \\
    (\execst', \registers'_7, (\imX'_\im¬state)_\ps¬accounts) &\equiv \begin{cases}
      ...
      (\continue, \mathtt{WHO}, (\imX_\im¬state)_\ps¬accounts) &\otherwhen \mathbf{d} = \error \vee \mathbf{d}_\sa¬codehash \ne \se_{32}(\imX_\im¬id) \\
      (\continue, \mathtt{HUH}, (\imX_\im¬state)_\ps¬accounts) &\otherwhen \mathbf{d}_\sa¬items \ne 2 \vee \tup{h, l} \not\in \mathbf{d}_\sa¬requests \\
      (\continue, \mathtt{OK}, (\imX_\im¬state)_\ps¬accounts \setminus \set{d} \cup \set{ \kv{\imX_\im¬id}{\mathbf{s}'} }) &\otherwhen \mathbf{d}_\sa¬requests[h, l] = [x, y], y < t - \mathsf{D} \\
      (\continue, \mathtt{HUH}, (\imX_\im¬state)_\ps¬accounts) &\otherwise \\
    \end{cases} \\
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  ...
  $\begin{aligned}
    ...
    \end{cases} \\
    \using \mathbf{a} &= \begin{cases}
      (\imX_\im¬self)_\sa¬requests[h, z] &\when \tup{h, z} \in \keys{(\imX_\im¬self)_\sa¬requests}\\
      \error &\otherwise\\
    \end{cases} \\
  ...
  $\begin{aligned}
    ...
    \using \mathbf{a} &= \begin{cases}
      ...
      \quad \left.
        ...
        \end{aligned}
      \ \right\} &\when (\imX_\im¬self)_\sa¬requests[h, z] \in \set{\sq{}, \sq{x, y}},\ y < t - \mathsf{D} \\
      \quad \mathbf{a}_\sa¬requests[h, z] = [x, t] &\when (\imX_\im¬self)_\sa¬requests[h, z] = [x] \\
      \quad \mathbf{a}_\sa¬requests[h, z] = [w, t] &\when (\imX_\im¬self)_\sa¬requests[h, z] = [x, y, w],\ y < t - \mathsf{D} \\
      \error &\otherwise\\
    \end{cases} \\
    \tup{\execst', \registers'_7, \imX'_\im¬self} &\equiv \begin{cases}
      \tup{\panic, \registers_7, \imX_\im¬self} &\when h = \error \\
