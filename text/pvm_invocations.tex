...
\begin{align}
  ...
  \label{eq:isauthorizedmutator}F \in \Omega\ang{\emset} &\colon
    (n, \gascounter, \registers, \memory) \mapsto \begin{cases}
      ...
      (\continue, \gascounter - 10, \sq{\registers_0, \dots, \registers_6, \mathtt{WHAT}, \registers_8, \dots}, \memory) &\otherwise
    \end{cases}
\end{align}
...
\begin{align}
  &\Psi_R \colon \abracegroup{
    \tuple{\N, \workpackage, \blob, \sequence{\sequence{\segment}}, \N} &\to (\blob \cup \workerror, \sequence{\segment}, \gas) \\
...
\begin{align}
  ...
  F \in \Omega\ang{\tuple{\mathbf{X}, \mathbf{X}}} &\colon (n, \gascounter, \registers, \memory, (\mathbf{x}, \mathbf{y})) \mapsto \begin{cases}
  ...
  C&\colon\abracegroup{
    \tuple{\gas, \blob \cup \set{\oog, \panic}, \tuple{\mathbf{X}, \mathbf{X}}} &\to (\partialstate, \defxfers, \optional{\hash}, \gas, \sequence{\tuple{\serviceid, \blob}}) \\
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  $\begin{aligned}
    ...
    (\execst', \registers'_7, \memory'\subrange{o}{l}) &\equiv \begin{cases}
      (\panic, \registers_7, \memory\subrange{o}{l}) &\when \Nrange{o}{l} \not\subseteq \writable{\memory} \\
...
\begin{longtable}{p{4cm} p{12cm}}
  ...
  $\begin{aligned}
    ...
    (\execst', \registers'_7, \mathbf{m}') &\equiv \begin{cases}
      (\panic, \registers_7, \mathbf{m}) &\when \Nrange{s}{z} \not\subseteq \mathbb{V}_\memory \\
      (\continue, \mathtt{WHO}, \mathbf{m}) &\otherwhen n \not\in \keys{\mathbf{m}} \\
      (\continue, \mathtt{OOB}, \mathbf{m}) &\otherwhen \Nrange{o}{z} \not\subseteq \writable{\mathbf{m}[n]_\mathbf{u}} \\
  ...
  $\begin{aligned}
    ...
    (\registers'_7, \mathbf{m}') &\equiv \begin{cases}
      ...
      (\mathtt{HUH}, \mathbf{m}) &\otherwhen r > 2 \wedge (\mathbf{u}_\ram¬access)\subrange{p}{c} \ni \none \\
  ...
  $\begin{aligned}
    ...
    \using (g, \mathbf{w}) &= \begin{cases}
      (g, \mathbf{w}): \fixedencode{8}{g} \concat \fixedencode{8}{\mathbf{w}} = \mem\subrange{o}{112} &\when \Nrange{o}{112} \subseteq \writable{\mem} \\
      %(\decode{8}{\memr\subrange{o}{8}}, \sq{\build{\decode{4}{\memr\subrange{o+8+8x}{8}}}{x \orderedin \Nmax{13}}}) &\when \Nrange{o}{60} \subset \writable_\mem} \\
