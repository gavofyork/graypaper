...
\begin{align}
  ...
  &F \in \Omega\ang{\tuple{\dictionary{\N}{\pvm}, \sequence{\segment}}} \colon
  ...
  $\begin{aligned}
    \using \mathbf{v} &= \begin{cases}
      ...
      \multicolumn{2}{l}{\where \mathbf{c} = \encode{
        \begin{aligned}
          &\encode[8]{\mathsf{B}_I},
          \encode[8]{\mathsf{B}_L},
          \encode[8]{\mathsf{B}_S},
          \encode[2]{\corecount},
          \encode[4]{\mathsf{D}},
          \encode[4]{\epochlen},
          \encode[8]{\mathsf{G}_A},\\
          &\encode[8]{\mathsf{G}_I},
          \encode[8]{\mathsf{G}_R},
          \encode[8]{\mathsf{G}_T},
          \encode[2]{\mathsf{H}},
          \encode[2]{\mathsf{I}},
          \encode[2]{\mathsf{J}},
          \encode[2]{\mathsf{K}},\\
          ...
          \encode[2]{\mathsf{U}},\\
          &\encode[2]{\valcount},
          \encode[4]{\mathsf{W}_A},
          \encode[4]{\mathsf{W}_B},
          \encode[4]{\mathsf{W}_C},
          \encode[4]{\mathsf{W}_E},
          \encode[4]{\mathsf{W}_M},\\
          &\encode[4]{\mathsf{W}_P},
          \encode[4]{\mathsf{W}_R},
...
These assume some refine context pair $\tup{\mathbf{m}, \mathbf{e}} \in \tuple{\dictionary{\N}{\pvm}, \sequence{\segment}}$, which are both initially empty. Other than the gas-counter which is explicitly defined, elements of \textsc{pvm} state are each assumed to remain unchanged by the host-call unless explicitly specified.
\begin{align}
  \gascounter' &\equiv \gascounter - g\\
...
\begin{longtable}{p{4cm} p{12cm}}
  ...
  $\begin{aligned}
    ...
    \tup{\execst', \registers'_7, \mem'} &\equiv \begin{cases}
      ...
      \tup{\continue, \mathtt{OOB}, \mem} &\otherwhen \Nrange{s}{z} \not\subseteq \readable{\mathbf{m}\subb{n}_\pvm¬ram} \\
      \tup{\continue, \mathtt{OK}, \mem'} &\otherwise \\
      \multicolumn{2}{l}{\where \mem' = \mem \exc \mem\subrange{o}{z} = (\mathbf{m}\subb{n}_\pvm¬ram)\subrange{s}{z}}
    \end{cases} \\
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  ...
  $\begin{aligned}
    ...
    \tup{\execst', \registers'_7, \mathbf{m}'} &\equiv \begin{cases}
      ...
      \tup{\continue, \mathtt{OOB}, \mathbf{m}} &\otherwhen \Nrange{o}{z} \not\subseteq \writable{\mathbf{m}\subb{n}_\pvm¬ram} \\
      \tup{\continue, \mathtt{OK}, \mathbf{m}'}  &\otherwise \\
      \multicolumn{2}{l}{\where \mathbf{m}' = \mathbf{m} \exc (\mathbf{m}'\subb{n}_\pvm¬ram)\subrange{o}{z} = \mem\subrange{s}{z}}
    \end{cases} \\
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  ...
  $\begin{aligned}
    ...
    \using \mathbf{u} &= \begin{cases}
      \mathbf{m}\subb{n}_\pvm¬ram &\when n \in \keys{\mathbf{m}} \\
      \error &\otherwise\\
    \end{cases} \\
    ...
    \tup{\registers'_7, \mathbf{m}'} &\equiv \begin{cases}
      ...
      \tup{\mathtt{OK}, \mathbf{m}'} &\otherwise\,,\ \where \mathbf{m}' = \mathbf{m} \exc \mathbf{m}'\subb{n}_\pvm¬ram = \mathbf{u}' \\
    \end{cases} \\
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  ...
  $\begin{aligned}
    ...
    \end{cases} \\
    \using \tup{c, i', g', \mathbf{w}', \mathbf{u}'} &= \Psi(\mathbf{m}\subb{n}_\pvm¬code, \mathbf{m}\subb{n}_\pvm¬pc, g, \mathbf{w}, \mathbf{m}\subb{n}_\pvm¬ram)\\
    \using \mem^* &= \mem \exc \mem^*\subrange{o}{112} = \encode[8]{g'} \concat \encode[8]{\mathbf{w}'}\\
    \using \mathbf{m}^* &= \mathbf{m} \exc \begin{cases}
      \mathbf{m}^*\subb{n}_\pvm¬ram = \mathbf{u}'\\
      \mathbf{m}^*\subb{n}_\pvm¬pc = \begin{cases}
  ...
  $\begin{aligned}
    ...
    \tup{\registers'_7, \mathbf{m}'} &\equiv \begin{cases}
      ...
      \tup{\mathbf{m}\subb{n}_\pvm¬pc, \mathbf{m} \setminus n} &\otherwise \\
    \end{cases} \\
  \end{aligned}$\\
  \bottomrule
\end{longtable}
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  $\begin{aligned}
    ...
    \tup{\execst', \registers'_7, (\imX'_\im¬state)_\ps¬authqueue\subb{c}, (\imX'_\im¬state)_\ps¬assigners\subb{c}} &= \begin{cases}
      ...
      \tup{\continue, \mathtt{CORE}, (\imX_\im¬state)_\ps¬authqueue\subb{c}, (\imX_\im¬state)_\ps¬assigners\subb{c}} &\otherwhen c \ge \corecount \\
  ...
  $\begin{aligned}
    ...
    \tup{\execst', \registers'_7, (\imX'_\im¬state)_\ps¬accounts} &\equiv \begin{cases}
      ...
      \tup{\continue, \mathtt{OK}, (\imX_\im¬state)_\ps¬accounts \setminus \set{d} \cup \set{ \kv{\imX_\im¬id}{\mathbf{s}'} }} &\otherwhen \mathbf{d}_\sa¬requests\subb{h, l} = \sq{x, y}, y < t - \mathsf{D} \\
  ...
  $\begin{aligned}
    ...
    \using \mathbf{a} &= \begin{cases}
      ...
      \quad \left.
        ...
        \end{aligned}
      \ \right\} &\when (\imX_\im¬self)_\sa¬requests\subb{h, z} \in \set{\sq{}, \sq{x, y}},\ y < t - \mathsf{D} \\
      \quad \mathbf{a}_\sa¬requests\subb{h, z} = \sq{x, t} &\when (\imX_\im¬self)_\sa¬requests\subb{h, z} = \sq{x} \\
      \quad \mathbf{a}_\sa¬requests\subb{h, z} = \sq{w, t} &\when (\imX_\im¬self)_\sa¬requests\subb{h, z} = \sq{x, y, w},\ y < t - \mathsf{D} \\
      \error &\otherwise\\
    \end{cases} \\
    \tup{\execst', \registers'_7, \imX'_\im¬self} &\equiv \begin{cases}
      \tup{\panic, \registers_7, \imX_\im¬self} &\when h = \error \\
