...
\begin{align}
  ...
  &F \in \Omega\ang{(\dict{\N}{\pvm}, \seq{\Y})} \colon
    (n, \gascounter, \registers, \memory, (\mathbf{m}, \mathbf{e})) \mapsto \begin{cases}
      ...
      \Omega_Y(\gascounter, \registers, \memory, (\mathbf{m}, \mathbf{e}), i, p, \overline{\mathbf{i}}) &\when n = \mathtt{fetch}\\
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  $\begin{aligned}
    \using \mathbf{v} &= \begin{cases}
      \mathbf{i}_{\registers_{10}} &\when \registers_9 = 33 \wedge \registers_{10} < |\mathbf{i}| \\
      \none &\otherwise
    \end{cases} \\
    \using o &= \registers_7 \\
