...
\begin{align}
  ...
  &F \in \contextmutator{\tuple{\dictionary{\N}{\pvm¬instance}, \dictionary{\N}{\pvm¬module}, \sequence{\segment}}} \colon
    (n, \gascounter, \registers, \memory, \tup{\mathbf{m}, \pvm¬module¬map, \mathbf{e}}) \mapsto \begin{cases}
      ...
      \Omega_V(\gascounter, \registers, \memory, \pvm¬module¬map) &\when n = \mathtt{drop}\\
...
\begin{longtable}{p{4cm} p{12cm}}
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_V(\gascounter, \registers, \memory, \pvm¬module¬map)$ \\
  \texttt{drop} = 16 \\
  $g = 10$} &
  $\begin{aligned}
    \using h &= \registers_7 \\
    \tup{\execst', \registers'_7, \pvm¬module¬map'} &\equiv \begin{cases}
      \tup{\continue, \mathtt{WHO}, \pvm¬module¬map} &\when h \not\in \keys{\pvm¬module¬map} \\
      \tup{\continue, \mathtt{OK}, \pvm¬module¬map \setminus \set{h}} &\otherwise \\
    \end{cases} \\
  \end{aligned}$\\
  \bottomrule
\end{longtable}
