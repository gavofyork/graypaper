...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  $\begin{aligned}
    ...
    \using g &= 2^{32}\cdot g_h + g_l \\
    \using m &= 2^{32}\cdot m_h + m_l \\
    (\registers'_0, \mathbf{x}'[\mathbf{x}_s]_c, \mathbf{x}'[\mathbf{x}_s]_g, \mathbf{x}'[\mathbf{x}_s]_m) &\equiv \begin{cases}
      (\mathtt{OK}, c, g, m) &\when c \ne \error\\
      (\mathtt{OOB}, \mathbf{x}[\mathbf{x}_s]_c, \mathbf{x}[\mathbf{x}_s]_g, \mathbf{x}[\mathbf{x}_s]_m) &\otherwise
    \end{cases}
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  ...
  $\begin{aligned}
    ...
    \using \mathbf{t} \in \mathbb{T} \cup \{\error\} &= \begin{cases}
      (\mathbf{x}_s, d, a, m, g): m = \memory_{o\dots+\mathsf{M}} &\when \N_{o\dots+\mathsf{M}} \subset \mathbb{V}_{\memory} \\
      \error &\otherwise
    \end{cases} \\
  ...
  $\begin{aligned}
    ...
    \using \mathbf{t} \in \mathbb{T} \cup \{\error,\none\} &= \begin{cases}
      ...
      (\mathbf{x}_s, d, a, m, g): \se(m) = \memory_{o\dots+\mathsf{M}} &\otherwhen \N_{o\dots+\mathsf{M}} \subset \mathbb{V}_{\memory} \\
      \error &\otherwise
    \end{cases} \\
