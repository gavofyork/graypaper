...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  $\begin{aligned}
    ...
    \using \mathbf{a} \in \mathbb{A} \cup \{\error\} &= \begin{cases}
      ...
      \error &\otherwise
    \end{cases} \\
    \using \mathbf{s} &= \mathbf{x}_\mathbf{d}[\mathbf{x}_s] \exc \mathbf{s}_b = \mathbf{x}_\mathbf{d}[\mathbf{x}_s]_b - \mathbf{a}_t \\
    (\registers'_0, \mathbf{x}'_i, \mathbf{x}'_\mathbf{d}) &\equiv \begin{cases}
      (\mathbf{x}_i, \text{check}(\text{bump}(\mathbf{x}_i)), \mathbf{x}_\mathbf{d} \cup \{ \mathbf{x}_i \mapsto \mathbf{a}, \mathbf{x}_s \mapsto \mathbf{s} \}, b) &\when \mathbf{a} \ne \error \wedge b \ge (\mathbf{x}_\mathbf{s})_t \\
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  $\begin{aligned}
    \using \mathbf{a} &= \begin{cases}
      \mathbf{d}[s] &\when \registers_0 = 2^{32} - 1 \wedge s \in \keys{\mathbf{d}} \\
      \mathbf{d}[\registers_0] &\when \registers_0 \in \keys{\mathbf{d}} \\
      \none &\otherwise
    \end{cases} \\
    \using [h_o, b_o, b_z] &= \registers_{1..4} \\
    \using h &= \begin{cases}
