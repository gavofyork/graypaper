...
\begin{align}
  \Psi_I &\colon \left\{\begin{aligned}
    (\workpackage, \coreindex) &\to \tuple{\blob \cup \mathbb{J}, \gas} \\
    (\wpX, c) &\mapsto \begin{cases}
      \tup{\token{BAD}, 0} &\when \wpX_\mathbf{c} = \none \\
      \tup{\token{BIG}, 0} &\otherwhen \len{\wpX_\mathbf{c}} > \mathsf{W}_A \\
...
\begin{align}
  &\Psi_R \colon \left\{\begin{aligned}
    (\N, \workpackage, \blob, \seq{\seq{\G}}, \N) &\to (\blob \cup \mathbb{J}, \seq{\G}, \gas) \\
...
\begin{align}
  \Psi_A& \colon\left\{\begin{aligned}
    \tuple{
      \partialstate, \timeslot, \N_S, \gas, \seq{\operandtuple}
    }
    &\to
    \tuple{\partialstate, \defxfers, \optional{\hash}, \gas, \seq{\tuple{\N_S, \blob}}} \\
  ...
  C&\colon\left\{\begin{aligned}
    (\gas, \blob \cup \{\oog, \panic\}, (\mathbf{X}, \mathbf{X})) &\to (\partialstate, \defxfers, \optional{\hash}, \gas, \seq{\tuple{\N_S, \blob}}) \\
...
\begin{align}
  \Psi_T &: \begin{cases}
    (\dict{\N_S}{\serviceaccount}, \timeslot, \N_S, \seq{\mathbb{T}}) &\to \tuple{\serviceaccount, \gas} \\
    ...
    \multicolumn{2}{l}{
      \begin{aligned}
        \quad\where (u, \mathbf{r}, \mathbf{s}') &= \Psi_M(\mathbf{s}_\mathbf{c}, 10, \sum_{r \in \mathbf{t}}{(r_g)}, \se(t, s, \len{\mathbf{t}}), F, \mathbf{s}) \\
...
\begin{longtable}{p{4cm} p{12cm}}
  ...
  $\begin{aligned}
    ...
    (\execst', \registers'_7, \mathbf{e}') &\equiv \begin{cases}
      ...
      (\continue, \segoff + \len{\mathbf{e}}, \mathbf{e} \doubleplus \mathbf{x}) &\otherwise
    \end{cases}
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  $\begin{aligned}
    ...
    \using \mathbf{a} &= \begin{cases}
      \de_4(\memory_{a\dots+4\corecount}) &\when \mathbb{N}_{a \dots+ 4\corecount} \subseteq \mathbb{V}_{\memory} \\
      \error &\otherwise
    \end{cases} \\
  ...
  $\begin{aligned}
    \using o &= \registers_7 \\
    \using \mathbf{v} &= \begin{cases}
      \left[\memory_{o + 336i \dots+ 336} \mid i \orderedin \valindex\right] &\when \mathbb{N}_{o \dots+ 336\valcount} \subseteq \mathbb{V}_{\memory} \\
      \error &\otherwise
    \end{cases} \\
