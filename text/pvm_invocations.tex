...
TODO... remove all greek state, replace with context
...
\begin{align}
  ...
  F(n, \gascounter, \registers, \memory, (\mathbf{x}, \mathbf{y})) &\equiv \begin{cases}
    ...
    \Omega_U(\gascounter, \registers, \memory, (\mathbf{x}, \mathbf{y}), \mathbf{x}_s) &\when n = \mathtt{upgrade} \\
    \Omega_T(\gascounter, \registers, \memory, (\mathbf{x}, \mathbf{y}), \mathbf{x}_s, \mathbf{d}) &\when n = \mathtt{transfer} \\
    \Omega_Q(\gascounter, \registers, \memory, (\mathbf{x}, \mathbf{y}), \mathbf{x}_s) &\when n = \mathtt{quit} \\
...
TODO: For upgrade, transfer and quit, just use the context directly and don't pass s/d
...
\begin{align}
  ...
  F(n, \gascounter, \registers, \memory, \mathbf{s}) &\equiv \begin{cases}
    \Omega_L(\gascounter, \registers, \memory, \mathbf{s}, s, \delta^\ddagger) &\when n = \mathtt{lookup} \\
    \Omega_R(\gascounter, \registers, \memory, \mathbf{s}, s, \delta^\ddagger) &\when n = \mathtt{read} \\
    \Omega_W(\gascounter, \registers, \memory, \mathbf{s}) &\when n = \mathtt{write} \\
    \Omega_G(\gascounter, \registers, \memory) &\when n = \mathtt{gas} \\
    \Omega_I(\gascounter, \registers, \memory, \mathbf{s}, s, \delta^\ddagger) &\when n = \mathtt{info} \\
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  $g = 10$} &
  $\begin{aligned}
    \registers'_0 &\equiv \gascounter' \bmod 2^{32} \\
    \registers'_1 &\equiv \left\lfloor \gascounter' \div 2^{32}\right\rfloor
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  ...
  $g = 10$} &
  $\begin{aligned}
    \using \mathbf{a} &= \begin{cases} \mathbf{s} &\when \registers_0 \in \{ s, 2^{32} - 1 \} \\ \mathbf{d}[\registers_0] &\otherwise \end{cases} \\
    \using [h_o, b_o, b_z] &= \registers_{1..4} \\
    \using h &= \begin{cases}
  ...
  $\begin{aligned}
    \using \mathbf{a} &= \begin{cases}
      \mathbf{s} &\when \registers_0 \in \{ s, 2^{32} - 1 \} \\
      \mathbf{d}[\registers_0] &\otherwhen \registers_0 \in \keys{\mathbf{d}} \\
      \none &\otherwise
    \end{cases} \\
  ...
  $\begin{aligned}
    ...
    \end{cases} \\
    (\registers'_0, \mathbf{s}') &\equiv \begin{cases}
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_I(\gascounter, \registers, \memory, \mathbf{s}, s, \mathbf{d})$ \\
  \texttt{info} = 4 \\
  $g = 10$} &
  $\begin{aligned}
    \using \mathbf{t} &= \begin{cases} \mathbf{s} &\when \registers_0 \in \{s, 2^{32} - 1\} \\ (\mathbf{d} \cup \mathbf{x}_\mathbf{n})[\registers_0] &\otherwise \end{cases} \\
    \using o &= \registers_1 \\
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  $g = 10$}&
  $\begin{aligned}
    \using [m, a, v, o, n] &= \registers_{0 \dots 5} \\
    \using \mathbf{g} &= \begin{cases}
      ...
      \error &\otherwise
    \end{cases} \\
    (\registers'_0, \mathbf{x}'_p) &= \begin{cases}
  ...
  $\begin{aligned}
    ...
    \using \mathbf{c} &= \begin{cases}
      ...
      \error &\otherwise
    \end{cases} \\
    (\registers'_0, \mathbf{x}') &= \begin{cases}
      (\mathtt{OK}, \mathbf{x} \exc \mathbf{x}'_\mathbf{c}[\registers_0] = \mathbf{c}) &\when \registers_0 < \mathsf{C} \wedge \mathbf{c} \ne \error \\
  ...
  $\begin{aligned}
    ...
    (\registers'_0, \mathbf{x}'_i, \mathbf{x}'_\mathbf{n}, (\mathbf{x}'_\mathbf{s})_b) &\equiv \begin{cases}
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
    $\Omega_U(\gascounter, \registers, \memory, (\mathbf{x}, \mathbf{y}), s)$ \\
    \texttt{upgrade} = 10 \\
    $g = 10$
  } &
  $\begin{aligned}
    ...
    \using g &= 2^{32}\cdot g_h + g_l \\
    \using m &= 2^{32}\cdot m_h + m_l \\
    (\registers'_0, \mathbf{x}'[s]_c, \mathbf{x}'[s]_g, \mathbf{x}'[s]_m) &\equiv \begin{cases}
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_T(\gascounter, \registers, \memory, (\mathbf{x}, \mathbf{y}), s, \delta)$ \\
  \texttt{transfer} = 11 \\
  ...
  $\begin{aligned}
    ...
    \using \mathbf{t} \in \mathbb{T} \cup \{\error\} &= \begin{cases}
      (s, d, a, m, g): m = \memory_{o\dots+\mathsf{M}} &\when \N_{o\dots+\mathsf{M}} \subset \mathbb{V}_{\memory} \\
      \error &\otherwise
    \end{cases} \\
    \using b &= (\mathbf{x}_\mathbf{s})_b - a \\
    (\registers'_0, \mathbf{x}'_\mathbf{t}, (\mathbf{x}'_\mathbf{s})_b) &\equiv \begin{cases}
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_Q(\gascounter, \registers, \memory, (\mathbf{x}, \mathbf{y}), s)$ \\
  \texttt{quit} = 12 \\
  $g = 10 + \registers_1 + 2^{32}\cdot\registers_2 $} &
  $\begin{aligned}
    \using [d, o] &= \registers_{0,1} \\
    ...
    \using \mathbf{t} \in \mathbb{T} \cup \{\error,\none\} &= \begin{cases}
      ...
      (s, d, a, m, g): m = \de(\memory_{o\dots+\mathsf{M}}) &\otherwhen \N_{o\dots+\mathsf{M}} \subset \mathbb{V}_{\memory} \\
      \error &\otherwise
    \end{cases} \\
    (\registers'_0, \mathbf{x}'_\mathbf{s}, \mathbf{x}'_\mathbf{t}) &\equiv \begin{cases}
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_S(\gascounter, \registers, \memory, (\mathbf{x}, \mathbf{y}))$ \\
  \texttt{solicit} = 13 \\
  $g = 10$} &
  $\begin{aligned}
    \using [o, z] &= \registers_{0, 1} \\
    \using h &= \begin{cases}
    ...
    \using \mathbf{a} &= \begin{cases}
      ...
      \error &\otherwise\\
    \end{cases} \\
    (\registers'_0, \mathbf{x}'_\mathbf{s}) &\equiv \begin{cases}
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  \endhead
  \makecell*[l]{
  $\Omega_H(\gascounter, \registers, \memory, (\mathbf{m}, \mathbf{e}), s, \delta, t)$ \\
  \texttt{historical\_lookup} = 15 \\
  $g = 10$} &
  $\begin{aligned}
    \using \mathbf{a} &= \begin{cases}
      \delta[s] &\when \registers_0 = 2^{32} - 1 \wedge s \in \keys{\delta} \\
      \delta[\registers_0] &\when \registers_0 \in \keys{\delta} \\
      \none &\otherwise
    \end{cases} \\
    \using [h_o, b_o, b_z] &= \registers_{1..4} \\
    \using h &= \begin{cases}
  ...
  $\begin{aligned}
    \using \mathbf{v} &= \begin{cases}
      \mathbf{i}_{\registers_0} &\when \registers_0 < |\mathbf{i}| \\
      \none &\otherwise
    \end{cases} \\
    \using o &= \registers_1 \\
    \using l &= \min(\registers_2, \mathsf{W}_E\mathsf{W}_S) \\
  ...
  $\begin{aligned}
    ...
    (\registers'_0, \mathbf{m}) &\equiv \begin{cases}
  ...
  $\begin{aligned}
    ...
    \using \mathbf{m}^* &= \mathbf{m} \exc \begin{cases}
      ...
      \end{cases}
    \end{cases}\\
    (\registers'_0, \registers'_1, \mem', \mathbf{m}') &\equiv \begin{cases}
      (\mathtt{OOB}, \registers_1, \mem, \mathbf{m}) &\when g = \error \\
      (\mathtt{WHO}, \registers_1, \mem, \mathbf{m}) &\otherwhen n \not\in \mathbf{m} \\
      (\mathtt{HOST}, h, \mem^*, \mathbf{m}^*) &\otherwhen c = \host \times h \\
      (\mathtt{FAULT}, x, \mem^*, \mathbf{m}^*) &\otherwhen c = \fault \times x \\
      (\mathtt{PANIC}, \registers_1, \mem^*, \mathbf{m}^*) &\otherwhen c = \panic \\
      (\mathtt{HALT}, \registers_1, \mem^*, \mathbf{m}^*) &\otherwhen c = \halt \\
    \end{cases} \\
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
