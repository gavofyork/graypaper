...
\begin{align}
  ...
  \label{eq:isauthorizedmutator}F \in \contextmutator{\emset} &\colon
    \tup{n, \gascounter, \registers, \memory} \mapsto \begin{cases}
      ...
      \Omega_\Gemini(\gascounter, \registers, \memory, \mathbf{p}) &\when n = \mathtt{grow\_heap} \\
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_\Gemini(\gascounter, \registers, \memory, \mathbf{p})$ \\
  \texttt{grow\_heap} = \next¬hostcall} &
  $\begin{aligned}
    ...
    \using g &= 100 + \max(0, \registers_7 - a - c) \cdot 1000 \\
