...
The inner \textsc{pvm} invocation host-calls, meanwhile, depend on an integrated \textsc{pvm} type, which we shall denote $\pvm¬instance$. It holds a \textsc{pvm} program blob, instruction counter, \textsc{ram} and a flag denoting whether gas was already charged for the currently executing basic block:
...
\begin{align}
  ...
  &F \in \contextmutator{\tuple{\dictionary{\N}{\pvm¬instance}, \dictionary{\N}{\pvm¬module}, \sequence{\segment}}} \colon
    (n, \gascounter, \registers, \memory, \tup{\mathbf{m}, \pvm¬module¬map, \mathbf{e}}) \mapsto \begin{cases}
      ...
      \Omega_\Gamma(\gascounter, \registers, \memory, \tup{\mathbf{m}, \pvm¬module¬map}, w_\wi¬serviceindex, \accounts, (p_\wp¬context)_\wc¬lookupanchortime) &\when n = \mathtt{compile}\\
      \Omega_M(\gascounter, \registers, \memory, \tup{\mathbf{m}, \pvm¬module¬map}) &\when n = \mathtt{machine}\\
      ...
      \Omega_X(\gascounter, \registers, \memory, \tup{\mathbf{m}, \pvm¬module¬map}) &\when n = \mathtt{expunge}\\
      \tup{\oog, \gascounter', \registers', \memory} &\otherwhen \gascounter' < 0\\
      \tup{\continue, \gascounter', \registers', \memory} &\otherwise\\
      \multicolumn{2}{l}{\where \registers' = \registers \exc \registers'_7 = \mathtt{WHAT}} \\
      \multicolumn{2}{l}{\also \gascounter' = \gascounter - \Cgasunknown} \\
...
These assume some refine context triple $\tup{\mathbf{m}, \pvm¬module¬map, \mathbf{e}} \in \tuple{\dictionary{\N}{\pvm¬instance}, \dictionary{\N}{\pvm¬module}, \sequence{\segment}}$, which are all initially empty. Other than the gas-counter which is explicitly defined, elements of \textsc{pvm} state are each assumed to remain unchanged by the host-call unless explicitly specified.
\begin{align}
...
\begin{longtable}{p{5cm} p{11cm}}
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_\Gamma(\gascounter, \registers, \memory, \tup{\mathbf{m}, \pvm¬module¬map}, s, \mathbf{d}, t)$ \\
  \texttt{compile} = 9 \\
  $g = \begin{cases}
    \CgasGammamem +  \fnmemgas(\CgasGammamemlinear, l) &\when m = 0 \\
    \CgasGammadb + \fnmemgas(\CgasGammadblinear, l) &\when m = 1 \\
    \CgasGammamem &\otherwise \end{cases}$ }&
  $\begin{aligned}
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_M(\gascounter, \registers, \memory, \tup{\mathbf{m}, \pvm¬module¬map})$ \\
  \texttt{machine} = 10 \\
  $g = \CgasM$} &
  $\begin{aligned}
  ...
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
  $\Omega_X(\gascounter, \registers, \memory, \tup{\mathbf{m}, \pvm¬module¬map})$ \\
  \texttt{expunge} = 15 \\
  $g = \CgasX$} &
  $\begin{aligned}
