...
\begin{gather}
  ...
  \Psi_A \colon\abracegroup{
    ...
    &\phantom{{}\mapsto{}} \also \local¬postxferstate = \local¬basestate \exc \abracegroup{
      \local¬postxferstate_\ps¬accounts\subb{s}_\sa¬balance &= \local¬basestate_\ps¬accounts\subb{s}_\sa¬balance + \!\!\!\!\!\sum_{r \in \mathbf{x}, r_\dx¬destauth = \bot}\!\!\!\!\!r_\dx¬amount \\
      \local¬postxferstate_\ps¬accounts\subb{s}_\sa¬authbalance &= \local¬basestate_\ps¬accounts\subb{s}_\sa¬authbalance + \!\!\!\!\!\sum_{r \in \mathbf{x}, r_\dx¬destauth = \top}\!\!\!\!\!r_\dx¬amount
    } \\
  ...
  F \in \contextmutator{\implicationspair} \colon \tup{n, \gascounter, \registers, \memory, \imXY} \mapsto \begin{cases}
    ...
    \Omega_V(\gascounter, \registers, \memory, \imXY) &\when n = \mathtt{authority} \\
...
\begin{longtable}{p{3cm} p{13cm}}
  ...
  $\begin{aligned}
    ...
    \tup{\execst', \registers'_7, (\imX_\im¬state')_\ps¬accounts, \imX_\im¬modifiedaccounts'} &\equiv \begin{cases}
      ...
      \tup{\continue, \mathtt{HUH}, \mathbf{d}, \imX_\im¬modifiedaccounts} &\when \imX_\im¬id \not\in \set{d, \fneffectiveauthority(\mathbf{d}, d)} \\
      \tup{\continue, \mathtt{FULL}, \mathbf{d}, \imX_\im¬modifiedaccounts} &\otherwhen \mathbf{d}'\subb{d}_\sa¬minbalance > \mathbf{d}'\subb{d}_\sa¬balance + \mathbf{d}'\subb{d}_\sa¬authbalance \\
    ...
    \using \mathbf{v} &= \begin{cases}
      \begin{aligned}
        &\fnencode(
          ...
          \encode[4]{\mathbf{a}_\sa¬authority},
          \encode[8]{\mathbf{a}_\sa¬balance, \mathbf{a}_\sa¬authbalance, \mathbf{a}_\sa¬minbalance, \mathbf{a}_\sa¬minaccgas, \mathbf{a}_\sa¬minmemogas, \mathbf{a}_\sa¬octets}, \\
        &\phantom{\fnencode(}
  ...
  $\begin{aligned}
    ...
    \using \mathbf{a} \in \serviceaccount &= \tup{\begin{aligned}
      ...
      \sa¬balance,
      \is{\sa¬authority}{\tup{\imX_\im¬id, (\imX_\im¬self)_\sa¬created}},
      \sa¬authbalance, \\
      &\sa¬minaccgas,
      \sa¬minmemogas,
    ...
    \end{aligned}} \\
    &\phantom{{}={}} \where \tup{\sa¬balance, \sa¬authbalance} = \begin{cases}
    ...
    \using \mathbf{d}' &= \mathbf{d} \exc \begin{cases}
      ...
      \mathbf{d}'\subb{s}_\sa¬authbalance = \mathbf{d}\subb{s}_\sa¬authbalance - \mathbf{a}_\sa¬minbalance &\otherwise
    \end{cases} \\
    \using p &= \begin{cases}
      \imX_\im¬id \in \set{s, \fneffectiveauthority(\mathbf{d}, s)} &\when x \in \set{0, 2} \\
      \imX_\im¬id = \fneffectiveauthority(\mathbf{d}, s) &\otherwise
    \end{cases} \\
    \tup{\execst', \registers'_7, \imX'_\im¬nextfreeid, (\imX'_\im¬state)_\ps¬accounts, \imX'_\im¬modifiedaccounts} &\equiv \begin{cases}
      ...
      \tup{\continue, \mathtt{CASH}, \imX_\im¬nextfreeid, \mathbf{d}, \imX_\im¬modifiedaccounts} &\otherwhen \mathbf{d}'\subb{s}_\sa¬balance < 0 \lor \mathbf{d}'\subb{s}_\sa¬authbalance < 0 \lor {} \\
      &\phantom{\otherwhen} \mathbf{d}'\subb{s}_\sa¬balance + \mathbf{d}'\subb{s}_\sa¬authbalance < \mathbf{d}'\subb{s}_\sa¬minbalance \\
  ...
  $\begin{aligned}
    ...
    \end{cases} \\
    \using \mathbf{d} &= (\imX_\im¬state)_\ps¬accounts \\
    \using \mathbf{d}' &= \mathbf{d} \exc \mathbf{d}'\subb{d}_\sa¬authority = \tup{u, \mathbf{d}\subb{u}_\sa¬created} \\
  ...
  $\begin{aligned}
    ...
    \using \mathbf{d}' &= \mathbf{d} \exc \begin{cases}
      ...
      \mathbf{d}'\subb{\dx¬source}_\sa¬authbalance = \mathbf{d}\subb{\dx¬source}_\sa¬authbalance - \dx¬amount &\when x \in \set{1, 3} \\
      \mathbf{d}'\subb{\dx¬dest}_\sa¬balance = \mathbf{d}\subb{\dx¬dest}_\sa¬balance + \dx¬amount &\when o = 0 \land x \in \set{0, 1} \\
      \mathbf{d}'\subb{\dx¬dest}_\sa¬authbalance = \mathbf{d}\subb{\dx¬dest}_\sa¬authbalance + \dx¬amount &\when o = 0 \land x \in \set{2, 3}
    \end{cases} \\
    ...
    \using p_s &= \begin{cases}
      \imX_\im¬id \in \set{\dx¬source, \fneffectiveauthority(\mathbf{d}, \dx¬source)} &\when x \in \set{0, 2} \\
      \imX_\im¬id = \fneffectiveauthority(\mathbf{d}, \dx¬source) &\otherwise
    \end{cases} \\
    \using p_d &= o \ne 0 \lor (\imX_\im¬id \in \set{\dx¬dest, \fneffectiveauthority(\mathbf{d}, \dx¬dest)} \land (x \in \set{1, 2} \lor s \ne d)) \\
    \using \tup{c, t} &= \begin{cases}
      ...
      \tup{\mathtt{CASH}, 0} &\otherwhen \mathbf{d}'\subb{\dx¬source}_\sa¬balance < 0 \lor \mathbf{d}'\subb{\dx¬source}_\sa¬authbalance < 0 \lor \mathbf{d}'\subb{\dx¬source}_\sa¬balance + \mathbf{d}'\subb{\dx¬source}_\sa¬authbalance < \mathbf{d}'\subb{\dx¬source}_\sa¬minbalance \\
  ...
  $\begin{aligned}
    ...
    \using \mathbf{d}' &= \mathbf{d} \exc \abracegroup{
      &d \not\in \keys{\mathbf{d}'} \\
      &\mathbf{d}'\subb{\imX_\im¬id}_\sa¬balance = (\imX_\im¬self)_\sa¬balance + \mathbf{d}\subb{d}_\sa¬balance + \mathbf{d}\subb{d}_\sa¬authbalance \\
    } \\
    \tup{\registers'_7, (\imX'_\im¬state)_\ps¬accounts, \imX'_\im¬modifiedaccounts} &\equiv \begin{cases}
      \tup{\mathtt{WHO}, \mathbf{d}, \imX_\im¬modifiedaccounts} &\when d \not\in \mathbf{d} \\
      \tup{\mathtt{HUH}, \mathbf{d}, \imX_\im¬modifiedaccounts} &\otherwhen d = \imX_\im¬id \vee \imX_\im¬id \ne \fneffectiveauthority(\mathbf{d}, d) \vee \mathbf{d}\subb{d}_\sa¬items \ne 0 \\
  ...
  $\begin{aligned}
    ...
    \tup{\execst', \registers'_7, (\imX'_\im¬state)_\ps¬accounts, \imX'_\im¬modifiedaccounts} &\equiv \begin{cases}
      ...
      \tup{\continue, \mathtt{HUH}, \mathbf{d}, \imX_\im¬modifiedaccounts} &\otherwhen \imX_\im¬id \not\in \set{d, \fneffectiveauthority(\mathbf{d}, d)} \lor \mathbf{d}' = \error \\
      \tup{\continue, \mathtt{FULL}, \mathbf{d}, \imX_\im¬modifiedaccounts} &\otherwhen \mathbf{d}'\subb{d}_\sa¬balance + \mathbf{d}'\subb{d}_\sa¬authbalance < \mathbf{d}'\subb{d}_\sa¬minbalance \\
      \tup{\continue, \mathtt{OK}, \mathbf{d}', \imX_\im¬modifiedaccounts \cup \set{d}} &\otherwise
    \end{cases} \\
    g &\equiv \CgasS
  \end{aligned}$\\
