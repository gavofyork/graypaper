...
\begin{align}\label{eq:implications}
  \implications &\equiv \tuple{
    ...
    \isa{\im¬writtenkeys}{\protoset{\tuple{\serviceid, \blob}}},
    \isa{\im¬modifiedreqs}{\protoset{\tuple{\serviceid, \hash, \bloblength}}},
...
\begin{gather}
  ...
  \Psi_A \colon\abracegroup{
    ...
    \tup{\local¬basestate, t, s, g, \mathbf{i}} &\mapsto \begin{cases}
      \tup{
        ...
        \is{\ao¬writtenkeys}{\emset},
        \is{\ao¬modifiedreqs}{\emset},
        \is{\ao¬defxfers}{\sq{}},
        \is{\ao¬yield}{\none},
        \is{\ao¬gasused}{0},
        \is{\ao¬provisions}{\sq{}}
      }
        &\when s \not\in \local¬basestate_\ps¬accounts \vee \jam¬blob = \none \vee \len{\jam¬blob} > \Cmaxservicecodesize \\
      ...
      \begin{aligned}
        ...
        &\quad\also \local¬postxferstate = \local¬basestate \exc \abracegroup{
  ...
  I\colon\abracegroup{
    ...
    \tup{\im¬state, \im¬id} &\mapsto \tup{
      \im¬id,
      \im¬state,
      \is{\im¬writtenkeys}{\emset},
      \is{\im¬modifiedreqs}{\emset},
      \im¬nextfreeid,
      \is{\im¬xfers}{\sq{}},
      \is{\im¬yield}{\none},
      \is{\im¬provisions}{\sq{}}
  ...
  C\colon\abracegroup{
    ...
    \tup{\ao¬gasused, \mathbf{o}, \imXY} &\mapsto \begin{cases}
      \tup{
        ...
        \is{\ao¬writtenkeys}{\imY_\im¬writtenkeys},
        \is{\ao¬modifiedreqs}{\imY_\im¬modifiedreqs},
      ...
      \tup{
        ...
        \is{\ao¬writtenkeys}{\imX_\im¬writtenkeys},
        \is{\ao¬modifiedreqs}{\imX_\im¬modifiedreqs},
...
\begin{longtable}{p{3cm} p{13cm}}
  ...
  $\begin{aligned}
    ...
    \end{cases} \\
    \tup{\execst', \registers'_7, (\imX_\im¬state')_\ps¬accounts, \imX_\im¬writtenkeys'} &\equiv \begin{cases}
      \tup{\panic, \registers_7, \mathbf{d}, \imX_\im¬writtenkeys} &\when \mathbf{k} = \error \vee \mathbf{v} = \error\\
      \tup{\continue, \mathtt{WHO}, \mathbf{d}, \imX_\im¬writtenkeys} &\when d \not\in \keys{\mathbf{d}} \\
      \tup{\continue, \mathtt{HUH}, \mathbf{d}, \imX_\im¬writtenkeys} &\when \imX_\im¬id \not\in \set{d, \mathbf{d}\subb{d}_\sa¬authority} \\
      \tup{\continue, \mathtt{FULL}, \mathbf{d}, \imX_\im¬writtenkeys} &\otherwhen \mathbf{d}'\subb{d}_\sa¬minbalance > \mathbf{d}'\subb{d}_\sa¬balance + \mathbf{d}'\subb{d}_\sa¬authbalance \\
      \tup{\continue, l, \mathbf{d}', \imX_\im¬writtenkeys \cup \set{\tup{d, \mathbf{k}}}} &\otherwise\\
    \end{cases} \\
  ...
  $\begin{aligned}
    \using s &= \begin{cases}
      ...
      \registers_7 &\otherwise
    \end{cases} \\
    \using \sq{\local¬desiredid, x, a, o} &= \registers\subrange{8}{4} \\
    ...
    &\phantom{{}={}} \where \tup{\sa¬balance, \sa¬authbalance} = \begin{cases}
      \tup{a, 0} &\when x \in \set{0, 1} \\
      \tup{0, a} &\otherwise
    \end{cases} \\
    \using \mathbf{d} &= (\imX_\im¬state)_\ps¬accounts \\
    \using \mathbf{d}' &= \mathbf{d} \exc \begin{cases}
      \mathbf{d}'\subb{i'} = \mathbf{a} &\text{always} \\
      \mathbf{d}'\subb{s}_\sa¬balance = \mathbf{d}\subb{s}_\sa¬balance - a &\when x \in \set{0, 2} \\
      \mathbf{d}'\subb{s}_\sa¬authbalance = \mathbf{d}\subb{s}_\sa¬authbalance - a &\otherwise
    \end{cases} \\
    \using p &= \begin{cases}
      s &\when x \in \set{0, 2} \\
      \mathbf{d}\subb{s}_\sa¬authority &\otherwise
    \end{cases} \\
    \tup{\execst', \registers'_7, \imX'_\im¬nextfreeid, (\imX'_\im¬state)_\ps¬accounts} &\equiv \begin{cases}
      \tup{\panic, \registers_7, \imX_\im¬nextfreeid, \mathbf{d}} &\when \Nrange{o}{60} \not\subseteq \readable{\memory} \\
      \tup{\continue, \mathtt{WHO}, \imX_\im¬nextfreeid, \mathbf{d}} &\otherwhen s \not\in \mathbf{d} \\
      \tup{\continue, \mathtt{HUH}, \imX_\im¬nextfreeid, \mathbf{d}} &\otherwhen x > 3 \lor \imX_\im¬id \ne p \lor a < \mathbf{a}_\sa¬minbalance \lor (\sa¬gratis \ne 0 \wedge \imX_\im¬id \ne (\imX_\im¬state)_\ps¬manager) \\
      \tup{\continue, \mathtt{CASH}, \imX_\im¬nextfreeid, \mathbf{d}} &\otherwhen \mathbf{d}'\subb{s}_\sa¬balance < 0 \lor \mathbf{d}'\subb{s}_\sa¬authbalance < 0 \lor {} \\
      &\phantom{\otherwhen{}} \mathbf{d}'\subb{s}_\sa¬balance + \mathbf{d}'\subb{s}_\sa¬authbalance < \mathbf{d}'\subb{s}_\sa¬minbalance \\
      \tup{\continue, \mathtt{FULL}, \imX_\im¬nextfreeid, \mathbf{d}} &\otherwhen \imX_\im¬id = (\imX_\im¬state)_\ps¬registrar \wedge \local¬desiredid < \Cminpublicindex \wedge \local¬desiredid \in \keys{\mathbf{d}} \\
      \tup{\continue, i', i^*, \mathbf{d}'} &\otherwise
    \end{cases} \\
    g &\equiv \CgasN
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  ...
  $\begin{aligned}
    \using d &= \begin{cases}
      ...
      \registers_7 &\otherwise
    \end{cases} \\
    \using \sa¬authority &= \registers_8 \\
    \using \mathbf{d} &= (\imX_\im¬state)_\ps¬accounts \exc \mathbf{d}\subb{d}_\sa¬authority = \sa¬authority \\
    \tup{\registers'_7, (\imX'_\im¬state)_\ps¬accounts} &\equiv \begin{cases}
      \tup{\mathtt{WHO}, (\imX_\im¬state)_\ps¬accounts} &\when d \not\in \keys{(\imX_\im¬state)_\ps¬accounts} \lor \sa¬authority \not\in \serviceid \\
      \tup{\mathtt{HUH}, (\imX_\im¬state)_\ps¬accounts} &\otherwhen \imX_\im¬id \ne (\imX_\im¬state)_\ps¬accounts\subb{d}_\sa¬authority \\
      \tup{\mathtt{OK}, \mathbf{d}} &\otherwise
    \end{cases} \\
    g &\equiv \CgasV
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  ...
  $\begin{aligned}
    ...
    \tup{\execst', \registers'_7, (\imX'_\im¬state)_\ps¬accounts} &\equiv \begin{cases}
      \tup{\panic, \registers_7, \mathbf{d}} &\when c = \error \\
      \tup{\continue, \mathtt{WHO}, \mathbf{d}} &\when d \not\in \keys{\mathbf{d}} \\
      \tup{\continue, \mathtt{HUH}, \mathbf{d}} &\when \imX_\im¬id \not\in \set{d, \mathbf{d}\subb{d}_\sa¬authority} \\
      \tup{\continue, \mathtt{OK}, \mathbf{d}'} &\otherwise \\
    \end{cases} \\
    g &\equiv \CgasU
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  ...
  $\begin{aligned}
    ...
    \using \mathbf{d}' &= \mathbf{d} \exc \begin{cases}
      ...
      \mathbf{d}'\subb{\dx¬dest}_\sa¬balance = \mathbf{d}\subb{\dx¬dest}_\sa¬balance + \dx¬amount &\when o = 0 \land x = 1 \\
    ...
    \using p_s &= \begin{cases}
      \dx¬source &\when x \in \set{0, 2} \\
      \mathbf{d}\subb{\dx¬source}_\sa¬authority &\otherwise
    \end{cases} \\
    \using p_d &= \begin{cases}
      \dx¬dest &\when x = 1 \\
      \mathbf{d}\subb{\dx¬dest}_\sa¬authority &\when x = 2 \lor (x = 3 \land \dx¬source \ne \dx¬dest) \\
      \none &\otherwise
    \end{cases} \\
    \using \tup{c, t} &= \begin{cases}
      \tup{\panic, 0} &\when \mathbf{t} = \error \\
      \tup{\mathtt{WHO}, 0} &\otherwhen \dx¬source \not\in \keys{\mathbf{d}} \lor \dx¬dest \not\in \keys{\mathbf{d}} \\
      \tup{\mathtt{HUH}, 0} &\otherwhen x > 3 \lor \imX_\im¬id \ne p_s \lor (o = 0 \land \imX_\im¬id \ne p_d) \\
    ...
    \end{cases} \\
    \tup{\execst', \registers'_7, \imX'_\im¬xfers, (\imX'_\im¬state)_\ps¬accounts} &\equiv \begin{cases}
      \tup{\panic, \registers_7, \imX_\im¬xfers, \mathbf{d}} &\when c = \panic \\
      \tup{\continue, c, \imX_\im¬xfers, \mathbf{d}} &\otherwhen c \ne \mathtt{OK} \\
      \tup{\continue, \mathtt{OK}, \imX_\im¬xfers \concat \mathbf{t}, \mathbf{d}'} &\otherwise
    \end{cases} \\
    g &\equiv \CgasT + t
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  ...
  $\begin{aligned}
    \using d &= \registers_7 \\
    \using \mathbf{d} &= (\imX_\im¬state)_\ps¬accounts\subb{d} \\
    \using \mathbf{s}' &= \imX_\im¬self \exc \mathbf{s}'_\sa¬balance = (\imX_\im¬self)_\sa¬balance + \mathbf{d}_\sa¬balance + \mathbf{d}_\sa¬authbalance \\
    \tup{\registers'_7, (\imX'_\im¬state)_\ps¬accounts} &\equiv \begin{cases}
      \tup{\mathtt{WHO}, (\imX_\im¬state)_\ps¬accounts} &\when \mathbf{d} = \none \\
      \tup{\mathtt{HUH}, (\imX_\im¬state)_\ps¬accounts} &\otherwhen \imX_\im¬id \ne \mathbf{d}_\sa¬authority \vee \mathbf{d}_\sa¬items \ne 0 \\
      \tup{\mathtt{OK}, (\imX_\im¬state)_\ps¬accounts \setminus \set{d} \cup \set{ \kv{\imX_\im¬id}{\mathbf{s}'} }} &\otherwise
    \end{cases} \\
    g &\equiv \CgasJ
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  ...
  $\begin{aligned}
    ...
    \using \mathbf{d}' &= \begin{cases}
      ...
      \error &\otherwise\\
    \end{cases} \\
    \tup{\execst', \registers'_7, (\imX'_\im¬state)_\ps¬accounts, \imX'_\im¬modifiedreqs} &\equiv \begin{cases}
      \tup{\continue, \mathtt{HUH}, \mathbf{d}, \imX_\im¬modifiedreqs} &\when z \not \in \bloblength \\
      \tup{\panic, \registers_7, \mathbf{d}, \imX_\im¬modifiedreqs} &\otherwhen h = \error \\
      \tup{\continue, \mathtt{WHO}, \mathbf{d}, \imX_\im¬modifiedreqs} &\otherwhen d \not\in \keys{\mathbf{d}} \\
      \tup{\continue, \mathtt{HUH}, \mathbf{d}, \imX_\im¬modifiedreqs} &\otherwhen \imX_\im¬id \not\in \set{d, \mathbf{d}\subb{d}_\sa¬authority} \lor \mathbf{d}' = \error \\
      \tup{\continue, \mathtt{FULL}, \mathbf{d}, \imX_\im¬modifiedreqs} &\otherwhen \mathbf{d}'\subb{d}_\sa¬balance + \mathbf{d}'\subb{d}_\sa¬authbalance < \mathbf{d}'\subb{d}_\sa¬minbalance \\
      \tup{\continue, \mathtt{OK}, \mathbf{d}', \imX_\im¬modifiedreqs \cup \set{\tup{d, h, z}}} &\otherwise
    \end{cases} \\
    g &\equiv \CgasS
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
