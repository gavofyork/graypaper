...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  $\begin{aligned}
    ...
    % Clamp so that ranges make sense.
    \using h &= \max(a, \registers_7) \\
    \tup{\execst', \gascounter', \registers'_7, (\memory'_\ram¬access)\interval{a}{h}} &\equiv \begin{cases}
      % Not enough gas to even cover the constant cost of the call, regardless of whether it's a no-op or not.
      \tup{\oog, \gascounter, \registers_7, (\memory_\ram¬access)\interval{a}{h}} &\when \gascounter < 100 \\
      % Expand the accessible RW data region, and return the new page count.
      \tup{\continue, \gascounter - g, \max(a + c, h), \sq{\mathrm{W}, \dots, \mathrm{W}}} &\otherwhen h \leq b \land \gascounter \ge g \\
      ...
      \tup{\continue, \gascounter - 100, a + c, (\memory_\ram¬access)\interval{a}{h}} &\otherwise \\
    \end{cases}
  \end{aligned}$\\
  \cmidrule(lr){1-1}\cmidrule(lr){2-2}
  \makecell*[l]{
