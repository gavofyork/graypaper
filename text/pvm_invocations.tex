...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  $\begin{aligned}
    \using \mathbf{v} &= \begin{cases}
      ...
      \multicolumn{2}{l}{\where S(w) \equiv \se(\fixedencode{4}{w_\wi¬service}, w_\wi¬codehash, \fixedencode{8}{w_\wi¬refgaslimit, w_\wi¬accgaslimit}, \fixedencode{2}{w_\wi¬exportcount, \len{w_\wi¬importsegments}, \len{w_\wi¬extrinsics}}, \fixedencode{4}{\len{w_\wi¬payload}})} \\
    ...
    (\execst', \registers'_7, \memory'_{o\dots+l}) &\equiv \begin{cases}
      (\panic, \registers_7, \memory_{o\dots+l}) &\when \Nrange{o}{l} \not\subseteq \writable{\memory} \\
  ...
  $\begin{aligned}
    ...
    (\execst', \registers'_7, \memory'_{o\dots+l}) &\equiv \begin{cases}
      (\panic, \registers_7, \memory_{o\dots+l}) &\when \mathbf{v} = \error \vee \Nrange{o}{l} \not\subseteq \writable{\memory}\\
      (\continue, \mathtt{NONE}, \memory_{o\dots+l}) &\otherwhen \mathbf{v} = \none \\
      (\continue, \len{\mathbf{v}}, \mathbf{v}_{f\dots+l}) &\otherwise \\
    \end{cases}
  \end{aligned}$\\
  ...
  $\begin{aligned}
    ...
    \using \mathbf{k} &= \begin{cases}
      \memory_{k_o\dots+k_z} &\when \Nrange{k_o}{k_z} \subseteq \readable{\memory} \\
      \error &\otherwise
    \end{cases} \\
    \using \mathbf{a} &= \begin{cases}
      \mathbf{s}\,,\ \exc \keys{\mathbf{a}_\mathbf{s}} = \keys{\mathbf{a}_\mathbf{s}} \setminus \set{k} & \when v_z = 0 \\
      \mathbf{s}\,,\ \exc \mathbf{a}_\mathbf{s}[\mathbf{k}] = \memory_{v_o\dots+v_z} &\otherwhen \Nrange{v_o}{v_z} \subseteq \readable{\memory} \\
      \error &\otherwise
    \end{cases} \\
    \using l &= \begin{cases}
...
\begin{longtable}{p{4cm} p{12cm}}
  ...
  $\begin{aligned}
    ...
    \using \mathbf{x} &= \begin{cases}
      \zeropad_{\mathsf{W}_G}(\mem_{p\dots+z}) &\when \Nrange{p}{z} \subseteq \mathbb{V}_\memory\\
      \error &\otherwise
    \end{cases}\\
  ...
  $\begin{aligned}
    ...
    \using \mathbf{p} &= \begin{cases}
      \memory_{p_o\dots+p_z} &\when \Nrange{p_o}{p_z} \subseteq \readable{\memory} \\
      \error &\otherwise
    \end{cases} \\
  ...
  $\begin{aligned}
    ...
    \using (g, \mathbf{w}) &= \begin{cases}
      (g, \mathbf{w}): \fixedencode{8}{g} \concat \fixedencode{8}{\mathbf{w}} = \mem_{o \dots+ 112} &\when \Nrange{o}{112} \subseteq \writable{\mem} \\
      %(\decode{8}{\memr_{o\dots+8}}, [\decode{4}{\memr_{o+8+8x\dots+8}} \mid x \orderedin \Nmax{13}]) &\when \Nrange{o}{60} \subset \writable_\mem} \\
      (\error, \error) &\otherwise
    \end{cases} \\
    \using (c, i', g', \mathbf{w}', \mathbf{u}') &= \Psi(\mathbf{m}[n]_\mathbf{p}, \mathbf{m}[n]_i, g, \mathbf{w}, \mathbf{m}[n]_\mathbf{u})\\
    \using \mem^* &= \mem \exc \mem^*_{o \dots+ 112} = \fixedencode{8}{g'} \concat \fixedencode{8}{\mathbf{w}'}\\
...
\begin{longtable}{p{3.5cm} p{12.5cm}}
  ...
  $\begin{aligned}
    ...
    \using \mathbf{a} &= \begin{cases}
      \decode{4}{\memory_{a\dots+4\corecount}} &\when \Nrange{a}{4\corecount} \subseteq \readable{\memory} \\
      \error &\otherwise
    \end{cases} \\
    \using \mathbf{z} &= \begin{cases}
      \set{\build{\kv{s}{g} \ \where \fixedencode{4}{s} \concat \fixedencode{8}{g} = \memory_{o+12i\dots+12}}{i \in \Nmax{n}}} &\when \Nrange{o}{12n} \subseteq \readable{\memory} \\
      \error &\otherwise
    \end{cases} \\
  ...
  $\begin{aligned}
    ...
    \using \mathbf{q} &= \begin{cases}
      \left[\memory_{o + 32i \dots+ 32} \mid i \orderedin \N_\mathsf{Q}\right] &\when \Nrange{o}{32\mathsf{Q}} \subseteq \readable{\memory} \\
      \error &\otherwise
    \end{cases} \\
  ...
  $\begin{aligned}
    \using o &= \registers_7 \\
    \using \mathbf{v} &= \begin{cases}
      \left[\memory_{o + 336i \dots+ 336} \mid i \orderedin \valindex\right] &\when \Nrange{o}{336\valcount} \subseteq \readable{\memory} \\
      \error &\otherwise
    \end{cases} \\
  ...
  $\begin{aligned}
    ...
    \using c &= \begin{cases}
      \memory_{o\dots+32} &\when \Nrange{o}{32} \subseteq \readable{\memory} \wedge l \in \Nbits{32} \\
      \error &\otherwise
    \end{cases}\\
  ...
  $\begin{aligned}
    ...
    \using c &= \begin{cases}
      \memory_{o\dots+32} &\when \Nrange{o}{32} \subseteq \readable{\memory} \\
      \error &\otherwise
    \end{cases} \\
  ...
  $\begin{aligned}
    ...
    \using \mathbf{i} &= \begin{cases}
      \memory_{o\dots+z} &\when \Nrange{o}{z} \subseteq \readable{\memory} \\
      \error &\otherwise
    \end{cases} \\
    \using \mathbf{a} &= \begin{cases}
