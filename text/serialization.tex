...
Sets are encoded in a manner similar to dictionaries, but without the value parts. We encode a set as a sequence of its encoded elements, ordered to ensure a unique representation. Formally:

\begin{equation}
  \se(s \in \mathbb{S}) \equiv \se(\var{\orderby{x}{\se(x) \mid x \in s}})
\end{equation}
