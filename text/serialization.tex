...
We first define the trivial integer serialization functions which are subscripted by the number of octets of the final sequence. Values are encoded in a regular little-endian fashion. Formally:
\begin{equation}
...
We also define the variable-size prefix 29-bit integer serialization function $\se_{4*}$:
\begin{equation}
...
A block $\mathbf{B}$ is serialized as a tuple of its elements in regular order, as implied in equations \ref{eq:block}, \ref{eq:extrinsic} and \ref{eq:header}. For the header, we define both the regular serialization and the unsigned serialization $\se_U$ (the latter has no inverse). Formally:
\begin{align}
  ...
  \se_U(\mathbf{H}) &= \se(\mathbf{H}_p,\mathbf{H}_r,\mathbf{H}_x)\concat\se_4(\mathbf{H}_t)\concat\se(\maybe{\mathbf{H}_e},\maybe{\mathbf{H}_w},\se_4(\mathbf{H}_k),\mathbf{H}_v)\\
