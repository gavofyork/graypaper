...
%      [0] &\when x = 0 \\
%      \left[2^8-2^{8-l} + \ffrac{x}{2^{8l}}\right] \concat \fixeddecode{l}{x \bmod 2^{8l}} &\when \exists l \in \N_3 : 2^{7l} \le x < 2^{7(l+1)} \\
%      \left[2^8-2^5 + \ffrac{x}{2^{24}}\right] \concat \fixeddecode{3}{x \bmod 2^{24}} &\when 2^{21} \le x < 2^{29} \\
%    \end{cases}
%  }
%\end{equation}
...
\begin{equation}
  \se\colon\abracegroup{
    ...
    x &\mapsto \begin{cases}
      [0] &\when x = 0 \\
      \left[2^8-2^{8-l} + \ffrac{x}{2^{8l}}\right] \concat \fixeddecode{l}{x \bmod 2^{8l}} &\when \exists l \in \N_8 : 2^{7l} \le x < 2^{7(l+1)} \\
      [2^8-1] \concat \fixeddecode{8}{x} &\otherwhen x < 2^{64} \\
    \end{cases}
  }
\end{equation}

\subsubsection{Sequence Encoding}
We define the sequence serialization function $\se(\sequence{T})$ for any $T$ which is itself a subset of the domain of $\se$. We simply concatenate the serializations of each element in the sequence in turn:
\begin{equation}
...
\begin{equation}
  \forall K, V: \se(d \in \dict{K}{V}) \equiv
    \encode{
      \var{\sq{
        \orderby{k}{
          \build{
...
%    &[1] \concat \se(d[k]) &&\ \when k \in \keys{d} \\
%    &[0] &&\ \otherwise
%  \end{aligned}
%  \,\middle\mid\, k \in K}}
%\end{equation}
...
\begin{align}
  \se_{l \in \N}(a, b, \dots) &\equiv \fixeddecode{l}{\tup{a, b, \dots}}\\
  \se_{l \in \N}(\tup{a, b, \dots}) &\equiv \fixeddecode{l}{a} \concat \fixeddecode{l}{b} \concat \dots\\
  \se_{l \in \N}(\sq{\mathbf{i}_0, \mathbf{i}_1, \dots}) &\equiv \fixeddecode{l}{\mathbf{i}_0} \concat \fixeddecode{l}{\mathbf{i}_1} \concat \dots
\end{align}
...
\begin{align}
  ...
  \se_G(\xtguarantees) &= \se(\var{[\tup{w, \fixeddecode{4}{t}, \var{a}} \mid \tup{w, t, a} \orderedin \xtguarantees]}) \\
  \se_A(\xtassurances) &= \se(\var{[\tup{a, f, \fixeddecode{2}{v}, s} \mid \tup{a, f, v, s} \orderedin \xtassurances]}) \\
  \se_D((\mathbf{v}, \mathbf{c}, \mathbf{f})) &= \se(\var{[(r, \fixeddecode{4}{a}, [(v, \fixeddecode{2}{i}, s) \mid (v, i, s) \orderedin \mathbf{j}]) \mid (r, a, \mathbf{j}) \orderedin \mathbf{v}]}, \var{\mathbf{c}}, \var{\mathbf{f}}) \\
  \se(\mathbf{H}) &= \se_U(\mathbf{H})\concat\se(\H_\¬sealsig) \\
  \se_U(\mathbf{H}) &= \se(\H_\¬parent,\H_\¬priorstateroot,\H_\¬extrinsichash)\concat\fixeddecode{4}{\H_\¬timeslot}\concat\se(\maybe{\H_\¬epochmark},\maybe{\H_\¬winnersmark},\fixeddecode{2}{\H_\¬authorindex},\H_\¬vrfsig,\var{\H_\¬offendersmark})\\
  \se(x \in \workcontext) &\equiv \se(x_a, x_s, x_b, x_l)\concat\fixeddecode{4}{x_t}\concat\se(\var{x_\mathbf{p}})\\
  \se(x \in \avspec) &\equiv \se(x_h) \concat \fixeddecode{4}{x_l}\concat\se(x_u, x_e) \concat \fixeddecode{2}{x_n} \\
  \se(x \in \mathbb{L}) &\equiv \se(\fixeddecode{4}{x_s}, x_c, x_y, \fixeddecode{8}{x_g}, x_u, x_i, x_x, x_z, x_e, \seresult(x_\wd¬result))\\
  \se(x \in \workreport) &\equiv \se(x_s, x_x, x_c, x_a, x_g, \var{x_\wr¬authtrace}, \var{x_\mathbf{l}}, \var{x_\wr¬digests}) \\
  \se(x \in \workpackage) &\equiv \se(\fixeddecode{4}{x_\wp¬authcodehost}, x_\wp¬authcodehash, x_\wp¬context, \var{x_\wp¬authtoken}, \var{x_\wp¬authconfig}, \var{x_\wp¬workitems}) \\
  % TODO: \fixeddecode{4}{i} should be just \se(i), but we should wait for this to be written.
  \se(x \in \workitem) &\equiv \se(
    \fixeddecode{4}{x_\wi¬service},
    x_\wi¬codehash,
    \fixeddecode{8}{x_\wi¬refgaslimit},
    \fixeddecode{8}{x_\wi¬accgaslimit},
    \fixeddecode{2}{x_\wi¬exportcount}
    \var{x_\wi¬payload},
    \var{\se_I^\#(x_\wi¬importsegments)},
    \var{\sq{\tup{h, \fixeddecode{4}{i}} \mid \tup{h, i} \orderedin x_\wi¬extrinsics}},
  ) \\
  \se(x \in \safroleticket) &\equiv \se(x_\mathbf{y}, x_r) \\
  \se(x \in \defxfer) &\equiv \se(\fixeddecode{4}{x_s}, \fixeddecode{4}{x_d}, \fixeddecode{8}{x_a}, \se(x_m), \fixeddecode{8}{x_g}) \\
  ...
  \se_I(h \in \hash \cup \hash^\boxplus, i \in \Nbits{15}) &\equiv \begin{cases}
    (h, \fixeddecode{2}{i}) &\when h \in \hash\\
    (r, \fixeddecode{2}{i + 2^{15}}) &\when \exists r \in \hash, h = r^\boxplus\\
  \end{cases}
\end{align}
