...
\begin{equation}
  C\colon\abracegroup{
    ...
    (s, h) &\mapsto [n_0, a_0, n_1, a_1, n_2, a_2, n_3, a_3, a_4, a_5, \dots, a_{26}]\ \where n = \fixedencode{4}{s}, a = \blake(h)_{0\dots 27}
  }
\end{equation}
...
\begin{equation}
  T(\thestate) \equiv \abracegroup{
    ...
    &&C(10) &\mapsto \se([\maybe{(w, \fixedencode{4}{t})} \mid (w, t) \orderedin \reports]) \;, \\
    ...
    &&C(15) &\mapsto \encode{[\var{\mathbf{i}} \mid \mathbf{i} \orderedin \accumulated]} \;, \\
    &&C(16) &\mapsto \se(\var{[(\fixedencode{4}{s}, \encode{h}) \mid (s, h) \orderedin \lastaccout]}) \;, \\
    ...
    \forall (s \mapsto \mathbf{a}) \in \accounts, (\tup{h, l} \mapsto \mathbf{t}) \in \mathbf{a}_\mathbf{l}: &&C(s, \fixedencode{4}{l} \concat h) &\mapsto \se(\var{[\fixedencode{4}{x} \mid x \orderedin \mathbf{t}]})
  }
\end{equation}
...
\begin{align}
  ...
  L&\colon\abracegroup{
    ...
    (k, v) &\mapsto \begin{cases}
      [1, 0] \concat \text{bits}(\fixedencode{1}{\len{v}})_{2\dots} \concat \text{bits}(k) \concat \text{bits}(v) \concat [0, 0, \dots] &\when \len{v} \le 32\\
