...
\begin{equation}
  T(\thestate) \equiv \left\{\begin{aligned}
    ...
    &&C(4) &\mapsto \se\left(\tup{\gamma_\mathbf{k}, \gamma_z, \left\{\,\begin{aligned}
      ...
      1\ &\when \gamma_\mathbf{s} \in \seq{\hash_B}_\mathsf{E}\\
...
\begin{equation}
  T\colon\left\{\begin{aligned}
    (\seq{\blob_n}, \N_{|\mathbf{v}|}, \blob \to \hash)\ &\to \seq{\blob_n \cup \hash}\\
...
\begin{align}
  ...
  \mathcal{J}_x&\colon \left\{\begin{aligned}
    (\seq{\blob}, \N_{|\mathbf{v}|}, \blob \to \hash) &\to \seq{\hash}\\
...
We define a Merkle Mountain Range as being within the set $\seq{\hash?}$, a sequence of peaks, each peak the root of a Merkle tree containing $2^i$ items where $i$ is the index in the sequence. Since we support set sizes which are not always powers-of-two-minus-one, some peaks may be empty, $\none$ rather than a Merkle root.
...
\begin{equation}
  \begin{aligned}
    ...
    \mathcal{A}&\colon\deffunc{
      (\seq{\hash?}, \hash, \blob\to\hash) &\to \seq{\hash?}\\
      (\mathbf{r}, l, H) &\mapsto P(\mathbf{r}, l, 0, H)
    }\\
    \where P&\colon\deffunc{
      (\seq{\hash?}, \hash, \N, \blob\to\hash) &\to \seq{\hash?}\\
...
\begin{equation}
  \mathcal{M}_R\colon\deffunc{
    ...
    \mathbf{b} &\mapsto \begin{cases}
      \zerohash &\when |\mathbf{h}| = 0\\
      \mathbf{h}_0 &\when |\mathbf{h}| = 1\\
      \keccak(\token{\$peak} \concat \mathcal{M}_R(\mathbf{h}_{\dots|\mathbf{h}|-1}) \concat \mathbf{h}_{|\mathbf{h}|-1}) &\otherwise \\
