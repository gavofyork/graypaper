...
\begin{equation}
  T(\thestate) \equiv \left\{\begin{aligned}
    ...
    &&C(3) &\mapsto \se(\var{[(h, b, s, \var{\mathbf{p}})\mid \tup{h, b, s, \mathbf{p}} \orderedin \recenthistory]}, \se_M(\beefybelt)) \;, \\
    ...
    \forall (s \mapsto \mathbf{a}) \in \accounts, (\mathbf{k} \mapsto \mathbf{v}) \in \mathbf{a}_\mathbf{s}: &&C(s, \se_4(2^{32}-1) \frown \mathbf{k}) &\mapsto \mathbf{v} \;, \\
    \forall (s \mapsto \mathbf{a}) \in \accounts, (h \mapsto \mathbf{p}) \in \mathbf{a}_\mathbf{p}: &&C(s, \se_4(2^{32}-2) \frown h) &\mapsto \mathbf{p} \;, \\
...
With $T$ defined, we now define the rest of $\mathcal{M}_\thestate$ which primarily involves transforming the serialized mapping into a cryptographic commitment. We define this commitment as the root of the binary Patricia Merkle Trie with a format optimized for modern compute hardware, primarily by optimizing sizes to fit succinctly into typical memory layouts and reducing the need for unpredictable branching.
...
\begin{align}
  ...
  L&\colon\left\{\begin{aligned}
    ...
    (k, v) &\mapsto \begin{cases}
      ...
      [1, 1, 0, 0, 0, 0, 0, 0] \frown \text{bits}(k) \frown \text{bits}(\blake(v)) &\otherwise
    \end{cases}
  \end{aligned}\right.
\end{align}

We may then define the basic Merklization function $\mathcal{M}_\thestate$ as:
\begin{align}
  \mathcal{M}_\thestate(\thestate) &\equiv M(\{ (\text{bits}(k) \mapsto \tup{k, v}) \mid (k \mapsto v) \in T(\thestate) \})\\
  M(d: \dict{\mathbb{B}}{(\blob_{31}, \blob)}) &\equiv \begin{cases}
    \H^0 &\when |d| = 0\\
    \blake(\text{bits}^{-1}(L(k, v))) &\when \mathcal{V}(d) = \{ (k, v) \}\\
...
\begin{equation}
  N\colon\left\{\begin{aligned}
    (\seq{\blob_n}, \blob \to \H) &\to \blob_n \cup \H \\
...
\begin{equation}
  T\colon\left\{\begin{aligned}
    (\seq{\blob_n}, \N_{|\mathbf{v}|}, \blob \to \H)\ &\to \seq{\blob_n \cup \H}\\
...
\begin{align}
  ...
  \mathcal{J}_x&\colon \left\{\begin{aligned}
    (\seq{\blob}, \N_{|\mathbf{v}|}, \blob \to \H) &\to \seq{\H}\\
...
\begin{equation}
  \mathcal{M}_R\colon\deffunc{
    ...
    \mathbf{b} &\mapsto \begin{cases}
      ...
      \blake_K(\token{\$peak} \concat \mathcal{M}_R(\mathbf{h}_{\dots|\mathbf{h}|-1}) \concat \mathbf{h}_{|\mathbf{h}|-1}) &\otherwise \\
