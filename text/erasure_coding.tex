...
The foundation of the data-availability and distribution system of \Jam is a systematic Reed-Solomon erasure coding function in \textsc{gf}($2^{16}$) of rate $\Cecoriginalshardsize$:$\Cvalcount$, the same transform as done by the algorithm of \cite{lin2014novel}. We use a little-endian $\blob[2]$ form of the 16-bit \textsc{gf} points with a functional equivalence given by $\fnencode[2]$. From this we may assume the encoding function $\fnerasurecode: \sequence[\nicefrac{\Cecpiecesize}{2}]{\blob[2]} \to \sequence[\Cvalcount]{\blob[2]}$ and the recovery function $\fnecrecover: \protoset{\tuple{\blob[2], \Nmax{\Cvalcount}}}_{\nicefrac{\Cecpiecesize}{2}} \to \sequence[\nicefrac{\Cecpiecesize}{2}]{\blob[2]}$. Encoding is done by extrapolating a data blob of size $\Cecpiecesize$ octets (provided in $\fnerasurecode$ here as $\Cecoriginalshardsize$ octet pairs) into $\Cvalcount$ octet pairs. Recovery is done by collecting together any distinct $\Cecoriginalshardsize$ octet pairs, together with their indices, and transforming this into the original sequence of $\Cecoriginalshardsize$ octet pairs.
...
We assume some data blob $\mathbf{d} \in \blob[\Cecpiecesize{}k], k \in \N$. This blob is split into a whole number of $k$ pieces, each a sequence of $\Cecoriginalshardsize$ octet pairs. Each piece is erasure-coded using $\fnerasurecode$ as above to give $\Cvalcount$ octet pairs per piece.

The resulting matrix is grouped by its pair-index and concatenated to form $\Cvalcount$ \emph{chunks}, each of $k$ octet-pairs. Any $\Cecoriginalshardsize$ of these chunks may then be used to reconstruct the original data $\mathbf{d}$.
...
The original data may be reconstructed with any $\Cecoriginalshardsize$ of the $\Cvalcount$ resultant items (along with their indices). If the original $\Cecoriginalshardsize$ items are known then reconstruction is just their concatenation.
\begin{equation}
  \label{eq:erasurecodinginv}
  \fnecrecover_{k \in \N}\colon\abracegroup{
    \protoset{\tuple{\blob[2k], \Nmax{\Cvalcount}}}_{\nicefrac{\Cecpiecesize}{2}} &\to \blob[\Cecpiecesize{}k] \\
    \mathbf{c} &\mapsto \begin{cases}
      \encode{\sq{\build{\mathbf{x}}{\tup{\mathbf{x}, i} \orderedin \sqorderby{i}{\tup{\mathbf{x}, i} \in \mathbf{c}}}}} &\when \set{\build{i}{\tup{\mathbf{x}, i} \in \mathbf{c}}} = \Nmax{\nicefrac{\Cecpiecesize}{2}}\\
      \join(\join^\#({}^\text{T}\sq{
        \build{
          \ecrecover{{\set{\build{
