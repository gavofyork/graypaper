...
We assume some data blob $\mathbf{d} \in \blob[\Cecpiecesize{}k], k \in \N$. This blob is split into a whole number of $k$ pieces, each a sequence of $\nicefrac{\Cecpiecesize}{2}$ octet pairs. Each piece is erasure-coded using $\fnerasurecode$ as above to give $\Cvalcount$ octet pairs per piece.

The resulting matrix is grouped by its pair-index and concatenated to form $\Cvalcount$ \emph{chunks}, each of $k$ octet-pairs. Any $\nicefrac{\Cecpiecesize}{2}$ of these chunks may then be used to reconstruct the original data $\mathbf{d}$.
...
The original data may be reconstructed with any $\nicefrac{\Cecpiecesize}{2}$ of the $\Cvalcount$ resultant items (along with their indices). If the original $\nicefrac{\Cecpiecesize}{2}$ items are known then reconstruction is just their concatenation.
\begin{equation}
