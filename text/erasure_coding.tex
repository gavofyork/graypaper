...
\begin{equation}
  ...
  \fnecrecover_{k \in \N}\colon\abracegroup{
    \protoset{\tuple{\blob[2k], \Nmax{\Cvalcount}}}_{\Cecoriginalshards} &\to \blob[\Cecpiecesize{}k] \\
    \mathbf{c} &\mapsto \begin{cases}
      \encode{\sq{\build{\mathbf{x}}{\tup{\mathbf{x}, i} \orderedin \sqorderby{i}{\tup{\mathbf{x}, i} \in \mathbf{c}}}}} &\when \set{\build{i}{\tup{\mathbf{x}, i} \in \mathbf{c}}} = \Nmax{\Cecoriginalshards}\\
      \join(\join^\#({}^\text{T}\sq{
        \build{
          \ecrecover{{\set{\build{
