...
We name the generator of $\frac{\finitefield_{16}}{\finitefield_2}$, the root of the above polynomial, $\authpool$ as such: $\finitefield_{16} = \finitefield_2(\authpool)$.

Instead of using the standard basis $\set{1, \authpool, \authpool^2, \dots, \authpool^{15}}$, we opt for a representation of $\finitefield_{16}$ which performs more efficiently for the encoding and the decoding process. To that aim, we name this specific representation of $\finitefield_{16}$ as $\tilde{\finitefield}_{16}$ and define it as a vector space generated by the following Cantor basis:
