...
\begin{align}
  ...
  \label{eq:extrinsic}\mathbf{E} &\equiv (\mathbf{E}_T, \mathbf{E}_V, \mathbf{E}_P, \mathbf{E}_A, \mathbf{E}_G)
\end{align}
...
\begin{description}
  ...
  \item[judgements] Votes, by validators, on dispute(s) arising between them presently taking place. This is denoted $\mathbf{E}_V$.
...
\begin{align}\label{eq:transitionfunctioncomposition}
  ...
  \psi' &\prec (\mathbf{E}_V, \psi) \\
  \delta^\dagger &\prec (\mathbf{E}_P, \delta, \tau') \label{eq:deltadagger} \\
  \rho^\dagger &\prec (\mathbf{E}_V, \rho) \label{eq:rhodagger} \\
