...
\begin{align}\label{eq:transitionfunctioncomposition}
  ...
  \mathbf{W}^* &\prec (\xtassurances, \reports') \\
  (\ready', \accumulated', \accountspostxfer, \privileges', \stagingset', \authqueue', \lastaccout', \mathbf{I}, \mathbf{X}) &\prec (\mathbf{W}^*, \ready, \accumulated, \accountspre, \privileges, \stagingset, \authqueue, \thetime, \thetime') \label{eq:accountspostxfer} \\
  ...
  \activity' &\prec (\xtguarantees, \xtpreimages, \xtassurances, \xttickets, \thetime, \activeset', \activity, \theheader, \mathbf{I}, \mathbf{X})\!\!\!\!\!\!\!\!
\end{align}
...
\begin{equation}
  ...
  \tuple{\,
    \begin{aligned}
      &\{\halt, \panic, \oog\} \cup \{\fault,\host\} \times \pvmreg,\\
...
\begin{align}\label{eq:pvmmemory}
  \ram &\equiv \tuple{
    ...
    \isa{\ram¬access}{\seq{\{\text{W}, \text{R}, \none\}}_p}
...
\begin{align}
  \readable{\memory} &\equiv \{i \mid \memory_\ram¬access[\floor{\nicefrac{i}{\mathsf{Z}_P}}] \ne \none \} \\
  \writable{\memory} &\equiv \{i \mid \memory_\ram¬access[\floor{\nicefrac{i}{\mathsf{Z}_P}}] = \text{W} \}
\end{align}
