...
\begin{align}\label{eq:statecomposition}
  \sigma &\equiv (\alpha, \recenthistory, \gamma, \delta, \eta, \iota, \kappa, \lambda, \rho, \tau, \varphi, \chi, \psi, \pi, \ready, \accumulated)
\end{align}
...
\begin{align}\label{eq:transitionfunctioncomposition}
  ...
  \mathbf{W}^* &\prec (\xtassurances, \rho^\dagger) \\
  (\ready', \accumulated', \accountspostxfer, \chi', \iota', \varphi', \lastaccout', \mathbf{I}, \mathbf{X}) &\prec (\mathbf{W}^*, \ready, \accumulated, \accountspre, \chi, \iota, \varphi, \tau, \tau') \label{eq:accountspostxfer} \\
