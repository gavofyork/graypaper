...
\begin{align}
  \local¬tranche\sub{0} &= \set{\build{\wdcX}{\wdcX \in \mathbf{p}_{\dots+10}, \wdX \ne \none}} \\
  \where \mathbf{p} &= \Ffyshuffle{\sq{\build{\tup{\cX, \local¬reports\sub{\cX}}}{\cX \orderedin \coreindex}}, r} \\
  \also r &= \Fbanderout{\local¬seed\sub{0}}
\end{align}
...
%  \forall (\cX, \wdX) &\in \local¬tranche\sub{0} : v \in q\sub{0}(\wdX)
  % TODO: ^^^ Fix this.
\end{align}
...
\begin{equation}
  \begin{aligned}
  %  \forall (\cX, \wdX) \in \local¬tranche\sub{n} \!: e\sub{n}(\wdX) \!\Leftrightarrow\! \begin{cases}
...
%We denote the guarantors of a work-package $\wdX$ as $\guarantorassignments(\wdX)$ from a particular chain head, and this may be derived from the guarantees extrinsic $\xtguarantees$ of the ancestor block which introduced the work-report. Our restrictions on the guarantees extrinsic (described in section \ref{sec:accumulation}) ensures that this is unique.
%\begin{equation}
%\begin{aligned}
%    \guarantorassignments(\wdX) &\equiv [ v \mid (s, v) \orderedin a ] \\
%    \where \tup{\cX, \wdX, a} &\in \xtguarantees \\
