...
\newcommand*{\auditreports}{\mathbf{q}}

We define the sequence of work-reports which we may be required to audit as $\auditreports$, a sequence of length equal to the number of cores, which functions as a mapping of core index to a work-report pending which has just become available, or $\none$ if no report became available on the core. Formally:
\begin{align}\label{eq:auditselection}
  \auditreports &\in \seq{\optional{\workreport}}_\corecount \\
  \auditreports &\equiv \sq{\build{
    \begin{rcases}
      \reports\subb{c}_\rs¬workreport &\when \reports\subb{c}_\rs¬workreport \in \justbecameavailable \\
      \none &\otherwise
    \end{rcases}
  }{
...
\begin{align}
  s_0 &\in \bandersig{\activeset[v]_\vk¬bs}{\mathsf{X}_U \concat \banderout(\H_\¬vrfsig)}{[]} \\
...
\begin{align}
  \mathbf{a}_0 &= \set{\build{\tup{c, w}}{\tup{c, w} \in \mathbf{p}_{\dots+10}, w \ne \none}} \\
  \where \mathbf{p} &= \Ffyshuffle{\sq{\build{\tup{c, \auditreports_c}}{c \orderedin \coreindex}}, r} \\
  \also r &= \Fbanderout{s_0}
\end{align}
...
\begin{align}
  S &\equiv \sig{\activeset[v]_\vk¬ed}{\mathsf{X}_I \append n \concat \mathbf{x}_n \concat \blake(\H)} \\
  \where \mathbf{x}_n &= \se([\se_2(c) \concat \blake(w) \mid \tup{c, w} \in \mathbf{a}_n])\\
...
\begin{align}
  A_n: \workreport &\to \protoset{\valindex} \\
...
\begin{align}
  \nonumber\forall n > 0:&\\
  \ s_n(w) &\in \bandersig{\activeset[v]_\vk¬bs}{\mathsf{X}_U \concat \Fbanderout{\H_\¬vrfsig}\concat\blake(w)\append n}{[]} \\
  \ \mathbf{a}_n &\equiv \set{ \build{ w }{\textstyle\frac{\valcount}{256\mathsf{F}}\Fbanderout{s_n(w)}_0 < m_n, w \in \auditreports, w \ne \none }}\!\!\!\!\\
  \nonumber \where m_n &= \len{A_{n - 1}(w) \setminus J_\top(w)}
\end{align}
...
\begin{equation}
  ...
  %    w = \computedigest(p, c)\!\!\!\!\! &\when \exists p \in \workpackage: \se(p) = F(w) \\
  %    \bot &\otherwise
  %  \end{cases}
...
\begin{align}
  \mathbf{j}_n &= \set{\build{\signdata{\activeset[v]_\vk¬ed}{\mathsf{X}_{e(w)} \concat \blake(w)}}{(c, w) \in \mathbf{a}_n}}
\end{align}

%We denote the guarantors of a work-package $w$ as $\guarantorassignments(w)$ from a particular chain head, and this may be derived from the guarantees extrinsic $\xtguarantees$ of the ancestor block which introduced the work-report. Our restrictions on the guarantees extrinsic (described in section \ref{sec:accumulation}) ensures that this is unique.
%\begin{equation}
%\begin{aligned}
%    \guarantorassignments(w) &\equiv [ v \mid (s, v) \orderedin a ] \\
%    \where (c, w, a) &\in \xtguarantees \\
%    (\H, \mathbf{E}) &\in \ancestors
%\end{aligned}
%\end{equation}
...
\begin{align}
%  U(w) &\Leftrightarrow J_\bot(w) = \emptyset \wedge \exists n : A_n(w) \subset J_\top(w) \vee |J_\top(w)| > \twothirds\valcount%\!\!\!\!\!\!\!\!\!\!\!\!\!\!
  U(w) &\Leftrightarrow \bigvee\,\abracegroup[\,]{
    &J_\bot(w) = \emptyset \wedge \exists n : A_n(w) \subset J_\top(w) \\
    &|J_\top(w)| > \twothirds\valcount
  }
\end{align}
...
\begin{align}
  \isaudited &\Leftrightarrow \forall w \in \justbecameavailable : U(w)
\end{align}
