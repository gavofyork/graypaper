...
\begin{align}\label{eq:auditselection}
  \local¬reports &\in \seq{\optional{\workreport}}_\corecount \\
...
\begin{align}
  \local¬seed\sub{0} &\in \bandersig{\activeset[v]_\vk¬bs}{\mathsf{X}_U \concat \Fbanderout{\H_\¬vrfsig}}{\sq{}} \\
...
\begin{align}
  \local¬tranche\sub{0} &= \set{\build{\wrcX}{\wrcX \in \mathbf{p}_{\dots+10}, \wrX \ne \none}} \\
  \where \mathbf{p} &= \Ffyshuffle{\sq{\build{\tup{\cX, \local¬reports\sub{\cX}}}{\cX \orderedin \coreindex}}, \Fbanderout{\local¬seed\sub{0}}} \\
\end{align}

Every $\mathsf{A} = 8$ seconds following a new time slot, a new tranche begins, and we may determine that additional cores warrant an audit from us. Such items are defined as $\local¬tranche\sub{n}$ where $n$ is the current tranche. Formally:
\begin{equation}
...
\begin{align}
  ...
  \where \mathbf{x}\sub{n} &= \se(\set{\build{\se_2(\cX) \concat \blake(\wrX)}{\wrcX \in \local¬tranche\sub{n}}})\\
...
%  \forall (\cX, \wrX) &\in \local¬tranche\sub{0} : v \in q\sub{0}(\wrX)
  % TODO: ^^^ Fix this.
\end{align}
...
\begin{align}
  \nonumber\forall n > 0:&\\
  \ \local¬seed\sub{n}(\wrX) &\in \bandersig{\activeset[v]_\vk¬bs}{\mathsf{X}_U \concat \Fbanderout{\H_\¬vrfsig}\concat\blake(\wrX)\append n}{[]} \\
  \ \local¬tranche\sub{n} &\equiv \set{ \build{ \wrX }{\textstyle\frac{\valcount}{256\mathsf{F}}\Fbanderout{\local¬seed\sub{n}(\wrX)}\sub{0} < m\sub{n}, \wrX \in \local¬reports, \wrX \ne \none }}\!\!\!\!\\
...
\begin{equation}
  \begin{aligned}
  %  \forall (\cX, \wrX) \in \local¬tranche\sub{n} \!: e\sub{n}(\wrX) \!\Leftrightarrow\! \begin{cases}
  %    \wrX = \computereport(p, \cX)\!\!\!\!\! &\when \exists p \in \workpackage: \se(p) = F(\wrX) \\
  %    \bot &\otherwise
  %  \end{cases}
    \forall (\cX, \wrX) \in \local¬tranche\sub{n} :\ \ &\\[-10pt]
...
\begin{align}
  \mathbf{j}\sub{n} &= \set{\build{\Fsigndata{\activeset\subb{v}_\vk¬ed}{\mathsf{X}_{e\sub{n}(\cX)} \concat \blake(\wrX)}}{(\cX, \wrX) \in \local¬tranche\sub{n}}}
\end{align}
...
%\begin{equation}
%\begin{aligned}
%    \guarantorassignments(\wrX) &\equiv [ v \mid (s, v) \orderedin a ] \\
%    \where \tup{\cX, \wrX, a} &\in \xtguarantees \\
%    \tup{\H, \mathbf{E}} &\in \ancestors
%\end{aligned}
%\end{equation}
