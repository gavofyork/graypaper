...
\begin{align}
  ...
  \where \mathbf{p} &= \mathcal{F}([\tup{c, \mathbf{Q}_c} \mid c \orderedin \coreindex], r) \\
...
\begin{align}
  S &\equiv \sig{\activeset[v]_e}{\mathsf{X}_I \doubleplus n \concat \mathbf{x}_n \concat \blake(\H)} \\
...
\begin{align}
  \nonumber\forall n > 0:&\\
  \ s_n(w) &\in \bandersig{\activeset[v]_b}{\mathsf{X}_U \concat \banderout{\H_\¬vrfsig}\concat\blake(w)\doubleplus n}{[]} \\
...
\begin{align}
  \mathbf{j}_n &= \{ \mathcal{S}_{\activeset[v]_e}(\mathsf{X}_{e(w)} \concat \blake(w)) \mid (c, w) \in \mathbf{a}_n \}
\end{align}
