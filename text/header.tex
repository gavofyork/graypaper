...
\begin{equation}
  \mathbf{H}_p \in \H \,,\quad \mathbf{H}_p \equiv \blake(\se(P(\mathbf{H})))
\end{equation}
...
\begin{align}
  ...
  \mathbf{H}_x \equiv \blake(\se(\blake^\#(\mathbf{a}))) \\
...
A block may only be regarded as valid once the time-slot index $\h¬timeslot$ is in the past. It is always strictly greater than that of its parent. Formally:
\begin{equation}
  \h¬timeslot \in \N_T \,,\quad
  P(\mathbf{H})_t < \h¬timeslot\ \wedge\ \h¬timeslot\cdot\mathsf{P} \leq \mathcal{T}
\end{equation}
...
\begin{equation}
  \mathbf{H}_r \in \H \,,\quad \mathbf{H}_r \equiv \mathcal{M}_\thestate(\thestate)
\end{equation}

We assume the state-Merklization function $\mathcal{M}_\thestate$ is capable of transforming our state $\thestate$ into a 32-octet commitment. See appendix \ref{sec:statemerklization} for a full definition of these two functions.
