...
The initial state $\gas¬sim_0$ of the simulation is defined as follows:

\begin{equation}
  \gas¬sim_0(\imath) = \abracegroup{
    \imath^{(0)} &= \imath \\
    \ctr¬cycles^{(0)} &= 0,
...
\begin{equation}
  \gas¬sim_{n + 1}(\gas¬sim_n, \mathbf{c}, \mathbf{k}) = \begin{cases}
    \gas¬sim_{n + 1}'(\gas¬sim_n, \mathbf{c}, \mathbf{k}) &\when n = 0 \lor (\imath^{(n)} \neq \none \land \cost¬decodeslots(\mathbf{c}, \mathbf{k}, \imath^{(n)}) \leq \remaining¬decodeslots^{(n)} \land |\rob¬state^{(n)}| < 32) \\
    \gas¬sim_{n + 1}''(\gas¬sim_n) &\when \find¬readytostart(\gas¬sim_n) \neq \none \land \remaining¬startpercycle^{(n)} > 0 \\
    \none &\when \imath^{(n)} = \none \land |\rob¬state^{(n)}| = 0 \\
    \gas¬sim_{n + 1}'''(\gas¬sim_n) &\otherwise \\
  \end{cases}
\end{equation}

Assuming $m \text{ such that } \gas¬sim_{m + 1} = \none$ the final gas cost of a basic block starting at $\imath$ is defined as:

\begin{equation}\label{eq:gascostforblock}
  \gascostforblock{\imath} = \max(\ctr¬cycles^{(m)} - 3, 1)
\end{equation}

The state transition function $\gas¬sim_{n + 1}'$ which decodes the instructions into a reorder buffer without triggering the virtual CPU pipeline simulation is defined as follows (those pieces of state which are not explicitly mentioned by the equations are assumed to be unchanged and omitted for clarity):

\begin{equation}
  \gas¬sim_{n + 1}'(\gas¬sim_n, \mathbf{c}, \mathbf{k}) = \begin{cases}
    \gas¬sim_{n + 1}^{\text{mov}}(\gas¬sim_n, \mathbf{c}, \mathbf{k}) &\when \text{opcode}(\mathbf{c}, \mathbf{k}, \imath^{(n)}) = \token{move\_reg} \\
    \gas¬sim_{n + 1}^{\text{decode}}(\gas¬sim_n, \mathbf{c}, \mathbf{k}) &\otherwise
  \end{cases}
\end{equation}
...
\begin{equation}
  \gas¬sim_{n + 1}^{\text{mov}}(\gas¬sim_n, \mathbf{c}, \mathbf{k}) = \abracegroup{
...
\begin{equation}
  \gas¬sim_{n + 1}^{\text{decode}}(\gas¬sim_n, \mathbf{c}, \mathbf{k}) = \abracegroup{
...
The state transition function $\gas¬sim_{n + 1}''$ which starts the execution of the next pending instruction is defined as follows:

\begin{equation}
  \gas¬sim_{n + 1}''(\gas¬sim_n) = \abracegroup{
    \rob¬state^{(n + 1)}_{\find¬readytostart(\gas¬sim_n)} &= \state¬executing \\
    \remaining¬execunits^{(n + 1)} &= \remaining¬execunits^{(n)} - \rob¬execunits^{(n)}_{\find¬readytostart(\gas¬sim_n)} \\
    \remaining¬startpercycle^{(n + 1)} &= \remaining¬startpercycle^{(n)} - 1
  }
\end{equation}

The function $\find¬readytostart(\gas¬sim_n)$ which checks which instruction inside of the reorder buffer is ready to start executing (and whether such an instruction even exists) is defined as follows:

\begin{equation}
  \find¬readytostart(\gas¬sim_n) = \min(j \in \mathbb{N} \mid \rob¬state^{(n)}_j = \state¬waiting \land \rob¬execunits^{(n)}_j \leq \remaining¬execunits^{(n)} \land (\forall k \in \rob¬deps^{(n)}_j \Rightarrow \rob¬cyclesleft^{(n)}_k \leq 0))
\end{equation}

The state transition function $\gas¬sim_{n + 1}'''$ which simulates the rest of the virtual CPU pipeline is defined as follows:
