...
\begin{equation}
  \Psi\colon \left\{\begin{aligned}
    ...
    (\mathbf{p}, \imath, \theta, \varrho, \mem) &\mapsto \begin{cases}
      \Psi(\mathbf{p}, \imath', \theta', \varrho', \mem') &\when \varepsilon = \continue\\
      (\oog, \imath', \theta', \varrho', \mem') &\when \theta' < 0\\
      (\varepsilon, \imath', \theta', \varrho', \mem') &\otherwise
    \end{cases} \\
    \where (\varepsilon, \imath', \theta', \varrho', \mem') &= \Psi_1(\mathbf{c}, \mathbf{k}, \mathbf{j}, \imath, \theta, \varrho, \mem)\\
...
If the latter condition cannot be satisfied, then $(\panic, \imath, \theta, \varrho, \mem)$ is the result.
...
\begin{equation}
  \Psi_1\colon \left\{\begin{aligned}
    ...
    (\mathbf{c}, \mathbf{k}, \mathbf{j}, \imath, \theta, \varrho, \mem) &\mapsto (\varepsilon, \imath', \theta', \varrho', \mem')
  \end{aligned}\right.
\end{equation}
...
%We alias the registers $\varrho$ to their \textsc{risc-v} specification name:
%\begin{equation}
%  (\RA, \SP, \T_0, \T_1, \T_2, \S_0, \S_1, \A_0, \A_1, \A_2, \A_3, \A_4, \A_5) \equiv \varrho,\qquad
%  (\RA', \SP', \T_0', \T_1', \T_2', \S_0', \S_1', \A_0', \A_1', \A_2', \A_3', \A_4', \A_5') \equiv \varrho'
%\end{equation}
...
\begin{equation}
  \varepsilon = \continue,\quad \imath' = \imath + 1 + \Fskip(\imath),\quad \theta' = \theta - \gas,\quad \varrho' = \varrho,\quad\mem' = \mem \text{ except as indicated }
\end{equation}
...
\begin{equation}
  \Psi_H\colon \left\{\begin{aligned}
    ...
    (\mathbf{c}, \imath, \theta, \varrho, \mem, f, \mathbf{x}) &\mapsto \begin{cases}
      (\fault \times a, \imath', \theta', \varrho', \mem', \mathbf{x}) &\when \bigwedge\left\{\;\begin{aligned}
        &\varepsilon = \host \times h\\[2pt]
        &\fault \times a = f(h, \theta', \varrho', \mem', \mathbf{x})\\[2pt]
      \end{aligned}\right.\\[8 pt]
      \Psi_H(\mathbf{c}, \imath' + 1 + \Fskip(\imath'), \theta'', \varrho'', \mem'', f, \mathbf{x}'') &\when \bigwedge\left\{\;\begin{aligned}
        &\varepsilon = \host \times h\\[2pt]
        &(\theta'', \varrho'', \mem'', \mathbf{x}'') = f(h, \theta', \varrho', \mem', \mathbf{x})
      \end{aligned}\right.\\[8pt]
      (\varepsilon, \imath', \theta', \varrho', \mem', \mathbf{x}) &\otherwise
    \end{cases} \\
    \where (\varepsilon, \imath', \theta', \varrho', \mem') &= \Psi(\mathbf{c}, \imath, \theta, \varrho, \mem)\\
    \end{aligned}\right.
\end{equation}
...
\end{align}
If the above cannot be satisfied, then $x = \none$, otherwise $x = (\mathbf{c}, \varrho, \mem)$ with $\mathbf{c}$ as above and $\mem$, $\varrho$ such that:
...
\end{equation}
\begin{equation}
  \forall i \in \N_{16} : \varrho_i = \begin{cases}
      2^{32} - 2^{16} &\when i = 1\\
...
\begin{align}
  \Psi_M&\colon \left\{\begin{aligned}
    ...
    (\mathbf{p}, \imath, \theta, \mathbf{a}, f, \mathbf{x}) &\mapsto \begin{cases}
      (\panic, \mathbf{x}) &\when Y(\mathbf{p}) = \none\\
      R(\Psi_H(\mathbf{c}, \imath, \theta, \varrho, \mem, f, \mathbf{x})) &\when Y(\mathbf{p}) = (\mathbf{c}, \varrho, \mem)
    \end{cases}
  \end{aligned}\right.\\
  \where R&\colon (\varepsilon, \imath', \theta', \varrho', \mem', \mathbf{x}) \mapsto \begin{cases}
    (\oog, \mathbf{x}) &\when \varepsilon = \oog \\
    ((\theta', \mu'_{\varrho'_{10}\dots+\varrho'_{11}}), \mathbf{x}) &\when \varepsilon = \halt \wedge \mathbb{Z}_{\varrho'_{10}\dots+\varrho'_{11}} \subset \mathbb{V}_{\mem'} \\
    ((\theta', []), \mathbf{x}) &\when \varepsilon = \halt \wedge \mathbb{Z}_{\varrho'_{10}\dots+\varrho'_{11}} \not\subset \mathbb{V}_{\mem'} \\
