...
\begin{equation}
  \basicblocks \equiv \left(\set{0} \cup \set{\build{n + 1 + \Fskip(n)}{n \in \N_{\len{\mathbf{c}}} \wedge \mathbf{k}_n = 1 \wedge \mathbf{c}_n \in T}}\right) \cap \set{\build{n}{\mathbf{k}_n = 1 \wedge \mathbf{c}_n \in U}}
\end{equation}
...
\begin{equation}
  \Psi_1\colon \abracegroup{
    (\blob, \mathbb{B}, \seq{\pvmreg}, \pvmreg, \gas, \regs, \ram) &\to (\set{\panic, \halt, \continue } \cup \set{\fault, \host} \times \pvmreg, \pvmreg, \signedgas, \regs, \ram)\\
    ...
    (\mathbf{p}, \imath, \gascounter, \mathbf{a}, f, \mathbf{x}) &\mapsto \begin{cases}
      ...
      \multicolumn{2}{l}{
        ...
        \end{alignedat}
          ...
          (u, \memory'_{\registers'_{7}\dots+\registers'_{8}}, \mathbf{x}') &\when \varepsilon = \halt \wedge \N_{\registers'_{7}\dots+\registers'_{8}} \subseteq \readable{\mem'} \\
          (u, [], \mathbf{x}') &\when \varepsilon = \halt \wedge \N_{\registers'_{7}\dots+\registers'_{8}} \not\subseteq \readable{\mem'} \\
