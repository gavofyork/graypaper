...
\begin{align}
  ...
  \revbitsfunc{n\in\N}&\colon\left\{\begin{aligned}
    \N_{2^{8n}} &\to \mathbb{B}_{8n}\\
    x &\mapsto \mathbf{y}: \forall i \in \N_{2^{8n}} : \mathbf{y}[2^{8n} - 1 - i] \Leftrightarrow \ffrac{x}{2^i}\bmod 2
  \end{aligned}\right.\\
  \revunbitsfunc{n\in\N}&\colon\left\{\begin{aligned}
    \mathbb{B}_{8n} &\to \N_{2^{8n}}\\
    \mathbf{x} &\mapsto y: \sum_{i \in \N_{2^{8n}}} \mathbf{x}_{2^{8n} - 1 - i} \cdot 2^i
  \end{aligned}\right.
\end{align}
...
\begin{longtable}{p{8mm} p{35mm} p{5mm} p{100mm}}
  ...
  145&\token{shlo\_r\_imm\_alt\_32}&0&$\reg'_A = \sext_4(\floor{\immed_X \div 2^{\reg_B \bmod 32}})$\\ \mrule
  146&\token{shar\_r\_imm\_alt\_32}&0&$\reg'_A = \unsigned{\floor{\signedn{4}{\immed_X} \div 2^{\reg_B \bmod 32}}}$\\ \mrule
...
\begin{longtable}[t]{p{8mm} p{20mm} p{5mm} p{100mm}}
  ...
  219&\token{cmov\_nz}&0&$\reg'_D = \begin{cases}
    ...
    \reg_D &\otherwise
  \end{cases}$\\ \mrule
  220&\token{rot\_l\_64}&0&$\forall i \in \N_{64} : \bitsfunc{8}(\reg'_D)_i = \bitsfunc{8}(\reg_A)_{(i + \reg_B) \bmod 64}$\\ \mrule
  221&\token{rot\_l\_32}&0&$\reg'_D = \sext_4(x)\ \where x \in \N_{2^{32}}, \forall i \in \N_{32} : \bitsfunc{4}(x)_i = \bitsfunc{4}(\reg_A)_{(i + \reg_A) \bmod 32}$\\ \mrule
  222&\token{rot\_r\_64}&0&$\forall i \in \N_{64} : \bitsfunc{8}(\reg'_D)_{(i + \reg_B) \bmod 64} = \bitsfunc{8}(\reg_A)_i$\\ \mrule
  223&\token{rot\_r\_32}&0&$\reg'_D = \sext_4(x)\ \where x \in \N_{2^{32}}, \forall i \in \N_{32} : \bitsfunc{4}(x)_{(i + \reg_A) \bmod 32} = \bitsfunc{4}(\reg_A)_i$\\ \mrule
