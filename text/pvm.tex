...
Note, the term $h$ above refers to the beginning of the heap, the second major section of memory as defined in equation \ref{eq:memlayout} as $2\mathsf{Z}_J + Q(|\mathbf{o}|)$. If $\token{sbrk}$ instruction is invoked on a \textsc{pvm} instance which does not have such a memory layout, then $h = 0$.
...
\begin{align}\label{eq:conditions}
  ...
  &\mathsf{Z}_N = 2^{14}\ ,\quad\mathsf{Z}_J = 2^{16}\ ,\quad\mathsf{Z}_I = 2^{24}\\
  &\using \rnp{x \in \N} \equiv \mathsf{Z}_N\left\lceil \frac{x}{\mathsf{Z}_N} \right\rceil\quad,\qquad\rnq{x \in \N} \equiv \mathsf{Z}_J\left\lceil \frac{x}{\mathsf{Z}_J} \right\rceil\\
  &5\mathsf{Z}_J + Q(|\mathbf{o}|) + Q(|\mathbf{w}| + z\mathsf{Z}_N) + Q(s) + \mathsf{Z}_I \leq 2^{32}
\end{align}
...
\begin{equation}\label{eq:memlayout}
  \forall i \in \N_{2^{32}} : ((\mem_\mathbf{V})_i, (\mem_\mathbf{A})_{\floor{\nicefrac{i}{\mathsf{Z}_P}}}) = \left\{\begin{alignedat}{5}
    &\tup{\is{\mathbf{V}}{\mathbf{o}_{i - \mathsf{Z}_J}}\ts\is{\mathbf{A}}{R}} &&\ \when
        \mathsf{Z}_J
            &\ \leq i < \ &&
                \mathsf{Z}_J + |\mathbf{o}|\\
    &(0, R) &&\ \when
        \mathsf{Z}_J + |\mathbf{o}|
            &\ \leq i < \ &&
                \mathsf{Z}_J + \rnp{|\mathbf{o}|} \\
    &(\mathbf{w}_{i - (2\mathsf{Z}_J + \rnq{|\mathbf{o}|})}, W) &&\ \when
        2\mathsf{Z}_J + \rnq{|\mathbf{o}|}
            &\ \leq i < \ &&
                2\mathsf{Z}_J + \rnq{|\mathbf{o}|} + |\mathbf{w}|\\
    &(0, W) &&\ \when
        2\mathsf{Z}_J + \rnq{|\mathbf{o}|} + |\mathbf{w}|
            &\ \leq i < \ &&
                2\mathsf{Z}_J + \rnq{|\mathbf{o}|} + \rnp{|\mathbf{w}|} + z\mathsf{Z}_N\\
    &(0, W) &&\ \when
        2^{32} - 2\mathsf{Z}_J - \mathsf{Z}_I - \rnp{s}
            &\ \leq i < \ &&
                2^{32} - 2\mathsf{Z}_J - \mathsf{Z}_I\\
    &(\mathbf{a}_{i - (2^{32} - \mathsf{Z}_J - \mathsf{Z}_I)}, R) &&\ \when
        2^{32} - \mathsf{Z}_J - \mathsf{Z}_I
            &\ \leq i < \ &&
                2^{32} - \mathsf{Z}_J - \mathsf{Z}_I + |\mathbf{a}|\\
    &(0, R) &&\ \when
        2^{32} - \mathsf{Z}_J - \mathsf{Z}_I + |\mathbf{a}|
            &\ \leq i < \ &&
                2^{32} - \mathsf{Z}_J - \mathsf{Z}_I + \rnp{|\mathbf{a}|}\\
    &(0, \none) &&\otherwise&&&
  \end{alignedat}\right.\\
\end{equation}
\begin{equation}\label{eq:registers}
  \forall i \in \N_{13} : \registers_i = \begin{cases}
      2^{32} - 2^{16} &\when i = 0\\
      2^{32} - 2\mathsf{Z}_J - \mathsf{Z}_I &\when i = 1\\
      2^{32} - \mathsf{Z}_J - \mathsf{Z}_I &\when i = 7\\
      |\mathbf{a}|&\when i = 8\\
      0 &\otherwise
    \end{cases}
\end{equation}
