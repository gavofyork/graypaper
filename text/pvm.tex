...
\newcommand*{\signfunc}[1]{\mathcal{W}_{#1}}
...
\begin{equation}
  \Psi\colon \left\{\begin{aligned}
    ...
    \end{cases} \\
    \where (\zeta, \imath', \xi', \omega', \mem') &= \Psi_1(\mathbf{c}, \mathbf{k}, \mathbf{j}, \imath, \xi, \omega, \mem)\\
...
The latter, dynamic jump table, is a sequence of indices into the instruction data blob and is indexed into when dynamically-computed jumps are taken. It is encoded as a sequence of unsigned integers each encoded with the same length in octets. This length, term $z$ above, is itself encoded prior.
...
\begin{equation}
  \zeta = \continue,\quad \imath' = \imath + 1 + \Fskip(\imath),\quad \xi' = \xi - \gas,\quad \omega' = \omega,\quad\mem' = \mem \text{ except as indicated }
\end{equation}
...
In order to compactly encode signed integers, we use a zigzag encoding which gives larger quantities as the integer's absolute distance from zero increases. Small negative and small positive values are thus both encoded as slightly larger (but still small) positive integers. Since our encoding scheme allows for eliding any leading zeroes, we implicitly get compact signed encoding without having to do a signed-extension while decoding.

\newcommand{\zigzag}{\mathcal{Z}}
\newcommand{\unzz}{\mathcal{Z}^{-1}}
The Zigzag encoding function is defined as $\zigzag$:
\begin{align}\label{eq:zigzag}
  \zigzag\colon\left\{\begin{aligned}
    \N &\to \N\\
    n &\mapsto 2|n| - (n < 0)
  \end{aligned}\right.\\
  \unzz\colon\left\{\begin{aligned}
    \N &\to \N\\
    n &\mapsto \ceil{\frac{n}{2}}(1 - 2(n \bmod 2))
  \end{aligned}\right.
\end{align}

\subsection{Instruction Tables}\label{sec:instructiontables}

Note that in the case that the opcode is not defined in the following tables then the instruction is considered invalid, and it results in a panic; $\zeta=\panic$.
...
\begin{aligned}
  ...
  \immed_X \equiv \unzz(\de_{l_X}(\instructions_{\imath+1\dots+l_X}))
\end{aligned}
\end{equation}
...
\begin{aligned}
    ...
    \immed_X &\equiv \unzz(\de_{l_X}(\instructions_{\imath+2\dots+l_X})) \\
    \using l_Y &= \max(4, (\ell - l_X - 1) \bmod 2^8) \,,\quad&
    \immed_Y &\equiv \unzz(\de_{l_Y}(\instructions_{\imath+2+l_X\dots+l_Y}))
\end{aligned}
\end{equation}
...
\begin{aligned}
  \using l_X = \max(\ell, 4) \,,\quad
  \immed_X \equiv \imath + \unzz(\de_{l_X}(\instructions_{\imath+1\dots+l_X}))
\end{aligned}
\end{equation}
...
\begin{aligned}
    ...
    \immed_X &= \unzz(\de_{l_X}(\instructions_{\imath+2\dots+l_X})) \\
    \using l_Y &= \max(4, (\ell - l_X - 1) \bmod 2^8) \,,\quad&
    \immed_Y &= \unzz(\de_{l_Y}(\instructions_{\imath+2+l_X\dots+l_Y}))
\end{aligned}
\end{equation}
...
\begin{equation}
  \begin{aligned}
      ...
      \immed_X &= \unzz(\de_{l_X}(\instructions_{\imath+2\dots+l_X})) \\
      \using l_Y &= \max(4, (\ell - l_X - 1) \bmod 2^8) \,,\quad&
      \immed_Y &= \imath + \unzz(\de_{l_Y}(\instructions_{\imath+2+l_X\dots+l_Y}))
  \end{aligned}
\end{equation}
...
\begin{equation}
  \begin{aligned}
    ...
    \reg'_B \equiv \reg'_{r_B} \\
    \using l_X &= \max((\ell - 1) \bmod 2^8, 4) \,,\quad&
    \immed_X &\equiv \imath + \unzz(\de_{l_X}(\instructions_{\imath+2\dots+l_X}))
  \end{aligned}
\end{equation}
...
\begin{equation}
  \begin{aligned}
    ...
    \immed_X &= \unzz(\de_{l_X}(\instructions_{\imath+3\dots+l_X})) \\
    \using l_Y &= \max(4, (\ell - l_X - 2) \bmod 2^8) \,,\quad&
    \immed_Y &= \unzz(\de_{l_Y}(\instructions_{\imath+3+l_X\dots+l_Y}))
  \end{aligned}
\end{equation}
