...
\begin{longtable}[t]{p{8mm} p{20mm} p{5mm} p{100mm}}
  ...
  175&\token{rem\_u\_32}&0&$\reg'_D = \begin{cases}
    \reg_A &\when \reg_B \bmod 2^{32} = 0\\
    (\reg_A \bmod 2^{32}) \bmod (\reg_B \bmod 2^{32}) &\otherwise
  \end{cases}$\\ \mrule
  ...
  178&\token{shlo\_r\_32}&0&$\reg'_D = \floor{(\reg_A \bmod 2^{32}) \div 2^{\reg_B \bmod 32}}$\\ \mrule
