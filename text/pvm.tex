...
\newcommand*{\gascostforblock}[1]{\gascounter_{#1}^{\Delta}}
...
\begin{align}
  \Psi^*&\colon \abracegroup{
    ...
    \tup{\mathbf{p}, \imath, \gascounter, \gaschargedflag, \registers, \mem} &\mapsto \begin{cases}
      \Psi(\mathbf{p}, \imath', \gascounter', \gaschargedflag', \registers', \mem') &\when \varepsilon = \continue\\
    ...
    \where \tup{\varepsilon, \imath', \gascounter', \gaschargedflag', \registers', \mem'} &= \begin{cases}
      \Psi_1(\mathbf{c}, \mathbf{k}, \mathbf{j}, \imath, \gascounter, \gaschargedflag, \registers, \mem) &\when \tup{\mathbf{c}, \mathbf{k}, \mathbf{j}} = \deblob(\mathbf{p}) \\
      \tup{\panic, \imath, \gascounter, \gaschargedflag, \registers, \mem} &\otherwise
    \end{cases}
  }\\
  &\Psi(\mathbf{p}, \imath, \gascounter, \registers, \mem) \equiv \Psi^*(\mathbf{p}, \imath, \gascounter, \bot, \registers, \mem)\\
...
For convenience we will define $\startofbasicblock(\imath)$ which represents the start of a basic block for any $\imath$ within that basic block. Formally:

\begin{equation}
  \startofbasicblock(\imath) = \max(j \in \basicblocks \mid j \le \imath)
\end{equation}
...
\begin{equation}
  \Psi_1\colon \abracegroup{
    ...
    \tup{\mathbf{c}, \mathbf{k}, \mathbf{j}, \imath, \gascounter, \gaschargedflag, \registers, \mem} &\mapsto \tup{\varepsilon^*, \imath^*, \gascounter^*, \gaschargedflag^*, \registers^*, \mem^*}
  }
\end{equation}

On the very first step of execution, and every time the execution enters a new basic block or jumps back to the beginning of the current basic block, the gas counter of the machine is updated according to the gas cost $\gascostforblock{\startofbasicblock(\imath)}$~(\ref{eq:gascostforblock}) of the target basic block. No instruction is allowed to execute within a basic block without first paying the gas cost of the whole block. In case there's not enough gas remaining to cover the full gas cost, the execution is interrupted and the gas counter remains unchanged. Formally:

\begin{equation}
  \tup{\tilde{\varepsilon}, \gascounter', \gaschargedflag'} = \begin{cases}
    \tup{\continue, \gascounter, \top} &\when \gaschargedflag = \top \\
    \tup{\continue, \gascounter - \gascostforblock{\startofbasicblock(\imath)}, \top} &\otherwhen \gascounter \ge \gascostforblock{\startofbasicblock(\imath)} \\
...
\begin{longtable}{p{8mm} p{32mm} p{100mm}}
  ...
  102&\token{count\_set\_bits\_64}&$\displaystyle\reg'_D = \sum_{i = 0}^{63}\bitsfunc{8}(\reg_A)\sub{i}$\\ \mrule
  103&\token{count\_set\_bits\_32}&$\displaystyle\reg'_D = \sum_{i = 0}^{31}\bitsfunc{4}(\reg_A \bmod 2^{32})\sub{i}$\\ \mrule
  104&\token{leading\_zero\_bits\_64}&$\displaystyle\reg'_D = \max(n \in \Nmax{65})\ \where \sum_{i = 0}^{i < n} \revbitsfunc{8}(\reg_A)\sub{i} = 0$\\ \mrule
  105&\token{leading\_zero\_bits\_32}&$\displaystyle\reg'_D = \max(n \in \Nmax{33})\ \where \sum_{i = 0}^{i < n} \revbitsfunc{4}(\reg_A \bmod 2^{32})\sub{i} = 0$\\ \mrule
  106&\token{trailing\_zero\_bits\_64}&$\displaystyle\reg'_D = \max(n \in \Nmax{65})\ \where \sum_{i = 0}^{i < n} \bitsfunc{8}(\reg_A)\sub{i} = 0$\\ \mrule
  107&\token{trailing\_zero\_bits\_32}&$\displaystyle\reg'_D = \max(n \in \Nmax{33})\ \where \sum_{i = 0}^{i < n} \bitsfunc{4}(\reg_A \bmod 2^{32})\sub{i} = 0$\\ \mrule
  108&\token{sign\_extend\_8}&$\reg'_D = \unsigned{\signedn{1}{\reg_A \bmod 2^8}}$\\ \mrule
  109&\token{sign\_extend\_16}&$\reg'_D = \unsigned{\signedn{2}{\reg_A \bmod 2^{16}}}$\\ \mrule
  110&\token{zero\_extend\_16}&$\reg'_D = \reg_A \bmod 2^{16}$\\ \mrule
  111&\token{reverse\_bytes}&$\forall i \in \N_8 : \encode[8]{\reg'_D}\sub{i} = \encode[8]{\reg_A}_{7-i}$\\
...
\begin{align}
  &\Psi_H^*\colon \abracegroup{
    \tuple{\begin{aligned}
      &\blob, \pvmreg, \gas, \regs,\\&\ram, \bool, \contextmutator{X}, X
    \end{aligned}
    }
    &\to
    \tuple{\set{\panic, \oog, \halt} \cup \set{\fault} \times \pvmreg, \pvmreg, \signedgas, \bool, \regs, \ram, X}\\
    \tup{\mathbf{c}, \imath, \gascounter, \gaschargedflag, \registers, \mem, f, \mathbf{x}} &\mapsto \begin{cases}
      \multicolumn{2}{l}{\text{let }(\varepsilon', \imath', \gascounter', \gaschargedflag', \registers', \mem') = \Psi(\mathbf{c}, \imath, \gascounter, \gaschargedflag, \registers, \mem):} \\[8pt]
      ...
      \begin{aligned}
        &\Psi_H^*(\mathbf{c}, \imath'', \gascounter'', \gaschargedflag', \registers'', \mem'', f, \mathbf{x}'')\\[2pt]
    ...
    &\Psi_H(\mathbf{c}, \imath, \gascounter, \registers, \mem, f, \mathbf{x}) \equiv \Psi_H^*(\mathbf{c}, \imath, \gascounter, \bot, \registers, \mem, f, \mathbf{x})\\
...
\begin{equation}
  \Psi_M\colon \abracegroup{
    ...
    \tup{\mathbf{p}, \imath, \gascounter, \mathbf{a}, f, \mathbf{x}} &\mapsto \begin{cases}
      ...
      R(\gascounter, \Psi_H(\mathbf{c}, \imath, \gascounter, \bot, \registers, \mem, f, \mathbf{x})) &\when Y(\mathbf{p}, \mathbf{a}) = \tup{\mathbf{c}, \registers, \mem}\\
      \multicolumn{2}{l}{
...
\newcommand*{\rob¬state}{\vec{s}}
\newcommand*{\rob¬cyclesleft}{\vec{c}}
\newcommand*{\rob¬deps}{\vec{p}}
\newcommand*{\rob¬regs}{\vec{r}}
\newcommand*{\rob¬execunits}{\vec{x}}
\newcommand*{\ctr¬cycles}{\dot{c}}
\newcommand*{\ctr¬instructions}{\dot{n}}
\newcommand*{\remaining¬decodeslots}{\dot{d}}
\newcommand*{\remaining¬startpercycle}{\dot{e}}
\newcommand*{\remaining¬execunits}{\dot{x}}
...
\newcommand*{\state¬decoding}{1}
\newcommand*{\state¬waiting}{2}
\newcommand*{\state¬executing}{3}
\newcommand*{\state¬executed}{4}
\newcommand*{\ALU}{\text{A}}
...
\newcommand*{\emptyorderedmap}{\langle\,\rangle}
...
The gas cost $\gascostforblock{\imath}$ for a given basic block starting at instruction opcode index $\imath \in \basicblocks$ is defined by the number of virtual CPU cycles as determined by the the gas cost model transition function, up until every instruction of the basic block it has ingested has been retired and the simulation has converged.

The initial state $\Xi_0$ of the simulation is defined as follows:

\begin{equation}
  \Xi_0(\imath) = \abracegroup{
    \imath^{(0)} &= \imath \\
    \ctr¬cycles^{(0)} &= 0,
    \ctr¬instructions^{(0)} = 0,
    \remaining¬decodeslots^{(0)} = \const¬maxdecodepercycle,
    \remaining¬startpercycle^{(0)} = \const¬maxstartpercycle \\
    \rob¬state^{(0)} &= \emptyorderedmap,
    \rob¬cyclesleft^{(0)} = \emptyorderedmap,
    \rob¬deps^{(0)} = \emptyorderedmap,
    \rob¬regs^{(0)} = \emptyorderedmap,
    \rob¬execunits^{(0)} = \emptyorderedmap \\
    \remaining¬execunits^{(0)} &= \{
      \ALU = 4,
      \LOAD = 4,
      \STORE = 4,
      \MUL = 1,
...
\begin{equation}
  \Xi_{n + 1}(\Xi_n, \mathbf{c}, \mathbf{k}) = \begin{cases}
    \Xi_{n + 1}'(\Xi_n, \mathbf{c}, \mathbf{k}) &\when n = 0 \lor (\imath^{(n)} \neq \none \land \cost¬decodeslots(\mathbf{c}, \mathbf{k}, \imath^{(n)}) \leq \remaining¬decodeslots^{(n)} \land |\rob¬state^{(n)}| < 32) \\
    \Xi_{n + 1}''(\Xi_n) &\when \find¬readytostart(\Xi_n) \neq \none \land \remaining¬startpercycle^{(n)} > 0 \\
    \none &\when \imath^{(n)} = \none \land |\rob¬state^{(n)}| = 0 \\
    \Xi_{n + 1}'''(\Xi_n) &\otherwise \\
  \end{cases}
\end{equation}

Assuming $m \text{ such that } \Xi_{m + 1} = \none$ the final gas cost of a basic block starting at $\imath$ is defined as:

\begin{equation}\label{eq:gascostforblock}
  \gascostforblock{\imath} = \max(\ctr¬cycles^{(m)} - 3, 1)
\end{equation}

The state transition function $\Xi_{n + 1}'$ which decodes the instructions into a reorder buffer without triggering the virtual CPU pipeline simulation is defined as follows (those pieces of state which are not explicitly mentioned by the equations are assumed to be unchanged and omitted for clarity):

\begin{equation}
  \Xi_{n + 1}'(\Xi_n, \mathbf{c}, \mathbf{k}) = \begin{cases}
    \Xi_{n + 1}^{\text{mov}}(\Xi_n, \mathbf{c}, \mathbf{k}) &\when \text{opcode}(\mathbf{c}, \mathbf{k}, \imath^{(n)}) = \token{move\_reg} \\
    \Xi_{n + 1}^{\text{decode}}(\Xi_n, \mathbf{c}, \mathbf{k}) &\otherwise
  \end{cases}
\end{equation}
...
\begin{equation}
  \Xi_{n + 1}^{\text{mov}}(\Xi_n, \mathbf{c}, \mathbf{k}) = \abracegroup{
    \imath^{(n + 1)} &= \imath^{(n)} + 1 + \Fskip(\imath^{(n)}) \\
    \remaining¬decodeslots^{(n + 1)} &= \remaining¬decodeslots^{(n)} - 1 \\
    j \in \mathbb{N} \Rightarrow \rob¬regs^{(n + 1)}_j &= \begin{cases}
      \rob¬regs^{(n)}_j \cup \inst¬dstregs(\mathbf{c}, \mathbf{k}, \imath^{(n)}) &\when \rob¬regs^{(n)}_j \cap \inst¬srcregs(\mathbf{c}, \mathbf{k}, \imath^{(n)}) \neq \emptyset \\
      \rob¬regs^{(n)}_j \setminus \inst¬dstregs(\mathbf{c}, \mathbf{k}, \imath^{(n)}) &\otherwise
    \end{cases}
  }
\end{equation}
...
\begin{equation}
  \Xi_{n + 1}^{\text{decode}}(\Xi_n, \mathbf{c}, \mathbf{k}) = \abracegroup{
    \imath^{(n + 1)} &= \begin{cases}
      \none &\when \text{opcode}(\mathbf{c}, \mathbf{k}, \imath^{(n)}) \in T \\
      \imath^{(n)} + 1 + \Fskip(\imath^{(n)}) &\otherwise
    \end{cases} \\
    \remaining¬decodeslots^{(n + 1)} &= \remaining¬decodeslots^{(n)} - \cost¬decodeslots(\mathbf{c}, \mathbf{k}, \imath^{(n)}) \\
    \ctr¬instructions^{(n + 1)} &= \ctr¬instructions^{(n)} + 1 \\
    \rob¬state^{(n + 1)}_{\ctr¬instructions^{(n)}} &= \state¬decoding \\
    \rob¬cyclesleft^{(n + 1)}_{\ctr¬instructions^{(n)}} &= \cost¬cycles(\mathbf{c}, \mathbf{k}, \imath^{(n)}) \\
    \rob¬execunits^{(n + 1)}_{\ctr¬instructions^{(n)}} &= \cost¬execunits(\mathbf{c}, \mathbf{k}, \imath^{(n)}) \\
    \rob¬deps^{(n + 1)}_{\ctr¬instructions^{(n)}} &= \set{\build{j}{ \inst¬srcregs(\mathbf{c}, \mathbf{k}, \imath^{(n)}) \cap \rob¬regs^{(n)}_{j} \neq \emptyset }} \\
    j \in \mathbb{N} \Rightarrow \rob¬regs^{(n + 1)}_j &= \begin{cases}
      \inst¬dstregs(\mathbf{c}, \mathbf{k}, \imath^{(n)}) &\when j = \ctr¬instructions^{(n)} \\
      \rob¬regs^{(n)}_j \setminus \inst¬dstregs(\mathbf{c}, \mathbf{k}, \imath^{(n)}) &\otherwise
    \end{cases} \\
  }
\end{equation}

The state transition function $\Xi_{n + 1}''$ which starts the execution of the next pending instruction is defined as follows:

\begin{equation}
  \Xi_{n + 1}''(\Xi_n) = \abracegroup{
    \rob¬state^{(n + 1)}_{\find¬readytostart(\Xi_n)} &= \state¬executing \\
    \remaining¬execunits^{(n + 1)} &= \remaining¬execunits^{(n)} - \rob¬execunits^{(n)}_{\find¬readytostart(\Xi_n)} \\
    \remaining¬startpercycle^{(n + 1)} &= \remaining¬startpercycle^{(n)} - 1
  }
\end{equation}

The function $\find¬readytostart(\Xi_n)$ which checks which instruction inside of the reorder buffer is ready to start executing (and whether such an instruction even exists) is defined as follows:

\begin{equation}
  \find¬readytostart(\Xi_n) = \min(j \in \mathbb{N} \mid \rob¬state^{(n)}_j = \state¬waiting \land \rob¬execunits^{(n)}_j \leq \remaining¬execunits^{(n)} \land (\forall k \in \rob¬deps^{(n)}_j \Rightarrow \rob¬cyclesleft^{(n)}_k \leq 0))
\end{equation}

The state transition function $\Xi_{n + 1}'''$ which simulates the rest of the virtual CPU pipeline is defined as follows:

\begin{equation}
  \Xi_{n + 1}'''(\Xi_n) = \abracegroup{
    j \in \mathbb{N} \Rightarrow \rob¬state^{(n + 1)}_j &= \begin{cases}
      \none &\when \forall k \in \mathbb{N},\quad 0 \leq k \leq j \Rightarrow \rob¬state^{(n)}_k = \state¬executed \\
      \state¬waiting &\when \rob¬state^{(n)}_j = \state¬decoding \\
      \state¬executed &\when \rob¬state^{(n)}_j = \state¬executing \land \rob¬cyclesleft^{(n)}_j = 0 \\
      \rob¬state^{(n)}_j &\otherwise \\
    \end{cases} \\
    j \in \mathbb{N} \Rightarrow \rob¬cyclesleft^{(n + 1)}_j &= \begin{cases}
      \rob¬cyclesleft^{(n)}_j - 1 &\when \rob¬state^{(n)}_j = \state¬executing \\
      \rob¬cyclesleft^{(n)}_j &\otherwise \\
    \end{cases} \\
    j \in \mathbb{N} \Rightarrow \rob¬regs^{(n + 1)}_j &= \begin{cases}
      \none &\when \rob¬state^{(n)}_j = \state¬executing \land \rob¬cyclesleft^{(n)}_j = 1 \\
      \rob¬regs^{(n)}_j &\otherwise \\
    \end{cases} \\
    \remaining¬execunits^{(n + 1)} &= \remaining¬execunits^{(n)} + \sum_{j \in \mathbb{N} \Rightarrow \rob¬state^{(n)}_j = \state¬executing \land \rob¬cyclesleft^{(n)}_j = 1} \rob¬execunits^{(n)}_j \\
    \ctr¬cycles^{(n + 1)} &= \ctr¬cycles^{(n)} + 1 \\
    \remaining¬decodeslots^{(n + 1)} &= \const¬maxdecodepercycle \\
    \remaining¬startpercycle^{(n + 1)} &= \const¬maxstartpercycle
  }
\end{equation}

The function $\inst¬srcregs(\mathbf{c}, \mathbf{k}, \imath)$ returns a set of source registers read by a given instruction, and the function $\inst¬dstregs(\mathbf{c}, \mathbf{k}, \imath)$ returns a set of destination registers which are written by a given instruction, regardless of whether those registers would actually have been modified by that instruction when executed at runtime. \token{ecalli} is assumed to not read nor write to any registers in this model.

$\cost¬cycles$ is the number of cycles a given instruction needs to finish execution, $\cost¬decodeslots$ is the number of decoding slots necessary to decode it, and $\cost¬execunits$ is the number of virtual CPU execution units required to start its execution.
...
\begin{equation}
  \mathfrak{P}(a, b, \mathbf{c}, \mathbf{k}, \imath) = \begin{cases}
...
\begin{equation}
  \mathfrak{P}_{S}(a, b, \mathbf{c}, \mathbf{k}, \imath) = \begin{cases}
    a &\when \reg_A = \reg_D \\
    b &\otherwise
  \end{cases}
\end{equation}
...
The cost of a branch depends on whether any of its targets (either the jump target or the implicit fallthrough) points to an instruction byte which is equal to the opcode for the \token{unlikely} or the \token{trap} instruction; formally:

\begin{equation}
  \mathfrak{b}(\mathbf{c}, \mathbf{k}, \imath) = \begin{cases}
