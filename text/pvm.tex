...
On exit, the instruction counter $\imath'$ behavior depends on the exit reason. For \emph{halt} and \emph{panic} conditions, $\Psi$ returns $\imath' = 0$. For \emph{out-of-gas}, \emph{page fault}, and \emph{host-call} conditions, $\Psi$ returns $\imath'$ pointing to the instruction following the one that caused the exit.

When the machine exits due to out-of-gas or page fault, re-invoking with the returned instruction counter will re-execute the faulted instruction, which is appropriate since these conditions may be resolved (by providing more gas or making memory accessible) and the instruction retried.

For host-calls, $\Psi$ returns the exit reason as $\host \times h$ where $h$ is the host-call instruction value, while $\imath'$ points to the instruction following the \token{ecalli} instruction. This means that after successfully processing the host-call, the machine can resume execution directly using the returned instruction counter value without any adjustment.
